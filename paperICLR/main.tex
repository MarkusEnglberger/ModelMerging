...
\end{equation}
the model is updated immediately, $\theta^{(s,k)}=\theta^{(s,k-1)}+u_i^{(s,k)}$, with $\theta^{(s+1)}=\theta^{(s,T)}$ after all $T$ tasks in the sweep have been processed. Equation~\eqref{eq:clipped-update} is the weight--gate--clip step of the previous paragraph written in one line: on gated coordinates the sign condition makes $-g_{i,r}^{(s,k)}$ and $v_{i,r}^{(s,k)}$ agree, so the distance-weighted descent step $-\eta\,g_{i,r}^{(s,k)}|v_{i,r}^{(s,k)}|$ equals $\eta\,|g_{i,r}^{(s,k)}|\,v_{i,r}^{(s,k)}$, and the expert-distance cap limits the step fraction to one. The gate condition $g_{i,r}^{(s,k)}v_{i,r}^{(s,k)}<0$ keeps only the coordinates whose replay step also moves the current model toward expert $i$, and is exactly the attribution-patching sign that Section~\ref{sec:ap-view} derives in Equation~\eqref{eq:ap-gate}: the two perspectives agree coordinate by coordinate.
