...
We also test whether the methods preserve zero-shot capabilities on tasks excluded from merging and refinement. Models are built on the standard CLIP-8 suite and evaluated on seven additional CLIP-20 tasks on which the pretrained model performs meaningfully above chance. Every data-dependent method uses eight inputs per training task, and refinement hyperparameters are fixed from the corresponding CLIP-8 experiment; the full twelve-task split and protocol details are given in Appendix~\ref{app:clip20}.

APR retains more held-out accuracy than every checkpoint-only merge and both unconstrained refinements, and it dominates task arithmetic on both training accuracy and retention. AdaMerging remains the strongest matched-budget operating point in this lower-interference setting, so refinement from the pretrained model is not universally preferable to a strong data-fitted merge. The two ablations nevertheless show that APR's retention is not a consequence of using labels alone: it gains about $0.18$ training accuracy over ordinary descent and the ungated update while also preserving more held-out accuracy.

\begin{table}[t]
\centering
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{Held-out zero-shot retention after building models on CLIP-8 ($n=8$). The held-out score averages seven additional CLIP-20 tasks; parentheses report the change from $\theta_0$. Single seed; $^\ast$ marks the Pareto frontier.}
\label{tab:heldout8}
\renewcommand{\arraystretch}{0.95}
\begin{tabular}{lccc}
\toprule
...
AdaMerging-layer$^\ast$                   & $1.48$  & $0.720$ & $\mathbf{0.751}\ (-0.029)$ \\
Task arithmetic ($\lambda{=}0.3$)         & $2.19$  & $0.673$ & $0.628\ (-0.151)$ \\
Breadcrumbs                               & $2.16$  & $0.705$ & $0.716\ (-0.064)$ \\
TIES$^\ast$                               & $3.82$  & $\mathbf{0.741}$ & $0.702\ (-0.077)$ \\
