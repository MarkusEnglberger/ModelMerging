...
% NOTE(authors): this section describes protocol v2 (scripts/cv_protocol.py:
% equal budget, K-fold select-then-refit, D draws, auto-extended grids).
% Tables 1-2 still report the earlier n+n split-selection runs and must be
% replaced by the v2 results (CLIP-8 first: slurm/cv_protocol_clip8.sbatch)
% before submission; their captions describe the old protocol until then.
\paragraph{Protocol.}
Every method may spend its $B$ labels as its structure allows, and no method sees a label outside them. Merge baselines, whose candidates are built from the checkpoints or from unlabeled inputs, score every candidate on the full budget by replay loss and report the best. Methods whose construction consumes labels---APR and its ablations---choose their learning rate $\eta$ and sweep count $S$ by $K$-fold cross-validation \emph{within} the budget: construct on $K-1$ folds, score the held-out fold, rotate, and take the cell with the best mean held-out replay loss (each task's loss normalized by the initialization's loss on the same fold); the selected cell is then refit once on all $B$ examples, and that model is evaluated. The usual caveat of select-then-refit applies: hyperparameters are chosen at construction size $B(K-1)/K$ and applied at $B$. Grids are extended automatically whenever the selected cell lies on a boundary in $\eta$ or $S$, so every reported cell is an interior optimum. The evaluation split is read only for selected cells. The whole procedure---draw, select, refit, evaluate---is repeated over $D$ independent buffer draws, and every cell is reported as mean $\pm$ standard deviation over draws; no single-draw number appears in the tables. Refinement runs $S$ sweeps from the given initialization at a constant learning rate, with the task order re-randomized every sweep and the expert-distance cap applied per update (Eq.~\eqref{eq:clipped-update}); each refinement family receives an equal-size $(\eta,S)$ grid. This keeps the label cost of every method explicit: merge coefficients are conventionally tuned on the tasks' full validation splits, outside any stated budget \citep{ilharco2022taskarithmetic,yadav2023ties,he2025mergebench}, whereas here that tuning happens inside the same $B$ labels every method is charged for.

\paragraph{Baselines.}
We compare against checkpoint-only merges (task arithmetic, TIES, DARE-TIES, Breadcrumbs, DOGE/APGD), merges fitted on unlabeled inputs (AdaMerging, RegMean; given the same $B$ inputs per task that APR sees labeled), and other ways of spending the same labels (per-task coefficients, loss-weighted merging, replay-Fisher merging, learned Localize-and-Stitch masks, and head-only recalibration, the one deliberate exception to frozen heads; Appendix~\ref{app:protocol}). APR's two ablations---ordinary replay gradient descent (GD; no anchor, no gate, which is also the rehearsal baseline of continual learning) and the ungated distance-scaled update (anchor without gate)---share APR's optimizer, schedule, and grid, so that differences are attributable to the anchor and the gate. They are run from the pretrained model, where they isolate what each ingredient contributes to a cold start; from merge initializations only APR is run, and the comparison is against the merge it starts from.
