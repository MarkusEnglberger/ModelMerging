...
A major line of work addresses interference between task vectors: TIES-Merging trims small-magnitude updates and merges only sign-consistent components \citep{yadav2023ties}; DARE randomly drops most delta parameters and rescales the rest \citep{yu2023dare}, generalized by DELLA \citep{deep2024della}; Model Breadcrumbs removes negligible and outlier perturbations \citep{davari2023breadcrumbs}; Localize-and-Stitch stitches sparse task-relevant regions back into the pretrained model \citep{he2024localize}. Most closely related to the trust-region reading of our update (Section~\ref{sec:replay-refinement}), Task Arithmetic in Trust Region (TATR) uses task-loss gradients at the pretrained model to identify gradient-orthogonal parameter dimensions in which task vectors can be merged with limited interference \citep{sun2025tatr}. Closest in optimization form, DOGE models merging as adaptive projective gradient descent (APGD): it learns a shared modification to the task vectors under a data-free Taylor objective, projects its gradient orthogonally to an SVD-derived shared subspace, and sets layer-wise merge coefficients from task-vector norms \citep{wei2025apgd}. TATR and DOGE both restrict the merge itself without labeled data; our method instead evaluates labeled task gradients at the evolving model and takes sequential, coordinate-wise expert-directed refinement steps. A complementary line uses small amounts of data or model outputs: APL scores task vectors by a data-dependent contrast \citep{kong2024apl}, RegMean solves each linear layer by regression on its inputs \citep{jin2023regmean}, and AdaMerging learns coefficients by entropy minimization \citep{yang2024adamerging}. Twin-Merging, Task Vector Bases, and FusionBench extend and standardize this space \citep{lu2024twin,zeng2025taskvectorbases,tang2024fusionbench}. The present work does not introduce another pruning rule: it asks whether a small \emph{labeled} replay buffer can repair an initial merge while staying close to the known experts, using the sign of a merge-to-expert attribution only as a local gate for refinement.
...
\end{equation}
the model is updated immediately, $\theta^{(s,k)}=\theta^{(s,k-1)}+u_i^{(s,k)}$, with $\theta^{(s+1)}=\theta^{(s,T)}$ after all $T$ tasks in the sweep have been processed. Equation~\eqref{eq:clipped-update} is the weight--gate--clip step of the previous paragraph written in one line: on gated coordinates the sign condition makes $-g_{i,r}^{(s,k)}$ and $v_{i,r}^{(s,k)}$ agree, so the distance-weighted descent step $-\eta\,g_{i,r}^{(s,k)}|v_{i,r}^{(s,k)}|$ equals $\eta\,|g_{i,r}^{(s,k)}|\,v_{i,r}^{(s,k)}$, and clipping it to the expert-distance trust region caps the step fraction at one. The gate condition $g_{i,r}^{(s,k)}v_{i,r}^{(s,k)}<0$ keeps only the coordinates whose replay step also moves the current model toward expert $i$, and is exactly the attribution-patching sign that Section~\ref{sec:ap-view} derives in Equation~\eqref{eq:ap-gate}: the two perspectives agree coordinate by coordinate.

Because the step fraction $\min(\eta\,|g_{i,r}^{(s,k)}|,1)$ never exceeds one, per-coordinate movement is bounded by $|v_{i,r}^{(s,k)}|$ for any $\eta$: the learning rate controls how many coordinates reach the trust-region boundary, never the maximum step. Optionally, $u_i^{(s,k)}$ is RMS-normalized tensor-wise. Algorithm~\ref{alg:expert-gated-refinement} summarizes the procedure.
...
\paragraph{Protocol.}
Refinement runs $S$ sweeps from the given initialization at a constant learning rate, with the trust-region clip applied per-update (Eq.~\eqref{eq:clipped-update}).
...
Pretrained      & alone      & $0$ & $0$ & $0.289\ (.06)$ & $0.550\ (.10)$ \\
                ...
                & $+$APR     & $0.315\ ({-}.12)$ & $0.432\ ({-}.16)$ & $0.365^{\dagger}\ (.22)$ & $0.647\ (.22)$ \\
\midrule
...
\scriptsize
\setlength{\tabcolsep}{3.5pt}
% TODO(authors): slurm array 25964903 (slurm/mb3b_apr_bracket_random.sbatch) is
% re-running the MergeBench APR cells whose selected (lr, S) sat on the grid
% edge -- pretrained +APR at 16+16 (Table 1) and 8+8, and TA +APR at 8+8 --
% with lr up to 8192 and S up to 12. Replace those three cells (and the
% per-task appendix rows + Section 5.1 numbers) from
% results/compare/grid_nn_mb3b_{pretrained_n16,pretrained_n8,ta_n8}_apr_s2to12_random.json
% when it lands.
\caption{\textbf{The canonical grid under split selection at an $8{+}8$ budget.} Each task contributes eight labeled replay examples and a disjoint eight-example selection buffer. Every hyperparameter is selected on the selection buffer before the evaluation split is read; the encoder tracks use $S\in\{5,20,50\}$ with a fixed task order, and MergeBench uses $S\in\{1,2,3,5\}$ with the task order re-randomized each sweep (the same grid as its from-pretrained cells in Table~\ref{tab:main16}). DOGE/APGD remains checkpoint-only and uses this buffer only to select $\eta$. Entries follow Table~\ref{tab:main16}: mean normalized retention with worst-task retention in parentheses, except MergeBench and CLIP-20, which report absolute accuracy. The decoder-only MergeBench track uses Llama-3.2-3B with mathematics, coding, instruction-following, and multilingual experts; its pretrained floor is $0.289$ and expert ceiling $0.548$. $^{\dagger}$ marks a selected horizon at the longest value offered; dashes are configurations not run.}
\label{tab:main8}
\renewcommand{\arraystretch}{0.95}
\begin{tabular}{llcccc}
\toprule
...
Pretrained      & alone      & $0$ & $0$ & $0.289\ (.06)$ & $0.550\ (.10)$ \\
                ...
                & $+$APR     & $0.223^{\dagger}\ ({-}.33)$ & $0.295^{\dagger}\ ({-}.45)$ & $0.378^{\dagger}\ (.27)$ & $0.645\ (.22)$ \\
\midrule
Task arithmetic & alone      & $0.320\ ({-}.18)$ & $0.270\ ({-}.53)$ & $0.470\ (.33)$ & $0.495\ (.15)$ \\
                ...
                & $+$APR     & $0.498\ ({-}.18)$ & $0.462^{\dagger}\ ({-}.12)$ & $0.481^{\dagger}\ (.37)$ & $0.648\ (.22)$ \\
\midrule
...
DARE-TIES       & alone      & $0.301\ ({-}.34)$ & $0.364\ ({-}.32)$ & $0.490\ (.35)$ & $0.520\ (.20)$ \\
                ...
                & $+$APR     & $\mathbf{0.576}\ ({+}.19)$ & $0.498^{\dagger}\ ({-}.31)$ & $0.495\ (.37)$ & $0.649^{\dagger}\ (.27)$ \\
\midrule
TIES            & alone      & $0.208\ ({-}.22)$ & $0.525\ ({+}.21)$ & $0.464\ (.36)$ & $0.619\ (.19)$ \\
                ...
                & $+$APR     & $0.489^{\dagger}\ ({+}.01)$ & $0.566^{\dagger}\ ({-}.08)$ & $0.468\ (\mathbf{.38})$ & $0.665\ (.28)$ \\
\midrule
DOGE/APGD       & alone      & $0.016\ ({+}.00)$ & $0.138\ ({-}1.93)$ & --- & $0.516\ (.16)$ \\
\midrule
AdaMerging      & alone      & $0.028\ ({-}3.01)$ & $0.463\ ({-}.04)$ & $\mathbf{0.496}\ (.36)$ & $0.592\ (.10)$ \\
...
Tables~\ref{tab:glue8-n8-pertask}--\ref{tab:mergebench-n8-pertask}
give the task-level scores underlying the GLUE-8, CLIP-8, and MergeBench
...
accuracy, and MergeBench reports the judge-free automatic task scores described
...
\midrule
Pretrained & $+$APR & $0.333$ & $0.407$ & $0.267$ & $0.504$ & $0.378$ \\
\midrule
Task arithmetic & alone & $0.613$ & $0.430$ & $0.333$ & $0.504$ & $0.470$ \\
                & $+$APR & $0.577$ & $0.477$ & $0.367$ & $0.504$ & $0.481$ \\
\midrule
