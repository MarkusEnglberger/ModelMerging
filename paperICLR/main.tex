...
\paragraph{Baselines.}
Baselines are grouped by the data they consume, mirroring Section~\ref{sec:related}: \emph{checkpoint-only} (task arithmetic, TIES, DARE-TIES, Breadcrumbs, DOGE/APGD); \emph{data-dependent but label-free} (AdaMerging, RegMean); and \emph{labeled}---the regime APR itself occupies---where the grid carries its two ablations, ordinary replay GD from the same initialization and the ungated distance-scaled update, isolating the anchor and the gate, so that all three labeled arms consume identical data: the same replay examples, the same selection buffer, and the same hyperparameter search budget. Sanity controls (the inverted and density-matched random gates) complete the roster. Each merge baseline's own hyperparameters (coefficient, density, sparsity, or global APGD scale) are likewise selected on the selection buffer; a wider labeled roster (coefficient tuning on the buffer, replay-Fisher merging, learned Localize-and-Stitch masks, head-only recalibration) is catalogued in Appendix~\ref{app:protocol}.
