...
\centering
\small
\caption{\textbf{Eight-task CLIP under the equal-budget protocol} ($B{=}32$ labels per task for every method, mean $\pm$ std over three buffer draws). Mean and worst-task top-1 accuracy; every selected cell interior in $\eta$ and $S$. Merges are deterministic given the checkpoints, so their spread reflects selection alone. GD and ungated are APR's ablations, run from the pretrained model where they isolate the anchor and the gate.}
\label{tab:cv-clip8}
\begin{tabular}{lcc}
\toprule
Model & Mean acc. & Worst-task acc. \\
\midrule
Zero-shot $\thetab$        & $0.478$ & $0.315$ \\
\midrule
Task arithmetic            & $0.692\pm0.000$ & $0.518\pm0.000$ \\
Breadcrumbs                & $0.705\pm0.000$ & $0.521\pm0.000$ \\
DARE-TIES                  & $0.708\pm0.000$ & $0.486\pm0.000$ \\
TIES                       & $0.741\pm0.000$ & $\mathbf{0.528}\pm0.000$ \\
\midrule
$\thetab+$GD (ablation)      & $0.558\pm0.018$ & $0.369\pm0.039$ \\
$\thetab+$ungated (ablation) & $0.565\pm0.008$ & $0.401\pm0.024$ \\
$\thetab+$APR                & $\mathbf{0.756}\pm0.004$ & $0.523\pm0.007$ \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:cv-clip8} reports the eight-task CLIP suite under the protocol of Section~\ref{sec:protocol}: every method given the same $B=32$ labels per task, hyperparameters selected inside that budget, every cell an interior optimum, and every number a mean over three independent buffer draws. From the pretrained model APR reaches $0.756\pm0.004$ mean accuracy, above every checkpoint-only merge including TIES ($0.741$), while using $32$ labels per task in place of the eight expert checkpoints a merge requires. Its two ablations sit at $0.558$ and $0.565$---barely above the zero-shot floor once one accounts for the merges' head start---so the anchored, capped step is worth ${\approx}0.19$ accuracy over unconstrained descent from the same initialization, and the gate alone does not account for it.
