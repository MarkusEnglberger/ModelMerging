...
One axis of the grid is genuinely missing, however, and we tested it rather than assume it away. Every cell in Table~\ref{tab:main16} runs APR at a constant rate with a fixed task order, whereas the $n=64$ control of Table~\ref{tab:longapr} found the annealed package---cosine decay to $5\%$ of the peak, task order re-randomized each sweep---worth as much as $+0.10$ on GLUE-8. Offering both schedules to split selection on CLIP-8, from task arithmetic and from TIES, splits the difference. From TIES the annealed configuration is better on the selection buffer \emph{and} on the evaluation split, lifting the cell from $0.548$ to $0.615$---past GD's $0.599$, so this row's ordering was an artifact of the missing axis. From task arithmetic it goes the other way: annealing wins on the selection buffer by a clear margin ($0.458$ vs.\ $0.511$) but is \emph{worse} on the evaluation split ($0.495$ vs.\ $0.524$), so honest selection costs $0.029$ here by preferring a configuration the metric does not. The axis therefore matters, is not uniformly favorable, and is one we have so far searched on only two of five initializations; we report the CLIP-8 composition row at the uniform constant-schedule budget of Table~\ref{tab:main16} rather than mixing search budgets across rows, and flag the TIES cell as a lower bound of $0.615$.
