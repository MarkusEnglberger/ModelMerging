...
We introduce expert-anchored gradient descent, which can start from any merge or directly from the pretrained model. 
For each task, a first-order loss attribution selects coordinates where moving toward its expert is locally beneficial; updates are scaled by their distance to the expert and clipped to prevent overshoot. This yields an equivalent attribution-patching interpretation, and the update provably can never overshoot the experts at any learning rate. We evaluate the method on multi-head GLUE-8 and the eight- and twenty-task CLIP suites, using at most sixteen labeled examples per task and tuning only on held-out buffers. From the pretrained model, the anchored update performs best on every benchmark; from merge initializations, it typically improves performance and reduces sensitivity to the initial merge. With eight examples per task, it still outperforms tuned task arithmetic while unconstrained replay descent often overfits. On tasks never merged or refined, it retains more of the pretrained model's zero-shot ability than any checkpoint-only merge.
\end{abstract}
\section{Introduction}
\label{sec:intro}
Merging fine-tuned variants of a shared pretrained model combines their skills in weight space, without retraining, ensembling, or routing. But merges degrade under task interference, and the data-free repairs---sparsification, sign election, learned coefficients---plateau well below the experts. In many practical settings a small \emph{labeled} replay buffer per task is available.

We refine the merged model with replay gradients \emph{anchored to the task experts}: each coordinate moves toward its expert only where a first-order attribution score predicts that the move reduces the task's replay loss, by an amount proportional to that predicted saving, and never past the expert---applied task-sequentially, from \emph{any} initialization, including the pretrained model itself. The update is provably non-expansive toward the experts' box hull at any learning rate (Proposition~\ref{prop:box-nonexpansive}).

Our contributions are: (i)~a replay-refinement method with two equivalent derivations---expert-anchored gradient descent and parameter-space attribution patching---and a divergence-safety guarantee; (ii)~a uniform evaluation across three benchmark families (multi-head GLUE-8 and the eight- and twenty-task CLIP suites) where every hyperparameter is chosen on a small held-out selection buffer, with matched data budgets; (iii)~evidence that from the pretrained model---the setting with no merge to inherit---the anchored update is the only one of the three replay families that turns a $16$-example buffer into multi-task ability, while from merge initializations it usually improves performance; (iv)~a measurement of what the refinement costs the pretrained model on tasks never merged or refined, under a train/held-out split: the anchored update dominates both unconstrained controls on accuracy and retention simultaneously, and retains more than every checkpoint-only merge.

\section{Related Work and Background}
\label{sec:related}

\paragraph{Merging with checkpoints alone.}
Model merging combines fine-tuned variants of a shared pretrained model into a single model in weight space, avoiding ensembling overhead. The basic operations use nothing but the checkpoints: model soups average compatible ones \citep{wortsman2022modelsoups}, and task arithmetic represents fine-tuning as a \emph{task vector} $\taskvec_t=\theta_t-\thetab$ and merges as $\thetab+\sum_t\lambda_t\taskvec_t$ \citep{ilharco2022taskarithmetic}. Interference between task vectors---over-counted directions, sign and magnitude conflicts---is the central failure mode, and the checkpoint-only repairs differ mainly in the rule that suppresses it: TIES-Merging trims small-magnitude updates and merges only sign-consistent components \citep{yadav2023ties}; DARE randomly drops most delta parameters and rescales the rest \citep{yu2023dare}, generalized by DELLA \citep{deep2024della}; Model Breadcrumbs removes negligible and outlier perturbations \citep{davari2023breadcrumbs}; magnitude-based Localize-and-Stitch stitches sparse task-relevant regions back into the pretrained model \citep{he2024localize}; and, closest in optimization form to our update, DOGE models merging as adaptive projective gradient descent (APGD), learning a shared modification to the task vectors under a data-free Taylor objective with gradients projected orthogonally to an SVD-derived shared subspace \citep{wei2025apgd}.

\paragraph{Data-dependent merging.}
A second line spends a small amount of data---usually unlabeled inputs or model outputs, not labels---to shape the merge: Fisher merging weights parameters by Fisher information \citep{matena2022fisher}; RegMean solves each linear layer by regression on its inputs \citep{jin2023regmean}; AdaMerging learns merge coefficients by entropy minimization \citep{yang2024adamerging}; APL scores task vectors by a data-dependent contrast \citep{kong2024apl}; and Task Arithmetic in Trust Region (TATR) uses task-loss gradients at the pretrained model to identify low-interference dimensions in which to merge \citep{sun2025tatr}. Twin-Merging, Task Vector Bases, and FusionBench extend and standardize this space \citep{lu2024twin,zeng2025taskvectorbases,tang2024fusionbench}. Common to both lines, whatever data is used shapes coefficients, regressions, or masks that are fixed at merge time, and nothing corrects the merged model afterwards. The present work operates one regime further: it asks whether a small \emph{labeled} replay buffer can repair an initial merge---or build multi-task ability directly from the pretrained model---while staying close to the known experts, evaluating labeled task gradients at the \emph{evolving} model and taking sequential, coordinate-wise steps toward the experts sized by per-coordinate attribution scores, rather than introducing another pruning rule.
...
% ---------------------------------------------------------------------------
\paragraph{Settings.}
We evaluate on three benchmark families. \emph{Multi-head} GLUE-8 (CoLA, SST-2, MRPC, STS-B, QQP, MNLI, QNLI, RTE) \citep{wang2018glue} with RoBERTa-base \citep{liu2019roberta} merges the shared encoder and evaluates with each task's own trained head---an intentionally \emph{oracle-task-id} setting. A \emph{CLIP/ViT-B/32} vision track uses the standard eight-task task-vector suite \citep{radford2021clip,ilharco2022taskarithmetic} and its twenty-task extension \citep{wang2024tall}, probing a different modality and---since more task vectors then compete for the same encoder weights---a substantially higher-interference regime (Section~\ref{sec:clip20}). Heads are never fine-tuned, following each benchmark's convention: GLUE-8 keeps each task's trained head, and the CLIP suites keep their frozen text-derived zero-shot heads. Refinement uses $n\le16$ labeled replay examples per task, disjoint from both the selection buffer and the evaluation split; Appendix~\ref{app:protocol} specifies the exact sampling rule.


\paragraph{Protocol.}
Each task contributes two disjoint labeled buffers of equal size ($8$ or $16$ examples each): a \emph{replay} buffer that the labeled methods consume, and a \emph{selection} buffer on which every hyperparameter of every method---merge coefficient, density, sparsity, APGD scale, refinement learning rate, and sweep count $S$---is chosen by held-out loss, so the evaluation split is read only at the selected cells. Refinement runs $S$ sweeps from the given initialization at a constant learning rate with the expert-distance cap applied per-update (Eq.~\eqref{eq:clipped-update}), and each refinement family receives an equal-size $(\eta,S)$ search. This accounting makes explicit a cost the literature usually leaves uncounted: in prior work, ``training-free'' merges are reported at coefficients tuned on the tasks' full validation splits---thousands of labeled examples outside any stated budget \citep{ilharco2022taskarithmetic,yadav2023ties}---and MergeBench finds that this tuning dominates the cost of merging \citep{he2025mergebench}. In our protocol the same tuning exists for every method (TIES's density and $\lambda$ included) but is confined to the $n$-example selection buffer and counted in the stated $n{+}n$ budget. Strictly, only the pretrained model is data-free. One asymmetry of this design should be stated: methods whose construction consumes no replay data effectively use only $n$ of the $n{+}n$ labels---we hold the selection buffer identical across all methods rather than granting them the unused replay half as extra selection data, so selection is performed under the same conditions everywhere, at the price of slightly under-resourcing the tiers that could have spent those labels on a less noisy selection.

\paragraph{Baselines.}
Since selection is common to every method, no tier below is data-free; the tiers are bands on a single axis---how much label information can reach the reported weights---and differ in degree, not kind, mirroring Section~\ref{sec:related}:
\begin{itemize}[leftmargin=1.2em,itemsep=1pt,topsep=2pt]
\item \emph{Checkpoint-only construction}: task arithmetic, TIES, DARE-TIES, Breadcrumbs, and DOGE/APGD. Every candidate is a deterministic function of the checkpoints; the labels only pick one of the $K\le9$ candidates each grid offers, so at most $\log_2 K < 4$ bits of label information ever reach the reported model---against the unbounded label access of the labeled tier.
\item \emph{Unlabeled construction}: AdaMerging and RegMean, fitted on the same $n$ unlabeled inputs per task that APR sees labeled.
\item \emph{Labeled construction}, the tier APR occupies. Its ablations consume identical data and appear in every grid: ordinary replay gradient descent (GD; no anchor, no gate---simultaneously the rehearsal baseline of continual learning, with optimizer and schedule matched to APR's so that differences are attributable to the anchor and the gate) and the ungated distance-scaled update (anchor without gate). The alternative uses of the same labels---per-task coefficients, loss-weighted merging, replay-Fisher merging, learned Localize-and-Stitch masks, and head-only recalibration, the one deliberate exception to frozen heads (full roster in Appendix~\ref{app:protocol})---are reported at the $16{+}16$ budget in the labeled-alternatives block of Table~\ref{tab:main16}.
\item \emph{Sanity controls}: the inverted and density-matched random gates.
\end{itemize}

\paragraph{Metrics.}
Tasks are scored by their standard validation metrics: top-1 accuracy on the CLIP suites and the conventional per-task GLUE metrics (enumerated in Appendix~\ref{app:protocol}). These are not commensurable across tasks---a Matthews correlation and an accuracy do not average meaningfully---so the primary aggregate throughout is normalized retention,
\begin{equation}
    \mathrm{NormRet}_t
    =
    \frac{
...
\end{equation}
which places the pretrained model at $0$ and the task expert at $1$ ($\epsilon_{\mathrm{metric}}$ guards small denominators). Mean and worst-task values are always reported together---worst-task matters because a merge can improve the average while catastrophically harming a small task. One benchmark departs from this convention because its retention denominators degenerate: on the twenty-task CLIP suite several experts barely improve on the zero-shot backbone, so a near-zero denominator makes per-task retention explode. That benchmark therefore reports absolute mean and worst-task accuracy, quoting the zero-shot floor and expert ceiling for scale (Section~\ref{sec:clip20}).
...
\midrule
% TODO(authors): the labeled-alternatives block is partial -- per-task
% coefficients, loss weighting, and head-only recalibration are still to be
% run under split selection (extend scripts/labeled_baselines_n16.py
% --baselines and fill from grid_nn_{glue8,clip8,clip20}_labeled_n16.json).
% Replay fine-tuning is NOT a separate row (the GD ablation is that baseline),
% and neither is GLOBAL coefficient tuning: under split selection it coincides
% with the task-arithmetic row, whose lambda is already selected on the
% buffer. The row here is per-task lambda_i by coordinate descent on the
% replay buffer.
...
RegMean         & alone      & $0.477\ ({+}.06)$ & $0.627\ ({+}.32)$ & $0.697\ (.21)$ \\
                ...
                & $+$ungated & $0.462\ ({+}.07)$ & $0.622^{\dagger}\ ({+}.28)$ & $\mathbf{0.709}\ (.24)$ \\
...
\label{sec:clip20}
Table~\ref{tab:main16} reports the three encoder benchmarks at a labeled budget of $16$ refinement and $16$ selection examples per task; Table~\ref{tab:main8} repeats them at half that budget. Two conventions matter for reading the CLIP-20 column. First, it is scored in absolute top-1 accuracy (zero-shot floor $0.550$, expert ceiling $0.896$), for the degeneracy reason declared in Section~\ref{sec:protocol}; on that scale interference is visibly dominant---the best checkpoint-only merge (TIES) recovers only a fifth of the floor-to-ceiling gap, against roughly half of the expert--base gap at eight tasks ($0.525$)---which Appendix~\ref{app:clip20} traces to the geometry of the twenty-expert compromise. Second, its task-arithmetic row carries a protocol artifact: the coefficient grid offered to selection is the standard eight-task one ($\lambda\in\{0.2,\ldots,0.5\}$), whose optimum at twenty tasks lies \emph{outside} the grid ($\lambda\approx0.05$--$0.08$, accuracy $0.604$; Appendix~\ref{app:clip20}), so the selected $\lambda=0.2$ is a boundary pick and that merge lands below the zero-shot floor ($0.495$).
...
\caption{Where good solutions sit relative to the pretrained model, on a single absolute frame: $x$ is the distance from $\thetab$ (log), $y$ the absolute score, so every point shares one origin. Coloured markers are the three split-selected \emph{from-pretrained} cells of Table~\ref{tab:main16}; gray squares are the merge baselines at their own $\thetab$-distance. Only from-pretrained cells are plotted, because a refinement launched from a merge inherits that merge's score and is displaced from a different origin, so such points share no common frame with these and their comparison is made in Table~\ref{tab:main16} instead. Refinement is a small-displacement operation relative to merging: on GLUE-8 the tuned merge sits ${\approx}108$ from $\thetab$ for retention $0.320$, while APR reaches $0.315$ at distance $2.5$---matching it at ${\approx}2\%$ of the drift---and on all three benchmarks APR lies up and to the right of both unconstrained arms, converting distance into score where they do not. The gray squares are label-free, and the comparison they support is drift at comparable score rather than a claim that from-pretrained refinement outscores every merge: on CLIP-8 the best of them (TIES, $0.525$) is above APR's $0.432$, at ${\approx}3.6{\times}$ the distance, and refinement launched \emph{from} that merge is the stronger comparison, reported in Table~\ref{tab:main16}. Parameter distance is comparable in magnitude but is not a drift proxy \emph{across} merge families with different geometry (RegMean sits hundreds of units out while retaining well; Appendix~\ref{app:clip20}), so whether these small displacements preserve pretrained capability is measured rather than inferred, in Table~\ref{tab:drift} and the held-out-task study below.}
\label{fig:n16disp}
\end{figure}
...
\paragraph{Held-out-task retention.}
The drift probe measures retention in the base model's own objective; what matters downstream is retained \emph{ability}: whether the refined model still performs tasks that were never merged or refined. For this we move to the vision track and split the twenty-task suite along a boundary fixed by the literature rather than by us: train on exactly the eight-task CLIP suite, hold out the twelve remaining tasks, evaluated purely zero-shot through frozen text-derived heads so that nothing task-specific leaks. The budget is $n=8$ and every refinement cell is pinned at the split-selected $(\eta,S)$ of the corresponding CLIP-8 experiment, so nothing is tuned or selected inside the study. Seven held-out tasks have meaningful base accuracy (mean $0.780$) and form the primary aggregate; the five at or near chance---KMNIST and EMNIST at ${\approx}0.10$, the binary PCAM and RenderedSST2 barely above the coin flip, FER2013 marginal---are recorded but reported separately, by a rule fixed before the run: there is nothing to retain on a chance-level task, and averaging them in dilutes every drop by roughly a third. Only refinement from the pretrained model is reported, since a merge initialization carries no retention guarantee of its own and a composed pipeline's held-out score would confound the two.

Table~\ref{tab:heldout8} splits in two. Against the checkpoint-only merges, APR retains more than every one of them---a $-0.047$ held-out drop against $-0.064$ (Breadcrumbs), $-0.077$ (TIES), $-0.151$ (task arithmetic), and $-0.166$ (DARE-TIES)---and in the task-arithmetic case it dominates outright, with more training accuracy \emph{and} more retention. Against the strongest label-free merge it does not: at eight training tasks the interference merging has to repair is mild, AdaMerging reaches $0.720$ training accuracy at a $-0.029$ drop, and it dominates from-pretrained refinement on both axes, so the frontier consists of the base model, AdaMerging, and TIES, and contains no refinement point. This holds at matched data: AdaMerging is fitted here from the same eight unlabeled inputs per task that APR sees labeled. Its standard \emph{transductive} formulation---entropy minimised on the full evaluation sets of the eight training tasks, $87{,}433$ unlabeled images against APR's $64$ labeled examples---reaches $0.759$ at a $-0.031$ drop instead, so the extra data buys ${\approx}0.04$ of training accuracy on the split it adapts to and essentially nothing in retention: the retention ordering is a property of the objective rather than of data access. The honest reading is that the refinement buys retention relative to checkpoint-only merging (and, below, relative to unconstrained use of the same labels), not that it is the best available operating point wherever interference is mild. Within the labeled tier, the two ablation arms confirm that this retention belongs to the anchored update rather than to small labeled budgets per se: at the same pinned budget, ordinary descent and the ungated update reach ${\approx}0.18$ less training accuracy than APR while losing \emph{more} held-out accuracy ($-0.082$ and $-0.080$ against $-0.047$). RegMean reverses relative to its behaviour at larger Gram budgets, finishing worst-retaining in the table ($-0.318$ at eight inputs), so its inference-invisible displacement is budget-dependent rather than a general property.

\begin{table}[t]
\centering
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{Held-out retention under the literature-fixed split: train on the eight-task CLIP suite, hold out the twelve remaining tasks of the twenty-task suite, evaluated zero-shot through frozen heads. $n=8$, single seed; every refinement cell is pinned at the split-selected $(\eta,S)$ of the corresponding CLIP-8 $8{+}8$ experiment (constant rate, fixed order), and the merge hyperparameters are those experiments' selections, so nothing is tuned inside this study. ``Good-7'' averages the seven held-out tasks with meaningful base accuracy (rule fixed before the run); the five near-chance tasks are excluded from the primary aggregate and dilute every drop by roughly a third when averaged in (all-12 base $0.599$). Only refinement from the pretrained model is listed: an initialization that is itself a merge carries no retention guarantee, so a composed pipeline's held-out score confounds what the refinement did with what it inherited. Every data-dependent method here draws on the \emph{same} eight unlabeled inputs per task that APR sees labeled---AdaMerging's entropy objective and RegMean's Gram matrices included---so no row is given data the others are denied. $^\ast$ marks the train/good-7 Pareto frontier.}
\label{tab:heldout8}
\renewcommand{\arraystretch}{0.95}
\begin{tabular}{lccc}
\toprule
Model (built from the 8 train tasks) & Dist.\ $\theta_0$ & Train-8 & Good-7 held-out (drop) \\
\midrule
...
This work develops an attribution-patching perspective on replay-guided merging: the sign of $g_{i,r}v_{i,r}$ tells, per coordinate, whether moving toward expert $i$ locally reduces replay loss; the same sign gates the anchored descent step, and both views motivate one procedure---mask, distance-scale, clip, apply task-sequentially. Evaluated under a protocol that never reads the evaluation split at $16$ labeled examples per task or fewer, the refinement is a \emph{complement} to merging: it typically improves its initialization, reduces sensitivity to which merge it was handed, and can repair the worst task even when the mean is flat. Its decisive setting is the one with no merge to inherit---from the pretrained model the gated, expert-anchored update is the best treatment on every benchmark reported, while neither unconstrained baseline is viable at this budget---and its advantage grows as the buffer shrinks, where descent overfits while its replay loss still improves and the dedicated safety audit finds no corresponding anchored overfitting signature. Refining from the pretrained model also turns out to be cheap in what it costs the pretrained model: it reaches merge-level multitask retention while inflating the base objective's loss by well under half of what the merges at that level do (only TIES drifts less, at markedly lower task retention), and it does so while moving \emph{farther} in parameter space than ordinary descent, which drifts more---so the anchor buys a better-directed path rather than a shorter one. The same conclusion holds one level up, on retained \emph{ability} rather than the pretraining objective: on tasks never merged or refined, the anchored update dominates both unconstrained controls on accuracy and retention simultaneously and retains more than every checkpoint-only merge, with the gate---not just the anchor---supplying the retention. It does not dominate everything: where interference is mild enough that merging has little to repair, the strongest label-free merge retains more than refinement from $\thetab$ does. The result is a lightweight bridge between checkpoint-only task arithmetic and full multitask replay fine-tuning, usable at a labeled budget of a few dozen examples per task.
