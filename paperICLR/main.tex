...
\label{tab:main16}
\renewcommand{\arraystretch}{0.95}
% TODO(authors): the MergeBench column here is empty -- that track has only been
% run at 8+8 (Table~\ref{tab:main8}). Either run it at 16+16 and fill this in,
% or drop the column and say in the caption that MergeBench is 8+8 only.
\begin{tabular}{llcccc}
\toprule
...
\scriptsize
\setlength{\tabcolsep}{3.5pt}
\caption{\textbf{The canonical grid under split selection at an $8{+}8$ budget.} Each task contributes eight labeled replay examples and a disjoint eight-example selection buffer. Every hyperparameter is selected on the selection buffer before the evaluation split is read; the encoder tracks use $S\in\{5,20,50\}$ and MergeBench uses $S\in\{5,20,50,100\}$. DOGE/APGD remains checkpoint-only and uses this buffer only to select $\eta$. Entries follow Table~\ref{tab:main16}: mean normalized retention with worst-task retention in parentheses, except MergeBench and CLIP-20, which report absolute accuracy. The decoder-only MergeBench track uses Llama-3.2-3B with mathematics, coding, instruction-following, and multilingual experts; its pretrained floor is $0.289$ and expert ceiling $0.548$. $^{\dagger}$ marks a selected horizon at the longest value offered; dashes are configurations not run.}
\label{tab:main8}
\renewcommand{\arraystretch}{0.95}
\begin{tabular}{llcccc}
\toprule
...
Pretrained      & alone      & $0$ & $0$ & $0.289\ (.06)$ & $0.550\ ({+}.10)$ \\
                & $+$GD      & $-0.361\ ({-}3.52)$ & $-0.148\ ({-}1.04)$ & $0.333\ (.15)$ & $0.574^{\dagger}\ ({+}.10)$ \\
                & $+$ungated & $0.089^{\dagger}\ ({-}.31)$ & $-0.006\ ({-}.41)$ & --- & $0.568\ ({+}.08)$ \\
                & $+$APR     & $0.223^{\dagger}\ ({-}.33)$ & $0.295^{\dagger}\ ({-}.45)$ & $\mathbf{0.337}\ (\mathbf{.20})$ & $0.645\ ({+}.22)$ \\
\midrule
Task arithmetic & alone      & $0.320\ ({-}.18)$ & $0.270\ ({-}.53)$ & $0.469\ (.33)$ & $0.495\ ({+}.15)$ \\
                & $+$GD      & $0.530\ ({+}.04)$ & $0.167^{\dagger}\ ({-}1.10)$ & --- & $0.545\ ({+}.18)$ \\
                & $+$ungated & $0.529\ ({+}.03)$ & $0.241^{\dagger}\ ({-}.79)$ & --- & $0.576\ ({+}.23)$ \\
                & $+$APR     & $0.498\ ({-}.18)$ & $0.462^{\dagger}\ ({-}.12)$ & $0.477\ (.36)$ & $0.648\ ({+}.22)$ \\
\midrule
Breadcrumbs     & alone      & $0.030\ ({-}2.14)$ & $0.431\ ({+}.08)$ & $0.416\ (.25)$ & $0.602\ ({+}.12)$ \\
                & $+$GD      & $0.512\ ({-}.00)$ & $0.479^{\dagger}\ ({+}.04)$ & --- & $0.632^{\dagger}\ ({+}.15)$ \\
                & $+$ungated & $0.536\ ({+}.05)$ & $0.391^{\dagger}\ ({-}.40)$ & --- & $0.624\ ({+}.17)$ \\
                & $+$APR     & $0.528\ ({-}.06)$ & $0.339^{\dagger}\ ({-}.46)$ & $0.414\ (.31)$ & $0.652\ ({+}.22)$ \\
\midrule
DARE-TIES       & alone      & $0.301\ ({-}.34)$ & $0.364\ ({-}.32)$ & $0.490\ (.35)$ & $0.520\ ({+}.20)$ \\
                ...
                & $+$APR     & $\mathbf{0.576}\ ({+}.19)$ & $0.498^{\dagger}\ ({-}.31)$ & $\mathbf{0.508}\ (\mathbf{.39})$ & $0.649^{\dagger}\ ({+}.27)$ \\
\midrule
TIES            & alone      & $0.208\ ({-}.22)$ & $0.525\ ({+}.21)$ & $0.464\ (.36)$ & $0.619\ ({+}.19)$ \\
                ...
                & $+$APR     & $0.489^{\dagger}\ ({+}.01)$ & $0.566^{\dagger}\ ({-}.08)$ & $0.467\ (.37)$ & $0.665\ ({+}.28)$ \\
\midrule
DOGE/APGD       & alone      & $0.016\ ({+}.00)$ & $0.138\ ({-}1.93)$ & --- & $0.516\ ({+}.16)$ \\
\midrule
...
Table~\ref{tab:main16} reports the three encoder benchmarks at a labeled budget of $16$ refinement and $16$ selection examples per task; Table~\ref{tab:main8} repeats the two eight-task encoder benchmarks at half that budget and reports MergeBench at its $8{+}8$ budget.
...
\paragraph{High-interference setting: twenty-task CLIP.}
\label{sec:clip20}
The twenty-task suite of \citet{wang2024tall} (published fine-tuned ViT-B/32 encoders) is the high-interference end of the vision track: in Table~\ref{tab:main16} even the best checkpoint-only merge (TIES, $0.619$) recovers only a fifth of the gap between the zero-shot floor ($0.550$) and the expert ceiling ($0.896$), and the split-selected task-arithmetic merge lands \emph{below} the floor ($0.495$)---interference is severe enough that even the smallest coefficient in the standard grid, the selected $\lambda=0.2$, is still over-scaled, with the evaluation-split optimum lying further out at $\lambda\approx0.05$--$0.08$ (Appendix~\ref{app:clip20}). As declared in Section~\ref{sec:protocol}, normalized retention degenerates on near-zero expert--base gaps (STL10's expert beats zero-shot by $0.004$), so this benchmark reports absolute mean and worst-task top-1 accuracy.

\paragraph{Refining from the pretrained model.}
Refining \emph{from the pretrained model}---the setting with no merge to inherit, and the one where the refinement must supply all of the multi-task ability itself---is an unambiguous win for the gated, expert-anchored update. APR is the best treatment on all three encoder benchmarks, ahead of the better of the two unconstrained baselines by $0.06$ (CLIP-20), $0.11$ (GLUE-8), and $0.39$ (CLIP-8); the decoder track of Table~\ref{tab:main8}, where the margins are carried by the worst task, is discussed below. Neither unconstrained baseline is a viable cold-start method at this budget. Ordinary descent recovers between a twentieth and a fifth of the expert--base gap on the two retention benchmarks ($0.046$ on CLIP-8, $0.209$ on GLUE-8). The ungated update fares worse still: it fails to improve on the pretrained model at all on either benchmark ($-0.018$ on GLUE-8, $-0.036$ on CLIP-8), so distance scaling without the gate is not merely weaker than APR but useless from this initialization. Only the gated, distance-scaled update turns a small replay buffer into multi-task ability with no merge to build on.

\paragraph{The decoder-only track: MergeBench.}
On MergeBench (Table~\ref{tab:main8}), both labeled arms improve the pretrained Llama-3.2-3B at eight labels per task---mean accuracy $0.289\to0.333$ (GD) and $\to0.337$ (APR) against an expert ceiling of $0.548$---and the difference between them is carried almost entirely by the worst task: APR holds $0.200$ on mathematics, the task the pretrained model is weakest at, against GD's $0.150$, while the mean gap ($+0.004$) is within evaluation noise on $300$-item sets. The degenerate multilingual expert forces the absolute-accuracy convention (its negative expert--base gap would invert normalized retention).
Composition is substantially stronger than cold-start refinement on this decoder track. DARE-TIES is the best launch point, improving from $0.490$ mean/$0.350$ worst-task accuracy to $0.508/0.390$ with APR. Task arithmetic, TIES, and AdaMerging also improve in mean by $0.008$, $0.003$, and $0.001$, respectively. Breadcrumbs is the lone exception: its mean is effectively flat but nominally lower ($0.416\to0.414$), while its worst task improves sharply ($0.250\to0.313$). All five APR rows select $S=5$; except for DARE-TIES, the mean changes are within the resolution of the $300$-item generation evaluations, whereas the common worst-task repair is clearer.
...
\midrule
Pretrained & $+$GD & $0.150$ & $0.420$ & $0.270$ & $0.492$ & $0.333$ \\
           & $+$APR & $0.200$ & $0.403$ & $0.247$ & $0.498$ & $0.337$ \\
\midrule
Task arithmetic & alone & $0.613$ & $0.430$ & $0.330$ & $0.504$ & $0.469$ \\
                & $+$APR & $0.580$ & $0.467$ & $0.357$ & $0.507$ & $0.477$ \\
\midrule
Breadcrumbs & alone & $0.467$ & $0.447$ & $0.250$ & $0.502$ & $0.416$ \\
            & $+$APR & $0.413$ & $0.423$ & $0.313$ & $0.504$ & $0.414$ \\
\midrule
DARE-TIES & alone & $0.677$ & $0.440$ & $0.350$ & $0.493$ & $0.490$ \\
          & $+$APR & $0.680$ & $0.463$ & $0.390$ & $0.497$ & $0.508$ \\
\midrule
TIES & alone & $0.583$ & $0.423$ & $0.357$ & $0.492$ & $0.464$ \\
     & $+$APR & $0.567$ & $0.430$ & $0.373$ & $0.497$ & $0.467$ \\
\midrule
AdaMerging & alone & $0.680$ & $0.457$ & $0.357$ & $0.491$ & $0.496$ \\
           & $+$APR & $0.633$ & $0.467$ & $0.393$ & $0.495$ & $0.497$ \\
\bottomrule
\end{tabular}
\end{table}
