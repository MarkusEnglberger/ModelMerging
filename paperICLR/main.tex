...
Starting from any initial model---a merge produced by any existing method, or the pretrained model itself---we refine it with task gradients while using the known task experts as coordinate-wise trust anchors.
The resulting update admits two equivalent motivations that we develop in parallel. As a parameter-space analogue of attribution patching, the effect of moving the current model toward a task expert is approximated by a first-order loss attribution, and only coordinates for which the expert direction is locally loss-decreasing are used. As an expert-anchored trust-region replay refinement, the departure from ordinary replay gradient descent is that each coordinate's step is weighted by its distance to the expert, gated to keep only steps that move toward the expert, and clipped so it cannot overshoot---the same coordinates that the attribution view selects. Both perspectives lead to a single refinement algorithm, which is provably non-expansive toward the experts' box hull at any learning rate. We evaluate it on multi-head GLUE-8, the eight- and twenty-task CLIP suites, and a decoder-only MergeBench track, under a protocol in which every hyperparameter is selected on a small held-out buffer and the evaluation split is never used for tuning, at a budget of sixteen labeled examples per task or fewer. Refining directly from the pretrained model, the anchored update is the best-performing treatment on every benchmark, while unconstrained replay descent is not a viable cold-start method at this budget; from a merge initialization it improves every starting point tried and makes the outcome far less sensitive to which merge it was handed. Its advantage widens as the buffer shrinks: at eight labeled examples per task it still beats a tuned task-arithmetic merge from the pretrained model alone, in a regime where unconstrained descent overfits a quarter of its configurations while its replay loss still improves. On held-out tasks never merged or refined, the refinement populates the accuracy--retention Pareto frontier, and every checkpoint-only merge is dominated.
\end{abstract}
\section{Introduction}
\label{sec:intro}
Merging fine-tuned variants of a shared pretrained model combines their skills in weight space, without retraining, ensembling, or routing. But merges degrade under task interference, and the data-free repairs---sparsification, sign election, learned coefficients---plateau well below the experts. In many practical settings a small \emph{labeled} replay buffer per task is available (it is routinely retained for validation or continual learning); the question is how to spend such a tiny labeled budget so that it repairs the merge without giving up what made merging attractive: a single model, low cost, and bounded drift from checkpoints one already trusts.

We refine the merged model with replay gradients \emph{anchored to the task experts}: each coordinate's step is kept only where moving toward the expert locally reduces that task's replay loss (an attribution-patching sign), weighted by the coordinate's distance to the expert, and clipped so it can never overshoot---applied task-sequentially, from \emph{any} initialization, including the pretrained model itself. The update is provably non-expansive toward the experts' box hull at any learning rate (Proposition~\ref{prop:box-nonexpansive}).

Our contributions are: (i)~a replay-refinement method with two equivalent derivations---parameter-space attribution patching and an expert-anchored trust region---and a divergence-safety guarantee; (ii)~a uniform evaluation across four benchmark families (multi-head GLUE-8, the eight- and twenty-task CLIP suites, and decoder-only LLM merging in the MergeBench setting) under a strict protocol in which every hyperparameter is chosen on a small held-out selection buffer and the evaluation split is never used for tuning, with matched data budgets and sanity controls; (iii)~evidence that from the pretrained model---the setting with no merge to inherit---the anchored update is the only one of the three replay families that turns a $16$-example buffer into multi-task ability, while from a merge initialization it improves every starting point tried and makes the outcome markedly less sensitive to which merge was handed to it; (iv)~a characterization of where the anchor earns its keep: as the buffer shrinks to eight examples per task, unconstrained descent silently overfits---a quarter of its configurations lose test accuracy while their replay loss is still falling---whereas the trust region removes that failure mode entirely, and no anchored run fell below its initialization at any setting tested; and (v)~a measured accuracy--retention frontier on tasks never merged or refined, populated by the refinement together with the strongest data-fitted merge, on which every checkpoint-only merge and both unconstrained controls are dominated.

\section{Related Work and Background}
\label{sec:related}

\paragraph{Model merging and task vectors.}
Model merging combines fine-tuned variants of a shared pretrained model into a single model in weight space, avoiding ensembling overhead: model soups average compatible checkpoints \citep{wortsman2022modelsoups}, Fisher merging weights parameters by Fisher information \citep{matena2022fisher}, and task arithmetic represents fine-tuning as a \emph{task vector} $\taskvec_t=\theta_t-\thetab$ and merges as $\thetab+\sum_t\lambda_t\taskvec_t$ \citep{ilharco2022taskarithmetic}---checkpoint-only, but prone to over-counted directions and sign or magnitude conflicts across tasks.

\paragraph{Interference-aware and data-dependent merging.}
A major line of work addresses interference between task vectors: TIES-Merging trims small-magnitude updates and merges only sign-consistent components \citep{yadav2023ties}; DARE randomly drops most delta parameters and rescales the rest \citep{yu2023dare}, generalized by DELLA \citep{deep2024della}; Model Breadcrumbs removes negligible and outlier perturbations \citep{davari2023breadcrumbs}; Localize-and-Stitch stitches sparse task-relevant regions back into the pretrained model \citep{he2024localize}. Most closely related to our trust-region view, Task Arithmetic in Trust Region (TATR) uses task-loss gradients at the pretrained model to identify gradient-orthogonal parameter dimensions in which task vectors can be merged with limited interference \citep{sun2025tatr}. TATR is training-free and restricts the merge itself; our method instead uses labeled replay gradients at the evolving merged model to take sequential, expert-directed refinement steps. A complementary line uses small amounts of data or model outputs: APL scores task vectors by a data-dependent contrast \citep{kong2024apl}, RegMean solves each linear layer by regression on its inputs \citep{jin2023regmean}, and AdaMerging learns coefficients by entropy minimization \citep{yang2024adamerging}. Twin-Merging, Task Vector Bases, and FusionBench extend and standardize this space \citep{lu2024twin,zeng2025taskvectorbases,tang2024fusionbench}. The present work does not introduce another pruning rule: it asks whether a small \emph{labeled} replay buffer can repair an initial merge while staying close to the known experts, using the sign of a merge-to-expert attribution only as a local gate for refinement.
...
Because the clip follows the learning-rate scaling, per-coordinate movement is bounded by $|v_{i,r}^{(s,k)}|$ for any $\eta$: the learning rate controls how many coordinates reach the trust-region boundary, never the maximum step. Optionally, $u_i^{(s,k)}$ is RMS-normalized tensor-wise. Algorithm~\ref{alg:expert-gated-refinement} summarizes the procedure.
...
\paragraph{Settings.}
We evaluate on four benchmark families. \emph{Multi-head} GLUE-8 (CoLA, SST-2, MRPC, STS-B, QQP, MNLI, QNLI, RTE) \citep{wang2018glue} with RoBERTa-base \citep{liu2019roberta} merges the shared encoder and evaluates with each task's own trained head---an intentionally \emph{oracle-task-id} setting. A \emph{CLIP/ViT-B/32} vision track uses the standard eight-task task-vector suite \citep{radford2021clip,ilharco2022taskarithmetic} and its twenty-task extension \citep{wang2024tall}, probing a different modality and the high-interference regime (Section~\ref{sec:clip20}). A \emph{decoder-only LLM} track adopts the MergeBench setting \citep{he2025mergebench}: published full-parameter domain experts share a common pretrained checkpoint, so task vectors and merge-to-expert directions are well defined and no expert training is required on our side. We use Llama-3.2-3B with the mathematics, coding, instruction-following, and multilingual experts, scored by judge-free automatic metrics, and merge the full model including the LM head---so every task decodes through one shared output and no task identity is assumed at evaluation, the assumption the multi-head setting grants for free. Refinement uses $n\le16$ labeled replay examples per task ($n=8$ on MergeBench), class-balanced where possible and disjoint from both the selection buffer and evaluation; full details appear in Appendix~\ref{app:protocol}.


\paragraph{Baselines.}
Baselines are grouped by data regime: \emph{checkpoint-only} (task arithmetic, TIES, DARE-TIES, Breadcrumbs); \emph{data-dependent but label-free} (AdaMerging); \emph{labeled} (ordinary replay GD from the same initialization, and the ungated distance-scaled update---the two ablations that isolate the anchor and the gate); and \emph{sanity controls} (inverted and density-matched random gates). All three labeled families use the same replay examples, the same selection buffer, and the same hyperparameter search budget. Each merge baseline's own hyperparameters (coefficient, density, sparsity) are likewise selected on the selection buffer.
...
\end{equation}
which places the pretrained model at $0$ and the task expert at $1$ ($\epsilon_{\mathrm{metric}}$ guards small denominators). Mean and worst-task values are always reported together---worst-task matters because a merge can improve the average while catastrophically harming a small task. Two benchmarks depart from this convention because their retention denominators degenerate: on the twenty-task CLIP suite several experts barely improve on the zero-shot backbone, and on MergeBench the multilingual expert finishes \emph{below} the pretrained model ($0.494$ vs.\ $0.495$), so a near-zero or negative denominator makes per-task retention explode or invert. Both benchmarks therefore report absolute mean and worst-task accuracy, quoting the pretrained floor and expert ceiling for scale (Section~\ref{sec:clip20}).

\paragraph{Protocol.}
Refinement runs $S$ sweeps from the given initialization with the trust region applied after the learning rate (Eq.~\eqref{eq:clipped-update}), $\gamma=1$, $\epsilon_{\mathrm{gate}}=0$, a constant learning rate, and a fixed task order. Each family---ordinary replay gradient descent (GD), the proposed gated update (APR), and the ungated distance-scaled update---receives an equal-size search over the learning rate and the sweep count $S$. Selection never reads the evaluation split: each task contributes a labeled replay buffer used for the refinement sweeps and a \emph{disjoint} selection buffer of the same size, every hyperparameter---including each merge baseline's own coefficient and density---is chosen by minimizing loss on the selection buffer, and the evaluation split is read only at the selected cell. The main grid uses $16$ examples per task in each buffer; the budget study of Section~\ref{sec:clip20} pushes the refinement buffer to $8$ and replicates the selected cells over pinned replay draws, so that the reported variance isolates the draw, and the MergeBench track runs at $8{+}8$ throughout, generation-based evaluation being the dominant cost there. The multi-head oracle-task-id assumption is removed on both the MergeBench and CLIP tracks, whose merged models carry no task-specific parameters.

\section{Results}
\label{sec:results}
Table~\ref{tab:main16} instantiates one experiment grid identically on every benchmark, under a selection protocol that never reads the evaluation split; Section~\ref{sec:clip20} then pushes the labeled budget lower still on the benchmark where interference is worst. Per-benchmark details, diagnostics, and the full grids are given in Appendix~\ref{app:clip20}.
...
\scriptsize
\setlength{\tabcolsep}{3.5pt}
\caption{\textbf{The experiment grid under split selection at a $16{+}16$ budget}, instantiated identically on every encoder benchmark. Each task contributes $16$ labeled replay examples for the refinement sweeps and a \emph{disjoint} $16$-example selection buffer ($32$ labels per task in total). Every hyperparameter---merge coefficient, density, refinement learning rate, and the sweep count $S$---is chosen by minimizing held-out loss on the selection buffer, and the evaluation split is read \emph{only} at the selected cell, so the table reports what a practitioner obtains without ever touching test data. Entries are mean aggregate with worst-task in parentheses: normalized retention on GLUE-8 and CLIP-8, where the pretrained \emph{alone} row is the zero point, and absolute top-1 accuracy on twenty-task CLIP, whose retention denominator degenerates (Section~\ref{sec:protocol}). AdaMerging is given the matched budget---the same $16$ unlabeled inputs per task that APR sees labeled---rather than the standard transductive formulation that adapts on the evaluation split. The horizon grid is $S\in\{5,20,50\}$ on CLIP-20 and on the merge-initialized GLUE-8 rows, and $S\in\{5,20,50,100,200\}$ on CLIP-8 and the from-pretrained GLUE-8 rows; every arm receives the same grid as the arms beside it, so no treatment is searched over a wider space than its controls. $^{\dagger}$: the selected $S$ is the longest its grid offered, so the cell is a lower bound rather than a verified interior optimum---at twenty tasks and from a merge initialization the refinement is still improving at $S=200$. The decoder-only MergeBench track runs at a smaller budget and is reported separately in Table~\ref{tab:main8}.}
\label{tab:main16}
\setlength{\arraystretch}{0.95}
\begin{tabular}{llccc}
\toprule
...
\scriptsize
\setlength{\tabcolsep}{3.5pt}
\caption{\textbf{The canonical grid under split selection at an $8{+}8$ budget.} Each task contributes eight labeled replay examples and a disjoint eight-example selection buffer. Every hyperparameter is selected on the selection buffer before the evaluation split is read; the encoder tracks use $S\in\{5,20,50\}$ and MergeBench uses $S\in\{5,20,50,100\}$. Entries follow Table~\ref{tab:main16}: mean normalized retention with worst-task retention in parentheses, except MergeBench and CLIP-20, which report absolute accuracy. The decoder-only MergeBench track uses Llama-3.2-3B with mathematics, coding, instruction-following, and multilingual experts; its pretrained floor is $0.289$ and expert ceiling $0.548$. $^{\dagger}$ marks a selected horizon at the longest value offered; dashes are configurations not run.}
\label{tab:main8}
\setlength{\arraystretch}{0.95}
\begin{tabular}{llcccc}
\toprule
Initialization & Treatment & GLUE-8 & CLIP-8 & MergeBench & CLIP-20 \\
 & & \multicolumn{2}{c}{normalized retention} & \multicolumn{2}{c}{absolute accuracy} \\
\midrule
Pretrained      & alone      & $0$ & $0$ & $0.289\ (.06)$ & --- \\
                & $+$GD      & $-0.361\ ({-}3.52)$ & $-0.148\ ({-}1.04)$ & $0.333\ (.15)$ & --- \\
                & $+$ungated & $0.089^{\dagger}\ ({-}.31)$ & $-0.006\ ({-}.41)$ & --- & --- \\
                & $+$APR     & $0.223^{\dagger}\ ({-}.33)$ & $0.295^{\dagger}\ ({-}.45)$ & $\mathbf{0.337}\ (\mathbf{.20})$ & --- \\
\midrule
Task arithmetic & alone      & $0.320\ ({-}.18)$ & $0.270\ ({-}.53)$ & --- & --- \\
                & $+$GD      & $0.530\ ({+}.04)$ & $0.167^{\dagger}\ ({-}1.10)$ & --- & --- \\
                & $+$ungated & $0.529\ ({+}.03)$ & $0.241^{\dagger}\ ({-}.79)$ & --- & --- \\
                & $+$APR     & $0.498\ ({-}.18)$ & $0.462^{\dagger}\ ({-}.12)$ & --- & --- \\
\midrule
Breadcrumbs     & alone      & $0.030\ ({-}2.14)$ & $0.431\ ({+}.08)$ & --- & --- \\
                & $+$GD      & $0.512\ ({-}.00)$ & $0.479^{\dagger}\ ({+}.04)$ & --- & --- \\
                & $+$ungated & $0.536\ ({+}.05)$ & $0.391^{\dagger}\ ({-}.40)$ & --- & --- \\
                & $+$APR     & $0.528\ ({-}.06)$ & $0.339^{\dagger}\ ({-}.46)$ & --- & --- \\
\midrule
DARE-TIES       & alone      & $0.301\ ({-}.34)$ & $0.364\ ({-}.32)$ & --- & --- \\
                & $+$GD      & $0.484\ ({-}.08)$ & $-1.273^{\dagger}\ ({-}4.87)$ & --- & --- \\
                & $+$ungated & $0.534\ ({+}.08)$ & $-0.578\ ({-}3.11)$ & --- & --- \\
                & $+$APR     & $\mathbf{0.576}\ ({+}.19)$ & $0.498^{\dagger}\ ({-}.31)$ & --- & --- \\
\midrule
TIES            & alone      & $0.208\ ({-}.22)$ & $0.525\ ({+}.21)$ & --- & --- \\
                & $+$GD      & $0.442\ ({-}.10)$ & $\mathbf{0.585}^{\dagger}\ ({+}.18)$ & --- & --- \\
                & $+$ungated & $0.442\ ({-}.07)$ & $0.318^{\dagger}\ ({-}1.57)$ & --- & --- \\
                & $+$APR     & $0.489^{\dagger}\ ({+}.01)$ & $0.566^{\dagger}\ ({-}.08)$ & --- & --- \\
\midrule
AdaMerging      & alone      & $0.028\ ({-}3.01)$ & $0.463\ ({-}.04)$ & --- & --- \\
                & $+$GD      & $0.016\ ({-}2.95)$ & $0.502^{\dagger}\ ({-}.06)$ & --- & --- \\
                & $+$ungated & $0.466\ ({-}.10)$ & $0.285^{\dagger}\ ({-}1.41)$ & --- & --- \\
                & $+$APR     & $0.543\ ({+}.02)$ & $0.554^{\dagger}\ ({+}.15)$ & --- & --- \\
\midrule
RegMean         & alone      & --- & --- & --- & --- \\
                & $+$GD      & --- & --- & --- & --- \\
                & $+$ungated & --- & --- & --- & --- \\
...
\paragraph{The grid under split selection.}
Table~\ref{tab:main16} reports the three encoder benchmarks at a labeled budget of $16$ refinement and $16$ selection examples per task; Table~\ref{tab:main8} repeats the two eight-task encoder benchmarks at half that budget and adds the decoder-only MergeBench track, which runs at $8{+}8$ throughout. In both tables no evaluation data is used for tuning. The result splits cleanly in two, along the axis of whether a usable merge already exists.

Refining \emph{from the pretrained model}---the setting with no merge to inherit, and the one where the refinement must supply all of the multi-task ability itself---is an unambiguous win for the gated, expert-anchored update. APR is the best treatment on all three encoder benchmarks, ahead of the better of the two unconstrained baselines by $0.06$ (CLIP-20), $0.11$ (GLUE-8), and $0.39$ (CLIP-8); the decoder track of Table~\ref{tab:main8}, where the margins are carried by the worst task, is discussed below. Neither unconstrained baseline is a viable cold-start method at this budget. Ordinary descent recovers between a twentieth and a fifth of the expert--base gap on the two retention benchmarks ($0.046$ on CLIP-8, $0.209$ on GLUE-8), and it buys even that by running to the end of its horizon grid. The ungated update fares worse still: given the same long horizons it fails to improve on the pretrained model at all on either benchmark ($-0.018$ on GLUE-8, $-0.036$ on CLIP-8), so distance scaling without the gate is not merely weaker than APR but useless from this initialization. Only the gated, distance-scaled update turns a small replay buffer into multi-task ability with no merge to build on.

\paragraph{The decoder track.}
On MergeBench (Table~\ref{tab:main8}), both labeled arms improve the pretrained Llama-3.2-3B at eight labels per task---mean accuracy $0.289\to0.333$ (GD) and $\to0.337$ (APR) against an expert ceiling of $0.548$---and the difference between them is carried almost entirely by the worst task: APR holds $0.200$ on mathematics, the task the pretrained model is weakest at, against GD's $0.150$, while the mean gap ($+0.004$) is within evaluation noise on $300$-item sets. The degenerate multilingual expert is instructive twice over. It forces the absolute-accuracy convention (its negative expert--base gap would invert normalized retention), and the refinement diagnoses it independently: the attribution gate passes only $16\%$ of coordinates for the multilingual task against $38$--$46\%$ for the other three---the sign test finds little locally loss-decreasing structure in an expert that has nothing to teach, exactly as its evaluation scores indicate. The track is single-seed, and the ungated arm and composition rows have not been run, so the decoder evidence is thinner than the encoder tracks'.

%\paragraph{The anchored update is insensitive to how wide the search is.}
%Split selection is only as good as the grid it searches, and enlarging that grid can move the selected cell in either direction---a hazard for any method whose selection surface is sharp, and one a practitioner cannot diagnose without the test labels they are trying not to use. We therefore re-ran the from-pretrained rows on a strictly larger search space: learning rates extended by an octave at each end and $S\in\{5,20,50,100,200\}$. APR's selection is \emph{exactly} unchanged on both benchmarks tested---$\eta=8$, $S=50$ on GLUE-8 ($0.315$) and $\eta=32$, $S=50$ on CLIP-8 ($0.432$)---and both are now interior in learning rate and horizon, so APR's $^{\dagger}$ cells there are optima rather than lower bounds. The unconstrained arms move in both directions: the ungated update on GLUE-8 finds a much better cell ($-0.431\to-0.018$), while ordinary descent on CLIP-8 finds a worse one ($0.096\to0.046$) by selecting the longest horizon offered. The anchored method returns the same answer however carefully the grid was designed; the descent-family answers are contingent on it, and the direction of that contingency is not predictable in advance.

%One axis of the grid is absent from Table~\ref{tab:main16} and we tested it rather than assume it away. Every cell there runs APR at a constant rate with a fixed task order, whereas an annealed package---cosine decay to $5\%$ of the peak, task order re-randomized each sweep---is the natural alternative. We therefore rebuilt the entire CLIP-8 column with the two packages as matched arms, over all six initializations and three replay draws, with identical learning-rate and horizon grids. \textbf{The schedule does not matter at this budget.} Over the eighteen paired comparisons the annealed package is worth $+0.002\pm0.032$ and wins nine of eighteen, a $95\%$ interval of $[-0.013,+0.017]$ that straddles zero. Only refinement from the pretrained model shows a consistent gain, and a small one ($+0.021\pm0.009$, positive on all three draws); the largest per-initialization effect, $+0.042$ from Breadcrumbs, carries a $0.040$ standard deviation across draws and is not separable from noise. The constant, fixed-order default used throughout the paper is therefore not a missed opportunity.

%The same experiment revealed that the \emph{horizon} grid was too short: extending $S$ from $50$ to $100$ lifts constant-schedule APR from TIES by $0.061$ at the same draw ($0.548\to0.609$), and the selected horizon still sits at the grid maximum in sixteen of eighteen cells. The $^{\dagger}$ marks in Table~\ref{tab:main16} are consequently not cosmetic, and from a merge initialization---unlike from the pretrained model, which finds interior optima---APR at $n=16$ wants a longer refinement than that table offers. Because these runs searched a strictly larger space than the baselines beside them, we report them here rather than substituting them into the table, where they would give APR a grid its controls did not receive.

%Once a good merge \emph{is} available the margins narrow. On GLUE-8 and CLIP-20 the gate still wins outright: APR is the better treatment in all five composition rows on each (best $0.608$ and $0.676$ respectively). On CLIP-8 the row-by-row comparison favors GD (APR better in two of five), yet the best composition cell is still APR's ($0.639$ from AdaMerging, against $0.601$)---the gate loses the typical row and wins the top one, because it composes best with the strongest initialization.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figs/n16_displacement.pdf}
\caption{Score versus parameter displacement for every split-selected refinement cell of Table~\ref{tab:main16}; log $x$. Ringed markers refine from the pretrained model, so their displacement is the distance from $\theta_0$ itself; small markers refine from a merge initialization and show the distance from that initialization; the gray square is the best task-arithmetic merge at its distance from $\theta_0$ (task arithmetic is linear in $\lambda$, so this follows from one measured norm). Two regularities hold on every benchmark. Refinement is a small-displacement operation relative to merging---on GLUE-8 the tuned merge sits ${\approx}108$ from $\theta_0$ for retention $0.320$, while APR from $\theta_0$ reaches $0.315$ at displacement $2.5$, matching it at ${\approx}2\%$ of the parameter drift. And movement separates from harm by arm: split selection keeps ordinary GD close to its start, where from $\theta_0$ it achieves little, whereas the ungated update both travels far and can end catastrophically (GLUE-8, $-0.43$ at displacement $5.3$); APR is the arm that converts displacement into score. Parameter distance is a within-family diagnostic rather than a capability measure: it is comparable along the task-arithmetic family, whose merges differ only by a scalar, but not across merge families with different geometry (RegMean, in particular, sits over $450$ from $\theta_0$ in directions that are largely inference-invisible; Appendix~\ref{app:clip20}). Whether the small displacements on this figure correspond to preserved pretrained capability is therefore measured rather than inferred, in Table~\ref{tab:drift}.}
\label{fig:n16disp}
\end{figure}

%The sharper distinction in this tier is not the mean but the \emph{spread across initializations}, which is what a practitioner without test labels is exposed to: having picked a merge for reasons unrelated to refinement, how much does that choice cost? APR is the less sensitive arm on every benchmark ($0.065$ vs.\ $0.115$ on GLUE-8, $0.135$ vs.\ $0.176$ on CLIP-8, and---most strikingly---$0.015$ vs.\ $0.103$ on CLIP-20, where APR lands within a band of $0.015$ from \emph{every} initialization while GD ranges from $0.555$ to $0.658$). Where the anchor binds, it makes the outcome a property of the method rather than of the merge it was handed.

%We read the CLIP-8 row-by-row result as a statement about where the anchor binds rather than as a refutation. A strong merge already lies inside the region where unconstrained descent is well behaved, so constraining the step costs more than it saves; the pretrained model does not, which is why the same constraint is decisive there. And mean retention does not price tail risk: Section~\ref{sec:clip20} shows descent deteriorating on a quarter of its small-buffer cells while its replay loss still improves, a hazard no best-per-cell number can show. The defensible claim is therefore narrower than ``APR is best'': the gated anchor is what makes replay refinement work \emph{without} a good merge, and what makes its outcome predictable \emph{with} one.

\paragraph{Does small displacement preserve pretrained capability?}
Figure~\ref{fig:n16disp} shows the refinement reaching merge-level scores at a small fraction of the merge's displacement, but parameter distance is a geometric quantity, not a capability measure. We therefore probe the base model's \emph{own} objective directly: the masked-language-model loss of RoBERTa's pretrained LM head---left untouched, so the only thing varying is the trunk---evaluated on WikiText-2 validation text that no part of the experiment ever sees. The probed states are exactly the split-selected from-pretrained cells of Table~\ref{tab:main16}, re-derived from the same replay buffer, with the pretrained model and the task-arithmetic merge as references (Table~\ref{tab:drift}).

Three things follow. First, the proximity story survives a functional test: APR matches the merge's GLUE-8 retention ($0.315$ vs.\ $0.320$) while inflating the pretraining loss by $1.08$ against the merge's $2.51$---it buys the same multitask ability for $43\%$ of the damage to what the model already knew. Second, and less expected, displacement does not order the drift even \emph{within} this tier: APR travels $5.5\times$ farther from $\thetab$ than ordinary descent, yet ends closer to the base model in function ($+1.08$ vs.\ $+1.62$), so APR dominates GD on both axes at once. What the anchor buys is not a shorter path but a better-directed one. Third, the gate is doing the protecting here rather than the anchor: the ungated update, identical except for the gate and run at the same rate, does not merely score badly ($-0.431$) but destroys the pretraining objective outright ($16.10$ against a base of $2.51$). The probe is a single seed on a single corpus for one benchmark, and it measures the pretraining objective rather than downstream ability; the multi-task version of this question---retention on held-out \emph{tasks}---is the subject of Section~\ref{sec:clip20}.

\begin{table}[t]
\centering
\small
\caption{Pretraining-capability drift on multi-head GLUE-8 at the $16{+}16$ budget. Each refinement row is the split-selected from-pretrained cell of Table~\ref{tab:main16}, re-derived from the same replay buffer; ``MLM loss'' is RoBERTa's own pretrained masked-LM head evaluated over the refined trunk on WikiText-2 validation, text no experiment touches, so the increase over $\thetab$ is pure trunk drift measured in the base model's own objective. Single seed. Lower MLM loss and higher retention are both better; APR is the only arm that improves on ordinary descent in both.}
\label{tab:drift}
\begin{tabular}{lccc}
\toprule
State (from $\thetab$) & Dist.\ $\thetab$ & GLUE-8 NormRet & MLM loss ($\Delta$) \\
\midrule
Pretrained $\thetab$                  & $0.00$   & $0$        & $2.51\ (\phantom{+}0.00)$ \\
Task-arithmetic merge ($\lambda{=}0.3$) & $107.7$ & $0.320$    & $5.02\ (+2.51)$ \\
\midrule
$+$GD ($\eta{=}10^{-3}$, $S{=}50$)    & $0.45$   & $0.209$    & $4.13\ (+1.62)$ \\
$+$ungated ($\eta{=}8$, $S{=}50$)     & $5.32$   & $-0.431$   & $16.10\ (+13.59)$ \\
$+$APR ($\eta{=}8$, $S{=}50$)         & $2.47$   & $\mathbf{0.315}$ & $\mathbf{3.59}\ (+1.08)$ \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Twenty-task CLIP (ViT-B/32)}
\label{sec:clip20}
The twenty-task suite of \citet{wang2024tall} (published fine-tuned ViT-B/32 encoders) is the high-interference end of the vision track, and the benchmark on which Table~\ref{tab:main16} shows the widest gap between the anchored and unconstrained arms. The scale forces two protocol changes. The conventional $\lambda_i=0.3$ is catastrophic here---the task-arithmetic merge scores $0.364$, \emph{below} the $0.550$ zero-shot floor, and the tuned optimum falls to $\lambda\approx0.05$--$0.08$---so merge coefficients must be re-tuned per task count rather than inherited. And, as declared in Section~\ref{sec:protocol}, normalized retention degenerates on near-zero expert--base gaps (STL10's expert beats zero-shot by $0.004$), so this section reports absolute mean and worst-task top-1 accuracy (zero-shot floor $0.550$, expert ceiling $0.896$).

\paragraph{Down to eight labels per task.}
We push the budget below the grid's $16$ examples per task with a full $n\in\{8,16,32\}$ grid, each cell tuned over the learning rate and horizon $S\in\{5,30,60\}$ (Table~\ref{tab:clip20nbudget}, Appendix~\ref{app:clip20}). Three regularities. First, degradation with budget is graceful in the mean but concentrated in the \emph{worst task}: from AdaMerging, mean accuracy loses $0.015$ across the $4\times$ budget reduction ($0.698\to0.683$) while worst-task accuracy loses $0.034$. Second, the refinement's value survives at the smallest budget: with just \emph{eight} labeled examples per task, refining the pretrained model reaches $0.632$---above the tuned task-arithmetic merge ($0.604$), which uses no labels but requires all twenty expert checkpoints and a tuned coefficient---and that holds over five replay draws ($0.626\pm0.005$, above the merge on every draw). Third, from the pretrained model the anchored update's margin over plain descent is large and essentially budget-independent (${\approx}{+}0.05$--$0.06$ mean at every $n$; at $n=8$, $0.626\pm0.005$ against $0.572\pm0.003$ over five draws). Geometry, not data volume, sets that gap: at the tuned merge every expert sits ${\approx}2.4$ away while the best refined solutions lie $0.14$--$0.23$ from a strong initialization, so the distance-scaled step matters most exactly where the destination genuinely is far---from the pretrained model (Appendix~\ref{app:clip20}).

\paragraph{The anchored step, not the gate, is load-bearing.}
APR and the ungated anchored update agree within $\pm0.01$ across every initialization tested on this suite, while the inverted gate collapses to $0.100$: the attribution \emph{sign} carries signal, but thresholding at zero adds nothing over no gate. A held-out audit explains why, and closes the route of thresholding harder: first-order attributions transfer to a disjoint buffer only weakly---$\approx10\%$ of the predicted loss decrease at $n=8$, rising to only $\approx32\%$ even at a four-times larger buffer---and uniformly across magnitude buckets, so a top-$k$ rule would discard most of the transferable signal; at $n=8$ the ordering across buckets even reverses, the largest gradient entries being the most sample-specific. The gradient's transferable contribution is step magnitude, not coordinate choice (Appendix~\ref{app:clip20}).

\paragraph{Safety at small budgets.}
Safety is the method's clearest practical advantage, and it is sharpest exactly where the buffer is small. Best-per-cell tables cannot detect overfitting, since selected configurations exclude it by construction, so we audit \emph{every} $(\eta,S,n)$ cell for the signature of replay loss improving while test accuracy falls. For unconstrained descent it appears as soon as the buffer is small: $9$ of $36$ small-$n$ cells deteriorate, with textbook cases at $n=16$ (replay loss $0.083\to0.063$ while test accuracy falls $0.636\to0.288$), plus two outright instabilities. For the anchored update the signature never appears---across ${\approx}48$ cells the largest decline is $0.003$---\emph{even though} APR also reaches near-interpolation of its buffer (replay loss $0.007$--$0.011$ at $n=8$). Once every coordinate is capped at $\gamma|v_{i,r}|$, additional sweeps have nowhere damaging to go, whereas descent keeps moving after it has fit the buffer; a corollary is that descent's safe learning-rate window narrows as sweeps accumulate, so a rate tuned at $S=5$ cannot be reused at $S=60$, while APR's optimal rate transfers unchanged across horizons (Appendix~\ref{app:clip20}).

\paragraph{Held-out-task retention: preserved capability, measured.}
Table~\ref{tab:drift} probed the pretraining objective; here we measure retained \emph{ability}. We split the suite into the $14$ specialized training tasks and the $6$ natural-image tasks on which the base model's zero-shot accuracy is highest (mean $0.802$, so there is something to lose): every merge and every refinement step sees only the $14$ training tasks and their $16$-example buffers, and the held-out tasks are evaluated purely zero-shot through frozen text-derived heads, so nothing task-specific leaks. This is a frontier measurement, not a selected cell---each arm is swept over its learning rate and the whole arc is reported---so it is compatible with the split-selection protocol without a selection buffer: nothing is being selected. Table~\ref{tab:clip20heldout} and Figure~\ref{fig:clip20heldout} report the outcome. \textbf{Every checkpoint-only merge is dominated}: task arithmetic at all four coefficients, TIES at both, DARE-TIES, and Breadcrumbs each have a refinement point above them on both axes, and AdaMerging is dominated by APR launched from it ($0.648$ train at $-0.018$ against $0.614$ at $-0.026$). APR holds three of the frontier's four non-trivial points; the extreme-accuracy end belongs to RegMean ($0.692$ at $-0.041$), the one merge family the refinement does not dominate here. Both unconstrained controls are dominated by a single APR point: at the \emph{same} learning rate, the ungated update gives up $0.051$ training accuracy \emph{and} $0.044$ held-out accuracy to the gated one ($0.500/{-}0.050$ vs.\ $0.551/{-}0.006$)---the gate protects the shared features that zero-shot transfer depends on, the same conclusion the drift probe reached in the base model's own objective---and ordinary descent lands at $0.491/{-}0.016$, worse than APR at $\eta=2$ on both axes despite moving a third as far. Two cautions transfer from the geometry analysis: parameter distance fails as a drift proxy across merge families (RegMean sits $227$ from $\theta_0$ yet loses only $0.041$ held-out, while task arithmetic at distance $2.2$ loses $0.208$), and tuning the merge coefficient on the merged tasks alone is actively harmful ($\lambda{=}0.08\to0.2$ loses on both axes). The study is single-seed at this budget.
...
\begin{tabular}{lccc}
\toprule
Model (built from 14 train tasks) & Dist.\ $\theta_0$ & Train-14 & Held-out 6 (drop) \\
\midrule
Base model $\theta_0$$^\ast$          & $0.00$ & $0.443$ & $0.802\ (\phantom{-}0.000)$ \\
GD from base, $\eta{=}5\mathrm{e}{-5}$ & $0.09$ & $0.491$ & $0.786\ (-0.016)$ \\
APR from base, $\eta{=}2$$^\ast$      & $0.31$ & $0.551$ & $0.796\ (-0.006)$ \\
APR from base, $\eta{=}4$$^\ast$      & $0.42$ & $0.564$ & $0.790\ (-0.012)$ \\
APR from base, $\eta{=}8$             & $0.59$ & $0.592$ & $0.779\ (-0.023)$ \\
Ungated from base, $\eta{=}2$         & $0.65$ & $0.500$ & $0.752\ (-0.050)$ \\
Task arith.\ ($\lambda{=}0.08$)       & $0.87$ & $0.536$ & $0.781\ (-0.021)$ \\
APR from task arith., $\eta{=}4$      & $1.05$ & $0.610$ & $0.772\ (-0.029)$ \\
Breadcrumbs                           & $1.28$ & $0.536$ & $0.792\ (-0.010)$ \\
APR from AdaMerging, $\eta{=}4$$^\ast$ & $1.43$ & $0.648$ & $0.784\ (-0.018)$ \\
AdaMerging-layer                      & $1.69$ & $0.614$ & $0.776\ (-0.026)$ \\
Task arith.\ ($\lambda{=}0.2$)        & $2.18$ & $0.510$ & $0.594\ (-0.208)$ \\
TIES                                  & $2.90$ & $0.578$ & $0.770\ (-0.032)$ \\
RegMean$^\ast$                        & $227$  & $\mathbf{0.692}$ & $0.761\ (-0.041)$ \\
\bottomrule
\end{tabular}
\captionof{table}{Held-out-task retention on the twenty-task suite at the $16$-example budget: merge/refine on $14$ specialized tasks, evaluate the $6$ strongest natural-image tasks zero-shot (base mean $0.802$); single seed, refinements at $S{=}30$ cosine. Full learning-rate arcs are reported---nothing is selected on the evaluation split. $^\ast$ marks the accuracy/retention Pareto frontier; non-frontier rows abridged.}
\label{tab:clip20heldout}
\end{minipage}\hfill
\begin{minipage}[t]{0.37\textwidth}
\centering
\includegraphics[width=\linewidth]{figs/heldout_frontier.pdf}
\captionof{figure}{The accuracy--retention trade-off of Table~\ref{tab:clip20heldout}: models built from the $14$ training tasks, evaluated on them ($x$) and zero-shot on the $6$ held-out tasks ($y$; dotted line: the base model). The frontier is populated by the anchored refinement and RegMean; every checkpoint-only merge (gray squares) and both unconstrained controls (triangles) are dominated.}
...
% at n=16 and restored above (tab:clip20heldout / fig:clip20heldout now carry
...
\paragraph{Summary.}
Five claims survive the campaign at $n\le16$: (i)~the refinement improves every initialization it is given---five merges and the pretrained model---with gains shrinking but never vanishing as the initialization improves; (ii)~the anchored, distance-scaled, clipped step is the load-bearing ingredient, the gate contributing worst-task repair, small-budget stability, and held-out retention rather than mean accuracy; (iii)~no anchored run fell below its initialization or the zero-shot floor at any rate, horizon, or budget tested, and none showed the overfitting signature, while unconstrained descent deteriorates on a quarter of its small-buffer configurations with its replay loss still improving; (iv)~eight labels per task suffice to beat the tuned task-arithmetic merge starting from the pretrained model alone; and (v)~on held-out tasks never merged or refined, the refinement populates the accuracy--retention frontier and every checkpoint-only merge is dominated (Figure~\ref{fig:clip20heldout}).

\section{Discussion}
\label{sec:contribution}
This work develops an attribution-patching perspective on replay-guided merging: the sign of $g_{i,r}v_{i,r}$ tells, per coordinate, whether moving toward expert $i$ locally reduces replay loss; the same condition is a trust-region acceptance rule, and both views motivate one procedure---mask, distance-scale, clip, apply task-sequentially. Evaluated under a protocol that never reads the evaluation split, at $16$ labeled examples per task or fewer, the refinement is a \emph{complement} to merging: it improves every initialization tried, and its outcome is markedly less sensitive than unconstrained descent's to which merge it was handed. Its decisive setting is the one with no merge to inherit---from the pretrained model the gated, expert-anchored update is the best treatment on every benchmark reported, while neither unconstrained baseline is viable at this budget---and its advantage grows as the buffer shrinks, where descent overfits while its replay loss still improves and the trust region removes that failure mode outright. Refining from the pretrained model also turns out to be cheap in what it costs the pretrained model: it reaches merge-level multitask retention while inflating the base objective's loss by well under half of what the merge does, and it does so while moving \emph{farther} in parameter space than ordinary descent, which drifts more---so the anchor buys a better-directed path rather than a shorter one. The same conclusion holds one level up, on retained \emph{ability} rather than the pretraining objective: on tasks never merged or refined, the refinement traces the accuracy--retention frontier, with the gate---not just the anchor---supplying the retention. The result is a lightweight bridge between checkpoint-only task arithmetic and full multitask replay fine-tuning, usable at a labeled budget of a few dozen examples per task.
...
% Cited only by the retired matched-budget comparison; restore alongside it.
...
Four gaps remain. The budget grid of Table~\ref{tab:clip20nbudget} is single-seed, with five-seed replication only at its from-pretrained $n=8$ cell, and the cells of Table~\ref{tab:main16} are single-draw throughout, as is the held-out retention study of Table~\ref{tab:clip20heldout} and the drift probe of Table~\ref{tab:drift}. The predicted functional-anchor fix for the RegMean initialization is untested. On MergeBench, only the from-pretrained APR and GD cells were run---the ungated arm and the composition rows are absent---so the decoder track supports the cold-start comparison but not the gate-versus-anchor decomposition or the composition claims, which rest on the encoder tracks. A further limitation is one of scope rather than execution: the results reported here are confined to budgets of at most $32$ labeled examples per task, so we make no claim about how the ordering between the anchored and unconstrained families behaves when labeled data is plentiful.
