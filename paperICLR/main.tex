...
\section{Results}
\label{sec:results}
Every benchmark receives the same experiment grid---each initialization alone and refined, the unconstrained baselines from the pretrained model and from task arithmetic, and the gate controls---collected in one format in Table~\ref{tab:main}; dashes mark experiments not yet run, and the grid will be populated for every benchmark. The subsections take one benchmark each and add its benchmark-specific studies (replay budget, refinement horizon, held-out retention). Cross-benchmark analyses are consolidated in Appendix~\ref{app:mergebaselines}.
