...
\section{Results}
\label{sec:results}
Every benchmark receives the same experiment grid---each merge initialization (and the pretrained model) alone and under each refinement treatment, plus the gate controls---collected in one format in Table~\ref{tab:main}; dashes mark experiments not yet run, and the grid will be populated for every benchmark. The subsections take one benchmark each and add its benchmark-specific studies (replay budget, refinement horizon, held-out retention). Cross-benchmark analyses are consolidated in Appendix~\ref{app:mergebaselines}.

\begin{table}[t]
\centering
\scriptsize
\setlength{\tabcolsep}{3.5pt}
\caption{The canonical experiment grid, instantiated identically on every benchmark: each initialization \emph{alone} and under the three refinement treatments, plus the gate controls. Cells are mean aggregate with worst-task in parentheses---normalized retention, except twenty-task CLIP (absolute top-1 accuracy). The pretrained row \emph{alone} is the retention zero point (CLIP-20: the $0.550$ zero-shot floor); no initialization is privileged---task arithmetic is simply the one whose treatment set is complete so far. $\pm$ marks five-seed cells; unmarked cells are single-seed. Dashes are experiments not yet run; the grid will be populated for every benchmark.}
\label{tab:main}
...
DARE-TIES       & alone      & $0.301\ ({-}.34)$ & $0.364\ ({-}.32)$ & $0.391\ ({-}.42)$ & $0.617\ (.15)$ \\
                & $+$GD      & --- & --- & --- & --- \\
                & $+$ungated & --- & --- & --- & $0.697\ (.28)$ \\
                & $+$APR     & $0.620{\pm}.021\ ({+}.18)$ & $0.626\ (.19)$ & $0.687\ ({+}.11)$ & $0.695\ (.25)$ \\
\midrule
...
AdaMerging      & alone      & $0.173\ (.00)$ & $0.610\ ({+}.22)$ & --- & $0.665\ (.10)$ \\
                & $+$GD      & --- & --- & --- & --- \\
                & $+$ungated & --- & --- & --- & $0.701\ (.14)$ \\
                & $+$APR     & $0.583\ (.05)$ & $0.699{\pm}.044\ ({+}.31)$ & --- & $0.707\ (.19)$ \\
\midrule
RegMean         & alone      & $0.478\ ({+}.18)$ & $0.733\ ({+}.44)$ & --- & $0.723\ (.23)$ \\
                & $+$GD      & --- & --- & --- & --- \\
                & $+$ungated & --- & --- & --- & $0.756\ (.52)$ \\
                & $+$APR     & $0.103\ ({-}.06)$ & $0.762\ (.44)$ & --- & $0.749\ (.49)$ \\
\midrule
