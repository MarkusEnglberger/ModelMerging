...
Merging all eight experts is far more interference-prone (merge $0.320$, MRPC below pretrained). Both anchored updates clearly outperform ordinary replay GD, especially on worst-task retention (best mean $0.637$ ungated vs.\ $0.574$ GD; Table~\ref{tab:glue8}, Appendix~\ref{app:glue}); the gate is approximately neutral on mean retention but widens the safe learning-rate range (stable through $\eta=16$ vs.\ ungated divergence by $\eta\gtrsim8$). Reported rates are interior to the stable range (Appendix~\ref{app:glue}).
