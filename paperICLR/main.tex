...
% ---------------------------------------------------------------------------
\paragraph{Settings.}
We evaluate on three benchmark families. \emph{Multi-head} GLUE-8 (CoLA, SST-2, MRPC, STS-B, QQP, MNLI, QNLI, RTE) \citep{wang2018glue} with RoBERTa-base \citep{liu2019roberta} merges the shared encoder and evaluates with each task's own trained head---an intentionally \emph{oracle-task-id} setting. A \emph{CLIP/ViT-B/32} vision track uses the standard eight-task task-vector suite \citep{radford2021clip,ilharco2022taskarithmetic} and its twenty-task extension \citep{wang2024tall}, probing a different modality and---since more task vectors then compete for the same encoder weights---a substantially higher-interference regime (Section~\ref{sec:clip20}). Refinement uses $n\le16$ labeled replay examples per task, disjoint from both the selection buffer and the evaluation split; Appendix~\ref{app:protocol} specifies the exact sampling rule.


\paragraph{Baselines.}
Under split selection every method, in every tier, chooses its hyperparameters on the same labeled selection buffer, so the tiers are defined by what data \emph{constructs} the candidates being chosen among, mirroring Section~\ref{sec:related}: \emph{checkpoint-only construction} (task arithmetic, TIES, DARE-TIES, Breadcrumbs, DOGE/APGD), whose candidates are built from the checkpoints alone, so the labels influence nothing beyond the choice among a handful of them; \emph{unlabeled construction} (AdaMerging, RegMean), whose candidates are fitted on the same $n$ unlabeled inputs per task that APR sees labeled; and \emph{labeled construction}, the tier APR itself occupies, in which the replay buffer's labels enter the construction of the model. Strictly, then, only the pretrained model is data-free; this matches the field's practice rather than departing from it---``training-free'' merges are conventionally reported at coefficients tuned on held-out validation data \citep{ilharco2022taskarithmetic,yadav2023ties}, and MergeBench gives every method a validation-tuning budget \citep{he2025mergebench}---but here the selection cost is explicit: capped at the $n$-example buffer, identical across tiers, and counted in the stated $n{+}n$ label budget rather than drawn from an uncounted validation split. The labeled tier contains two kinds of comparison. The first are APR's ablations---ordinary replay GD from the same initialization (no anchor, no gate) and the ungated distance-scaled update (anchor without gate)---which attribute its behavior to its ingredients; all three arms consume identical data (the same replay examples, the same selection buffer, the same hyperparameter search budget) and appear in every grid. Replay GD is simultaneously the rehearsal baseline of the continual-learning literature---fine-tuning on the stored buffer with nothing tying the model to the experts---with optimizer and schedule matched to APR's so that differences are attributable to the anchor and the gate rather than to optimizer choices. The second kind are alternative ways to spend the same labels---per-task coefficient tuning, loss-weighted merging, Fisher merging with a replay-estimated Fisher, Localize-and-Stitch with masks learned on the buffer, and head-only recalibration (Appendix~\ref{app:protocol})---re-run under the same split-selection protocol at the $16{+}16$ budget and reported in the labeled-alternatives block of Table~\ref{tab:main16}. No arm fine-tunes the heads: the CLIP suites keep their frozen zero-shot heads and multi-head GLUE its per-task trained heads, following the benchmarks' convention; head-only recalibration is the deliberate exception that spends the labels on the head alone. Sanity controls (the inverted and density-matched random gates) complete the roster. Each merge baseline's own hyperparameters (coefficient, density, sparsity, or global APGD scale) are likewise selected on the selection buffer.
...
\paragraph{Protocol.}
Refinement runs $S$ sweeps from the given initialization at a constant learning rate, with the expert-distance cap applied per-update (Eq.~\eqref{eq:clipped-update}).
Each family---ordinary replay gradient descent (GD), the proposed expert-anchored gradient descent (APR), and the ungated distance-scaled update---receives an equal-size search over the learning rate and the sweep count $S$. Each task contributes a labeled replay buffer used for the refinement sweeps and a \emph{disjoint} selection buffer of the same size; every hyperparameter---including each merge baseline's own coefficient and density---is chosen by minimizing loss on the selection buffer. We use $8$ and $16$ examples per task in each buffer.
