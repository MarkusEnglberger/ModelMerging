...
\section{Results}
\label{sec:results}
Every benchmark receives the same experiment set---the three-family comparison from the tuned task-arithmetic merge, the two gate controls, composition with the strongest available merge, and refinement from the pretrained model---collected in one format in Table~\ref{tab:main}; dashes mark experiments not yet run, and the design will be populated for every benchmark. The subsections take one benchmark each and add its benchmark-specific studies (replay budget, refinement horizon, held-out retention). Cross-benchmark analyses are consolidated in Appendix~\ref{app:mergebaselines}.

\begin{table}[t]
\centering
\scriptsize
\setlength{\tabcolsep}{3.5pt}
\caption{The canonical experiment set, instantiated identically on every benchmark: mean aggregate with worst-task in parentheses---normalized retention, except twenty-task CLIP (absolute top-1 accuracy, zero-shot floor $0.550$). $\pm$ marks cells replicated over five replay-buffer seeds; unmarked cells are single-seed. ``APR $\leftarrow$ $X$'' rows compose the refinement with checkpoint-only and label-free merges; the initializations' own scores appear in Tables~\ref{tab:mergebaselines} and~\ref{tab:clip20matrix} (Appendix). Dashes are experiments not yet run; the design will be populated for every benchmark.}
\label{tab:main}
\begin{tabular}{lcccc}
\toprule
 & GLUE-8 & CLIP-8 & GLUE-8 t2t & CLIP-20 \\
 & \multicolumn{3}{c}{normalized retention} & top-1 acc. \\
\midrule
Task-arithmetic merge (init) & $0.320$ & $0.270$ & $0.383$ & $0.604\ (0.12)$ \\
\midrule
Ordinary replay GD           & $0.483{\pm}.017\ ({-}.05)$ & $0.460{\pm}.002\ ({-}.20)$ & $0.414\ (.00)$ & --- \\
Ungated anchored             & $0.593{\pm}.040\ ({+}.09)$ & $0.547{\pm}.007\ ({-}.07)$ & $0.594\ ({+}.03)$ & $0.670\ (.20)$ \\
APR (gated)                  & $0.542{\pm}.062\ ({-}.17)$ & $0.605{\pm}.008\ ({+}.15)$ & $0.537\ ({+}.02)$ & $0.680\ (.22)$ \\
\midrule
Random-gate control          & $0.568{\pm}.031\ ({+}.03)$ & $0.498{\pm}.117\ ({-}.56)$ & --- & --- \\
Inverted-gate control        & --- & $-1.682$ & $-6.185$ & $0.100$ \\
\midrule
APR $\leftarrow$ Breadcrumbs & $0.523\ (.10)$ & $0.606\ (.21)$ & $0.468\ ({+}.06)$ & $0.678\ (.18)$ \\
APR $\leftarrow$ DARE-TIES   & $0.620{\pm}.021\ ({+}.18)$ & $0.626\ (.19)$ & $0.687\ ({+}.11)$ & $0.695\ (.25)$ \\
APR $\leftarrow$ TIES        & $0.571\ (.17)$ & $0.635{\pm}.061\ ({+}.15)$ & $0.619\ ({+}.17)$ & $0.697\ (.26)$ \\
APR $\leftarrow$ AdaMerging  & $0.583\ (.05)$ & $0.699{\pm}.044\ ({+}.31)$ & --- & $0.707\ (.19)$ \\
APR $\leftarrow$ RegMean     & $0.103\ ({-}.06)$ & $0.762\ (.44)$ & --- & $0.749\ (.49)$ \\
APR $\leftarrow$ pretrained  & --- & --- & --- & $0.686{\pm}.004\ (.34)$ \\
\bottomrule
\end{tabular}
\end{table}
