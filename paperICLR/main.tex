...
Our contributions are: (i)~a replay-refinement method with two equivalent derivations---parameter-space attribution patching and an expert-anchored trust region---and a divergence-safety guarantee; (ii)~a uniform evaluation across four benchmark families (multi-head GLUE-8, shared-output text-to-text GLUE-8, and eight- and twenty-task CLIP suites) with matched data budgets, matched tuning, and sanity controls; (iii)~evidence that the refinement \emph{complements} merging---improving every initialization tried, with the largest gains from the strongest merges---while remaining safe where unconstrained replay descent shows catastrophic, draw-dependent failures; and (iv)~a measured accuracy--retention frontier on held-out tasks showing that refinement from the pretrained model reaches merge-level accuracy at a fraction of the parameter drift.
...
\paragraph{Settings.}
We evaluate on four benchmark families. \emph{Multi-head} GLUE-8 (CoLA, SST-2, MRPC, STS-B, QQP, MNLI, QNLI, RTE) \citep{wang2018glue} with RoBERTa-base \citep{liu2019roberta} merges the shared encoder and evaluates with each task's own trained head---an intentionally \emph{oracle-task-id}. A \emph{shared-output} track merges full flan-T5-base sequence-to-sequence models evaluated in text-to-text format through a single generation head \citep{raffel2020t5,tang2024fusionbench}, removing the oracle assumption entirely (Section~\ref{sec:t5}). A \emph{CLIP/ViT-B/32} vision track uses the standard eight-task task-vector suite \citep{radford2021clip,ilharco2022taskarithmetic} and its twenty-task extension \citep{wang2024tall}, probing a different modality and the high-interference regime (Section~\ref{sec:clip20}). A \emph{decoder-only LLM} track adopting the MergeBench setting \citep{he2025mergebench} is ongoing work. Refinement uses $n\in\{4,\ldots,128\}$ labeled replay examples per task, class-balanced where possible and disjoint from evaluation; full details appear in Appendix~\ref{app:protocol}.


\paragraph{Baselines.}
Baselines are grouped by data regime: \emph{checkpoint-only} (averaging, task arithmetic, TIES, DARE/DELLA, Breadcrumbs, magnitude Localize-and-Stitch); \emph{data-dependent but label-free} (RegMean, AdaMerging, APL); \emph{labeled} (ordinary replay GD from the same initialization, the ungated distance-scaled update, head-only recalibration, Fisher merging---whose empirical diagonal Fisher is estimated from per-example gradients of the \emph{labeled} loss---and learned-mask Localize-and-Stitch); and \emph{sanity controls} (inverted/random gates, wrong-expert directions, shuffled labels). Every initialization the refinement is launched from is itself label-free, so within each comparison the labeled budget is spent only by the refinement. All labeled baselines use the same replay examples and the same hyperparameter search budget---essential, because the strongest alternative explanation for a positive result is that the method simply performs supervised post-merge replay fine-tuning.

\paragraph{Metrics.}
Tasks are scored by their standard validation metrics: Matthews correlation for CoLA, correlation for STS-B, matched/mismatched accuracy for MNLI, accuracy or accuracy/F1 elsewhere on GLUE, and top-1 accuracy on the CLIP suites; the shared-output track decodes text-to-text and is scored with the same GLUE metrics. These are not commensurable across tasks, so the primary aggregate throughout is normalized retention,
\begin{equation}
    \mathrm{NormRet}_t
    =
    \frac{
...
\end{equation}
which places the pretrained model at $0$ and the task expert at $1$ ($\epsilon_{\mathrm{metric}}$ guards small denominators). Mean and worst-task values are always reported together---worst-task matters because a merge can improve the average while catastrophically harming a small task---with seed-level intervals for the main tables. One benchmark departs from this convention: on the twenty-task CLIP suite several experts barely improve on the zero-shot backbone, so the retention denominator degenerates and we report absolute mean and worst-task top-1 accuracy instead, quoting the zero-shot floor and expert ceiling for scale (Section~\ref{sec:clip20}). Where a base score is undefined---zero-shot flan-T5-base emits non-numeric strings on STS-B---it is set to zero, so that task's retention is measured against a zero floor.

\paragraph{Protocol.}
Unless noted, refinement runs $S$ sweeps from the task-arithmetic merge with the trust region applied after the learning rate (Eq.~\eqref{eq:clipped-update}) and $\epsilon_{\mathrm{gate}}=0$; each family---ordinary replay gradient descent (GD), the proposed gated update (APR), and the ungated distance-scaled update---receives an equal-size learning-rate search, reporting the best configuration by the benchmark's primary metric (mean normalized retention; mean top-1 accuracy on the twenty-task suite). Sweep cells are single-seed (spread ${\approx}\pm0.02$); headline configurations are replicated over five replay-buffer seeds (Table~\ref{tab:main}). Sweeps select per family on the evaluation split---uniform, but oracle; the stricter replay-only selection is applied in Table~\ref{tab:matched}. The multi-head oracle-task-id assumption is removed in Section~\ref{sec:t5}.

\section{Results}
\label{sec:results}
Every benchmark receives the same experiment grid---each initialization alone and refined, the unconstrained baselines, and the gate controls---and we report it at two tiers that differ in how hyperparameters are chosen. Table~\ref{tab:main} is the \emph{tuned} tier: $n=64$ replay examples per task, configurations selected on the evaluation split, which measures how good each method can be. Table~\ref{tab:main16} is the \emph{deployment} tier: $16$ replay examples plus a disjoint $16$-example selection buffer per task, every hyperparameter---including the sweep count---chosen on that buffer, and the evaluation split read only at the selected cell, which measures what a practitioner actually gets. The tiers are not cell-by-cell comparable, and we draw no conclusion from differences between them; each is internally consistent. Dashes mark experiments not yet run. The subsections take one benchmark each and add its benchmark-specific studies (replay budget, refinement horizon, held-out retention). Cross-benchmark analyses are consolidated in Appendix~\ref{app:mergebaselines}.

\begin{table}[t]
\centering
\scriptsize
\setlength{\tabcolsep}{3.5pt}
\caption{The canonical experiment grid, instantiated identically on every benchmark. All cells use $n=64$ labeled replay examples per task and $S=5$ refinement sweeps, except $^{\dagger}$ ($S=30$; the from-pretrained cells are replicated only at the longer horizon). Entries are the mean aggregate with worst-task in parentheses---normalized retention, except twenty-task CLIP (absolute top-1 accuracy). The pretrained \emph{alone} row is the retention zero point (CLIP-20: the $0.550$ zero-shot floor). No initialization is privileged; the two unconstrained baselines ($+$GD, $+$ungated) are run from the pretrained model and from task arithmetic, which suffices to separate the anchor and the gate from the initialization, while $+$APR is run from every initialization to test composition. The ungated update from the remaining five initializations is reported for CLIP-20 in Table~\ref{tab:clip20matrix}. $\pm$ marks five-seed cells; the rest are single-seed. Dashes are experiments not yet run.}
\label{tab:main}
...
\scriptsize
\setlength{\tabcolsep}{3.5pt}
\caption{\textbf{The same grid under split selection at a $16{+}16$ budget.} Each task contributes $16$ labeled replay examples and a \emph{disjoint} $16$-example selection buffer ($32$ labels per task in total). Every hyperparameter---merge coefficient, density, refinement learning rate, and the sweep count $S$---is chosen by minimizing held-out loss on the selection buffer, and the evaluation split is read \emph{only} at the selected cell. Table~\ref{tab:main} instead tunes on the evaluation split, so the two tables answer different questions and their numbers should not be compared cell by cell: this one reports what a practitioner obtains without ever touching test data. Entries are mean aggregate with worst-task in parentheses---normalized retention, except twenty-task CLIP (absolute top-1); the pretrained \emph{alone} row is the retention zero point. The search budget is identical across benchmarks and rows: $S\in\{5,20,50\}$ throughout. AdaMerging is given the matched budget---the same $16$ unlabeled inputs per task that APR sees labeled---rather than the standard transductive formulation that adapts on the evaluation split. $^{\dagger}$: the selected $S$ is the longest offered, so the cell is a lower bound rather than a verified interior optimum; the extended search reported in the text resolves these for the from-pretrained rows of GLUE-8 and CLIP-8. Dashes are experiments not yet run, except AdaMerging on the shared-output track, where the entropy objective requires a classification head and the baseline does not apply.}
\label{tab:main16}
\setlength{\arraystretch}{0.95}
\begin{tabular}{llcccc}
\toprule
Initialization & Treatment & GLUE-8 & CLIP-8 & GLUE-8 t2t & CLIP-20 \\
 & & \multicolumn{3}{c}{normalized retention} & top-1 acc. \\
\midrule
Pretrained      & alone      & $0$ & $0$ & $0$ & $0.550\ ({+}.10)$ \\
                & $+$GD      & $0.209^{\dagger}\ ({-}.18)$ & $0.096^{\dagger}\ ({-}.19)$ & $0.094^{\dagger}\ ({-}.52)$ & $0.583^{\dagger}\ ({+}.12)$ \\
                & $+$ungated & $-0.431^{\dagger}\ ({-}3.52)$ & $-0.036\ ({-}.59)$ & $0.131\ ({-}.36)$ & $0.586\ ({+}.11)$ \\
                & $+$APR     & $0.315^{\dagger}\ ({-}.12)$ & $0.432^{\dagger}\ ({-}.16)$ & $0.508^{\dagger}\ ({+}.03)$ & $0.647\ ({+}.22)$ \\
\midrule
Task arithmetic & alone      & $0.320\ ({-}.18)$ & $0.270\ ({-}.53)$ & $0.352\ ({-}.50)$ & $0.495\ ({+}.15)$ \\
                & $+$GD      & $0.574\ ({+}.06)$ & $0.425^{\dagger}\ ({-}.31)$ & $0.660\ ({+}.33)$ & $0.555\ ({+}.18)$ \\
                & $+$ungated & $0.586\ ({-}.01)$ & $0.450\ ({-}.25)$ & $0.538\ ({-}.33)$ & $0.602\ ({+}.17)$ \\
                & $+$APR     & $\mathbf{0.608}\ ({+}.10)$ & $0.524^{\dagger}\ ({-}.01)$ & $0.534\ ({+}.08)$ & $0.667\ ({+}.25)$ \\
\midrule
Breadcrumbs     & alone      & $0.030\ ({-}2.14)$ & $0.431\ ({+}.08)$ & $0.409\ ({+}.00)$ & $0.603\ ({+}.12)$ \\
                & $+$GD      & $0.534\ ({+}.04)$ & $0.513^{\dagger}\ ({+}.07)$ & $0.658\ ({+}.05)$ & $0.638^{\dagger}\ ({+}.17)$ \\
                & $+$ungated & $0.563\ ({+}.04)$ & $0.487^{\dagger}\ ({-}.04)$ & $0.563\ ({-}.27)$ & $0.650\ ({+}.19)$ \\
                & $+$APR     & $0.567\ ({+}.04)$ & $0.504^{\dagger}\ ({+}.06)$ & $0.564^{\dagger}\ ({+}.10)$ & $0.661\ ({+}.20)$ \\
\midrule
DARE-TIES       & alone      & $0.301\ ({-}.34)$ & $0.364\ ({-}.32)$ & $0.371\ ({-}.42)$ & $0.520\ ({+}.20)$ \\
                & $+$GD      & $0.521\ ({-}.08)$ & $0.515^{\dagger}\ ({-}.07)$ & $0.651^{\dagger}\ ({+}.33)$ & $0.594^{\dagger}\ ({+}.28)$ \\
                & $+$ungated & $0.574\ ({+}.09)$ & $0.284^{\dagger}\ ({-}1.99)$ & $0.489\ ({-}.17)$ & $0.579\ ({+}.09)$ \\
                & $+$APR     & $0.586\ ({+}.11)$ & $0.506^{\dagger}\ ({-}.58)$ & $0.662\ ({+}.20)$ & $0.664\ ({+}.30)$ \\
\midrule
TIES            & alone      & $0.208\ ({-}.22)$ & $0.525\ ({+}.21)$ & $0.418\ ({-}.17)$ & $0.619\ ({+}.19)$ \\
                & $+$GD      & $0.516\ ({-}.13)$ & $0.599^{\dagger}\ ({+}.21)$ & $\mathbf{0.716}\ ({+}.39)$ & $0.658^{\dagger}\ ({+}.26)$ \\
                & $+$ungated & $0.525\ ({-}.08)$ & $0.564^{\dagger}\ ({-}.23)$ & $0.606\ ({+}.00)$ & $0.672\ ({+}.28)$ \\
                & $+$APR     & $0.543\ ({-}.03)$ & $0.548^{\dagger}\ ({-}.27)$ & $0.575\ ({+}.17)$ & $\mathbf{0.676}\ ({+}.27)$ \\
\midrule
AdaMerging      & alone      & $0.029\ ({-}3.02)$ & $0.533\ ({+}.09)$ & --- & $0.597\ ({+}.10)$ \\
                & $+$GD      & $0.459^{\dagger}\ ({-}.35)$ & $0.601^{\dagger}\ ({+}.16)$ & --- & $0.637^{\dagger}\ ({+}.13)$ \\
                & $+$ungated & $0.538\ ({-}.36)$ & $0.569^{\dagger}\ ({-}.06)$ & --- & $0.649\ ({+}.16)$ \\
                & $+$APR     & $0.564\ ({-}.02)$ & $\mathbf{0.639}^{\dagger}\ ({+}.30)$ & --- & $0.665\ ({+}.20)$ \\
\midrule
RegMean         & alone      & --- & --- & --- & --- \\
                & $+$GD      & --- & --- & --- & --- \\
                & $+$ungated & --- & --- & --- & --- \\
...
\scriptsize
\setlength{\tabcolsep}{3.5pt}
\caption{\textbf{The canonical grid under split selection at an $8{+}8$ budget.} Each task contributes eight labeled replay examples and a disjoint eight-example selection buffer. Every hyperparameter, including $S\in\{5,20,50\}$, is selected on the selection buffer before the evaluation split is read. Entries follow Table~\ref{tab:main16}: mean normalized retention with worst-task retention in parentheses, except CLIP-20, which reports absolute top-1 accuracy. $^{\dagger}$ marks a selected horizon at the longest value offered. Blank cells are experiments not yet available.}
\label{tab:main8}
\setlength{\arraystretch}{0.95}
\begin{tabular}{llcccc}
\toprule
...
\paragraph{What split selection changes.}
Table~\ref{tab:main16} repeats the grid under a protocol in which no test data is used for tuning, at a quarter of the labeled budget. The result splits cleanly in two, along the axis of whether a usable merge already exists.

Refining \emph{from the pretrained model}---the setting with no merge to inherit, and the one where the refinement must supply all of the multi-task ability itself---is an unambiguous win for the gated, expert-anchored update. APR is the best treatment on all four benchmarks, ahead of the better of the two unconstrained baselines by $0.06$ (CLIP-20), $0.11$ (GLUE-8), $0.34$ (CLIP-8), and $0.38$ (shared-output). Neither unconstrained baseline is a viable cold-start method at this budget: ordinary descent recovers only a tenth to a fifth of the expert--base gap on the three retention benchmarks, and split selection puts the ungated update into an actively harmful configuration on GLUE-8 ($-0.431$, worst task $-3.5$). Only the gated, distance-scaled update turns a small replay buffer into multi-task ability with no merge to build on.
...
\label{sec:clip20}
We next scale the vision track to the twenty-task suite of \citet{wang2024tall}, using the published fine-tuned ViT-B/32 encoders. The scale forces two protocol changes: the conventional $\lambda_i=0.3$ is catastrophic here (merge $0.364$, \emph{below} the $0.550$ zero-shot floor; tuned $\lambda\approx0.08$ reaches $0.604$), and, as declared in Section~\ref{sec:protocol}, retention degenerates on near-zero expert--base gaps, so this section reports absolute mean and worst-task top-1 accuracy (zero-shot floor $0.550$, expert ceiling $0.896$). Cells are single-seed at $n=64$ with evaluation-split selection applied symmetrically (a disjoint-holdout pilot moves results by $0.001$); headline cells carry five-seed error bars (Table~\ref{tab:clip20seeds}).
...
Across an initialization$\times$method matrix (six initializations; APR column in Table~\ref{tab:main}, full matrix including the ungated column in Table~\ref{tab:clip20matrix}, Appendix~\ref{app:clip20}), APR and the ungated anchored update agree within $\pm0.01$ everywhere (five-seed paired differences $-0.002\pm0.006$ from TIES, $+0.001\pm0.005$ from AdaMerging), while the inverted gate collapses to $0.100$: the attribution \emph{sign} carries signal, but thresholding at zero adds nothing over no gate. Refinement improves every initialization and preserves their ordering. A held-out audit explains why: first-order attributions transfer to a disjoint buffer only weakly and uniformly across magnitude buckets---the gradient's transferable contribution is step magnitude, not coordinate choice (Appendix~\ref{app:clip20}).
...
\label{app:glue}
This appendix reproduces the full discussion of the multi-head RoBERTa experiments summarized in Section~\ref{sec:glue}.
...
\paragraph{Sign election, not sparsification.}
DARE alone reproduces task arithmetic almost exactly (its drop-and-rescale is expectation-preserving over ${\sim}10^8$ coordinates), isolating sign election---not sparsification---as TIES's operative ingredient.
