...
\paragraph{Protocol.}
Every method receives the same $B$ labeled examples per task and never sees a label outside them. Merge baselines choose their hyperparameters (coefficient, density, scale) by replay loss on all $B$ examples. APR and its ablations split the budget in half: they refine on one half, choose the learning rate $\eta$ and sweep count $S$ by replay loss on the other, and are then refit once on all $B$ examples at the chosen cell. Grids are extended until the selected cell is interior, the evaluation split is read only for selected cells, and every cell is repeated over $D$ independent buffer draws and reported as mean $\pm$ standard deviation. Appendix~\ref{app:protocol} gives the grids and the remaining details.
...
\paragraph{Metrics.}
Both CLIP suites report mean and worst-task top-1 accuracy, with the zero-shot floor and the expert ceiling quoted for scale. The GLUE-8 tasks use metrics that do not average meaningfully (Matthews correlation, correlations, accuracies), so GLUE-8 reports \emph{normalized retention}: each task's metric is rescaled to place the pretrained model at $0$ and the task expert at $1$ before averaging (Appendix~\ref{app:protocol}). Worst-task values accompany every mean, since a merge can raise the average while harming one task badly.
...
\caption{\textbf{Both eight-task suites under the equal-budget protocol} ($B{=}32$ labels per task for every method, mean $\pm$ std over three buffer draws). CLIP-8 reports top-1 accuracy, GLUE-8 normalized retention; worst-task columns are means over draws. Merges are deterministic given the checkpoints, so their spread reflects selection alone. GD and ungated are APR's ablations, run from the pretrained model where they isolate the anchor and the gate. APR's horizon grid is capped at $S\le100$ on GLUE-8 (Section~\ref{sec:merged-task-performance}).}
\label{tab:cv-v2}
\begin{tabular}{lcccc}
\toprule
 & \multicolumn{2}{c}{CLIP-8 (top-1 acc.)} & \multicolumn{2}{c}{GLUE-8 (NormRet)} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
Model & Mean & Worst-task & Mean & Worst-task \\
\midrule
Pretrained $\thetab$       & $0.478$ & $0.315$ & $0$ & $0$ \\
\midrule
Task arithmetic            & $0.692\pm0.000$ & $0.518\pm0.000$ & $\mathbf{0.320}\pm0.000$ & $-0.175$ \\
Breadcrumbs                & $0.705\pm0.000$ & $0.521\pm0.000$ & $0.030\pm0.000$ & $-2.145$ \\
DARE-TIES                  & $0.708\pm0.000$ & $0.486\pm0.000$ & $0.301\pm0.000$ & $-0.344$ \\
TIES                       & $0.741\pm0.000$ & $\mathbf{0.528}\pm0.000$ & $0.208\pm0.000$ & $-0.216$ \\
\midrule
$\thetab+$GD (ablation)      & $0.558\pm0.018$ & $0.369\pm0.039$ & $0.001\pm0.230$ & $-1.408$ \\
$\thetab+$ungated (ablation) & $0.565\pm0.008$ & $0.401\pm0.024$ & $-0.167\pm0.319$ & $-2.445$ \\
$\thetab+$APR                & $\mathbf{0.756}\pm0.004$ & $0.523\pm0.007$ & $0.261\pm0.182$ & $\mathbf{-0.119}$ \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:cv-v2} reports both eight-task suites under the protocol of Section~\ref{sec:protocol}: every method given the same $B=32$ labels per task, hyperparameters selected inside that budget, and every number a mean over three independent buffer draws. On CLIP-8, APR from the pretrained model reaches $0.756\pm0.004$ mean accuracy, above every checkpoint-only merge including TIES ($0.741$), while using $32$ labels per task in place of the eight expert checkpoints a merge requires; every selected cell is an interior optimum in $\eta$ and $S$. Its two ablations sit at $0.558$ and $0.565$---barely above the zero-shot floor once one accounts for the merges' head start---so the anchored, capped step is worth ${\approx}0.19$ accuracy over unconstrained descent from the same initialization, and the gate alone does not account for it.

On GLUE-8 the ordering against the ablations is the same but the ordering against the merges is not. APR beats ordinary descent and the ungated update on every draw ($0.369$, $0.051$, $0.362$ against $-0.201$, $-0.047$, $0.251$ for GD and $0.201$, $-0.336$, $-0.365$ ungated), yet its mean of $0.261\pm0.182$ falls below task arithmetic's $0.320$, because one draw of three selects a near-no-op. On that draw no cell of the grid lowers the held-out replay loss by more than $0.7\%$ of the pretrained model's---the objective rises with $S$ at every learning rate---so selection settles on $\eta=4$, $S=20$, a model that moves $0.47$ from $\thetab$ against $1.80$ and $2.82$ on the other two draws and retains almost nothing on any task. The same draw defeats the other labeled arms in the same way (GD reaches a better held-out objective there and a worse evaluation), so the failure belongs to the $16{+}16$ split on this suite rather than to the anchored update: sixteen selection examples spread over eight tasks with incommensurable losses do not always contain the signal that selection needs. We report the draw as it is; the two draws on which selection works place APR at $0.36$--$0.37$, above every merge. One protocol economy is specific to GLUE-8: APR's horizon grid is capped at $S\le100$ there, because an uncapped run kept extending the grid to $S=800$ while the held-out objective, still falling at $S=400$, bought nothing at evaluation ($0.368$ uncapped against $0.369$ capped on the draw affected); the ablations are uncapped.

On CLIP-8 the draw spread is the more interesting number. APR varies by $\pm0.004$ across buffer draws, the same order as the deterministic merges, and selects essentially one cell every time ($\eta=32$, $S\in\{50,100\}$). An earlier version of this experiment showed the opposite---a $\pm0.081$ spread with the fine-grained Cars task collapsing on two draws of three---which we traced to the replay sampler: with more classes than the budget holds it had been keeping the lowest class ids deterministically, so Cars was refined and selected on $32$ of its $196$ classes, identically for every seed. Drawing the covered classes at random per seed (Appendix~\ref{app:protocol}) removes both the collapse and the spread. We report this because the failure is instructive: at these budgets a fine-grained task is representable only in part, and a selection buffer that inherits the same blind spot cannot detect damage to the classes it never sees.
...
The selection objective is the mean over tasks of held-out replay loss, each task's loss normalized by the initialization's loss on the same examples, so that tasks with different loss scales weigh equally. Select-then-refit chooses $(\eta,S)$ at construction size $B/2$ and applies it at $B$; we accept this standard caveat rather than spend half the budget only on selection. A selected cell counts as interior when both neighbouring grid values are worse; when it lies on a boundary the grid is extended in that direction and selection repeated (at most twice). For $S$ the smallest value counts as interior when the unrefined initialization scores worse, since $S=0$ then brackets it. Refinement runs $S$ sweeps at a constant learning rate with the task order re-randomized every sweep from a fixed seed, so the $D$ draws vary only the buffer; the expert-distance cap is applied per update (Eq.~\eqref{eq:clipped-update}). Each refinement family receives an equal-size $(\eta,S)$ grid: $S\in\{5,20,50,100\}$ throughout; $\eta\in\{1,\ldots,32\}$ (APR), $\{0.5,\ldots,16\}$ (ungated), $\{10^{-5},\ldots,10^{-2}\}$ (GD) on the encoder suites, in factor-of-two steps. AdaMerging uses the matched budget (the same $B$ unlabeled inputs per task) rather than its transductive formulation on the evaluation split; DOGE/APGD builds its candidates without labels and uses the budget only to select its global scale. This accounting differs from common practice, where merge coefficients are tuned on the tasks' full validation splits outside any stated budget \citep{ilharco2022taskarithmetic,yadav2023ties} and that tuning dominates the cost of merging \citep{he2025mergebench}; here it happens inside the same $B$ labels every method is charged for.

\paragraph{Normalized retention.}
For GLUE-8, whose per-task metrics are not commensurable, the reported aggregate is
\begin{equation}
    \mathrm{NormRet}_t
    =
    \frac{
...
\end{equation}
which places the pretrained model at $0$ and the task expert at $1$ ($\epsilon_{\mathrm{metric}}$ guards small denominators). The CLIP suites report absolute accuracy instead: their per-task metric is already commensurable, and on the twenty-task suite several experts barely improve on the zero-shot backbone (STL10 by $0.004$), so a near-zero denominator would make per-task retention explode.
