...
\quad $+$XGD             & $0.793_{\pm0.010}$ & $0.556_{\pm0.013}$ & $\mathbf{0.699}_{\pm0.006}$ & $0.212_{\pm0.028}$ & $0.692_{\pm0.004}$ & $\mathbf{0.329}_{\pm0.018}$ \\
TIES                     & $0.732_{\pm0.002}$ & $0.524_{\pm0.018}$ & $0.549_{\pm0.000}$ & $0.234_{\pm0.000}$ & $0.619_{\pm0.000}$ & $0.194_{\pm0.000}$ \\
\quad $+$XGD             & $0.785_{\pm0.007}$ & $0.564_{\pm0.005}$ & $0.690_{\pm0.010}$ & $\mathbf{0.363}_{\pm0.028}$ & $0.695_{\pm0.003}$ & $0.319_{\pm0.010}$ \\
\midrule
...
RegMean                  & $\mathbf{0.810}_{\pm0.003}$ & $\mathbf{0.610}_{\pm0.008}$ & $0.625_{\pm0.004}$ & $0.120_{\pm0.015}$ & $\mathbf{0.721}_{\pm0.001}$ & $0.223_{\pm0.002}$ \\
...
\quad $+$XGD             & $0.740_{\pm0.004}$ & $0.539_{\pm0.015}$ & $0.676_{\pm0.012}$ & $0.052_{\pm0.042}$ & $0.653_{\pm0.007}$ & $0.215_{\pm0.032}$ \\
Breadcrumbs              & $0.705_{\pm0.000}$ & $0.521_{\pm0.000}$ & $0.470_{\pm0.020}$ & $0.132_{\pm0.017}$ & $0.603_{\pm0.000}$ & $0.119_{\pm0.000}$ \\
\quad $+$XGD             & $0.756_{\pm0.006}$ & $0.561_{\pm0.013}$ & $0.667_{\pm0.005}$ & $0.114_{\pm0.037}$ & $0.658_{\pm0.005}$ & $0.225_{\pm0.037}$ \\
DARE-TIES                & $0.708_{\pm0.000}$ & $0.486_{\pm0.000}$ & $0.554_{\pm0.016}$ & $0.256_{\pm0.021}$ & $0.520_{\pm0.000}$ & $0.202_{\pm0.000}$ \\
\quad $+$XGD             & $0.766_{\pm0.001}$ & $0.521_{\pm0.003}$ & $\mathbf{0.684}_{\pm0.003}$ & $0.181_{\pm0.013}$ & $0.662_{\pm0.006}$ & $\mathbf{0.287}_{\pm0.022}$ \\
TIES                     & $0.734_{\pm0.007}$ & $0.512_{\pm0.014}$ & $0.549_{\pm0.000}$ & $0.234_{\pm0.000}$ & $0.612_{\pm0.013}$ & $0.201_{\pm0.013}$ \\
\quad $+$XGD             & $0.774_{\pm0.004}$ & $0.550_{\pm0.011}$ & $0.673_{\pm0.006}$ & $\mathbf{0.290}_{\pm0.042}$ & $0.679_{\pm0.010}$ & $0.275_{\pm0.006}$ \\
\midrule
...
\midrule
TATR                     & $0.719_{\pm0.001}$ & $0.525_{\pm0.012}$ & $0.569_{\pm0.007}$ & $0.217_{\pm0.025}$ & $0.607_{\pm0.005}$ & $0.136_{\pm0.013}$ \\
RegMean                  & $\mathbf{0.800}_{\pm0.006}$ & $\mathbf{0.590}_{\pm0.009}$ & $0.610_{\pm0.009}$ & $0.115_{\pm0.035}$ & $\mathbf{0.716}_{\pm0.002}$ & $0.227_{\pm0.008}$ \\
...
The same rule, unchanged, applies from a merge initialization, and this is where the refinement is most reliable: across all three suites at both budgets, every one of the twenty-four composition cells improves the merge it started from, on every draw. The best cell of each suite is a composition rather than the cold start---$0.793$ on CLIP-8 and $0.699$ on GLUE-8 at $B=32$, both from DARE-TIES, and $0.695$ on CLIP-20 from TIES---and the gain is largest where the merge is weakest: $0.17$ from DARE-TIES on CLIP-20 ($0.520\to0.692$) and $0.19$--$0.21$ from Breadcrumbs on GLUE-8 at $B=16$ ($0.470\to0.667$). Refinement also compresses the spread between merges, so which merge one starts from matters less once the labels are spent: CLIP-20's four merges span $0.520$--$0.619$ before refinement and $0.664$--$0.695$ after, and GLUE-8's at $B=16$ span $0.470$--$0.554$ before and $0.667$--$0.684$ after. Composition is the more stable of the two settings at the smaller budget---on CLIP-8 at $B=16$ the cold start varies by $\pm0.050$ across draws while the four composition rows vary by $\pm0.001$--$0.006$---and it removes the one cell in which a merge beats refinement. The selected rates are an order of magnitude smaller than from the pretrained model ($\eta\in[0.25,4]$ on the CLIP suites against $\eta=32$; several draws sit at the twice-extended lower edge of the grid), which is what a starting point that already carries multi-task signal should require. Worst-task behaviour splits by initialization on GLUE-8: TIES$+$XGD raises the weakest task from $0.234$ to $0.363$, while task arithmetic$+$XGD lowers it from $0.130$ to $0.059$. The ablation arms are reported only from the pretrained model, where they isolate the anchor and the gate; from a merge the comparison of interest is against the merge itself, which every composition row makes directly.

\paragraph{Other uses of the same budget.}
Four baselines spend the same $B$ examples per task on constructing or reweighting the merge rather than on moving the encoder. Two are not competitive. Fisher merging with a replay-estimated Fisher and Localize-and-Stitch with masks learned on the buffer fall below the best checkpoint-only merge in all six suite--budget cells and below every composition row, and Localize-and-Stitch is the least stable estimator in the study: its GLUE-8 mean varies by $\pm0.077$ at $B=32$ and $\pm0.107$ at $B=16$, with a worst task below the pretrained model. TATR, which restricts task arithmetic to the low-conflict trust region using the budget's gradients at $\thetab$, improves plain task arithmetic in all six cells (by $0.005$--$0.034$) and becomes the best checkpoint-only merge on GLUE-8 at $B=16$, but stays below the cold start and below every composition.

RegMean is the exception, and the strongest baseline in the study. Fitting linear-layer Gram statistics on the budget's \emph{inputs} alone---it reads no labels---it reaches $0.810$ and $0.800$ on CLIP-8 and $0.721$ and $0.716$ on CLIP-20 at $B=32$ and $B=16$, above every other cell of Tables~\ref{tab:b32} and~\ref{tab:b16} including every composition, and with the tightest spreads we measure ($\pm0.001$--$0.006$). On GLUE-8 it does not transfer: $0.625$ and $0.610$, above refinement from the pretrained model but $0.07$ below the best composition at both budgets. We read this as the boundary of the claim rather than around it. Where a closed-form input-statistics estimator applies---CLIP's frozen linear heads over one shared visual encoder---it beats gradient refinement at these budgets; on multi-head GLUE-8, where each task keeps its own trained head and that structure is absent, refinement leads. A practitioner with CLIP-shaped models and no labels should try RegMean first; the refinement earns its place where labels exist and the head structure does not hand the merge a closed form.
