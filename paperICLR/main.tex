...
\midrule
% TODO(authors): the labeled-alternatives block is partial -- coefficient
% tuning, loss weighting, and head-only recalibration are still to be run
% under split selection (extend scripts/labeled_baselines_n16.py --baselines
% and fill from grid_nn_{glue8,clip8,clip20}_labeled_n16.json). Replay
% fine-tuning is NOT a separate row: the GD ablation is that baseline.
