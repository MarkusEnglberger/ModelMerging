...
\paragraph{Protocol.}
Unless noted, refinement runs $S$ sweeps from the task-arithmetic merge with the trust region applied after the learning rate (Eq.~\eqref{eq:clipped-update}) and $\epsilon_{\mathrm{gate}}=0$; each family---ordinary replay gradient descent (GD), the proposed gated update (APR), and the ungated distance-scaled update---receives an equal-size learning-rate search, reporting the best configuration by the benchmark's primary metric (mean normalized retention; mean top-1 accuracy on the twenty-task suite). Sweep cells are single-seed (spread ${\approx}\pm0.02$); headline configurations are replicated over five replay-buffer seeds (Tables~\ref{tab:glue8seeds} and~\ref{tab:clip8seeds}). Sweeps select per family on the evaluation split---uniform, but oracle; the stricter replay-only selection is applied in Table~\ref{tab:matched}. The multi-head oracle-task-id assumption is removed in Section~\ref{sec:t5}.

\section{Results}
\label{sec:results}
Each benchmark is presented in the same format: the setting, the three-family comparison at matched budget with its controls, and composition with stronger initializations. Cross-benchmark analyses (step-size scaling across initializations, the AdaMerging parameterization study, coefficient tuning) are consolidated in Appendix~\ref{app:mergebaselines}.
...
\centering
\footnotesize
\caption{Multi-head GLUE-8: five-seed replication of the headline configurations (mean $\pm$ std of mean normalized retention, worst-task in parentheses). Each cell re-runs the sweep-best configuration with a fresh replay buffer per seed; the random gate is density-matched to the attribution gate.}
\label{tab:glue8seeds}
\begin{tabular}{lc}
\toprule
Method & mean $\pm$ std (worst $\pm$ std) \\
\midrule
Ordinary replay GD $\leftarrow$ TA & $0.483\pm.017\ (-0.048\pm.058)$ \\
Random gate $\leftarrow$ TA        & $0.568\pm.031\ (+0.030\pm.043)$ \\
Ungated $\leftarrow$ TA            & $0.593\pm.040\ (+0.085\pm.096)$ \\
APR $\leftarrow$ TA                & $0.542\pm.062\ (-0.170\pm.430)$ \\
APR $\leftarrow$ DARE-TIES         & $\mathbf{0.620\pm.021}\ (+0.180\pm.053)$ \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\footnotesize
\caption{Eight-task CLIP: five-seed replication of the headline configurations (mean $\pm$ std of mean normalized retention, worst-task in parentheses); protocol as in Table~\ref{tab:glue8seeds}.}
\label{tab:clip8seeds}
\begin{tabular}{lc}
\toprule
Method & mean $\pm$ std (worst $\pm$ std) \\
\midrule
Ordinary replay GD $\leftarrow$ TA & $0.460\pm.002\ (-0.200\pm.007)$ \\
Random gate $\leftarrow$ TA        & $0.498\pm.117\ (-0.564\pm.888)$ \\
Ungated $\leftarrow$ TA            & $0.547\pm.007\ (-0.068\pm.016)$ \\
AdaMerging (layer-wise)            & $0.595\pm.037\ (+0.168\pm.076)$ \\
APR $\leftarrow$ TA                & $0.605\pm.008\ (+0.151\pm.020)$ \\
...
These results carry three qualifications. First, the hyperparameter-sweep cells are single-seed, with a run-to-run spread of roughly $\pm0.02$ mean retention (Table~\ref{tab:glue8}), so sweep-level differences smaller than that should not be read as orderings; the headline configurations are replicated over five replay-buffer seeds (Tables~\ref{tab:glue8seeds} and~\ref{tab:clip8seeds}). Second, hyperparameters (learning rate, and in Section~\ref{sec:mergebaselines} also $\lambda$, sparsity, and density) are selected \emph{per family} by mean normalized retention on the evaluation split---a uniform and therefore fair rule, but oracle selection; the stricter protocol, in which every labeled method selects its hyperparameters on the replay buffer alone, is applied in Table~\ref{tab:matched}. Third, the multi-head GLUE setting assumes oracle task identity.
...
A density-matched random mask makes this concrete. Over five replay-buffer seeds on GLUE-8 (Table~\ref{tab:glue8seeds}), the true gate ($0.542\pm0.062$) is statistically indistinguishable from---indeed nominally worse than---a random gate of the same density ($0.568\pm0.031$), and both trail the ungated update ($0.593\pm0.040$): on this modality the gate's \emph{sign} carries no signal beyond its density, as the embedding-sparsity account predicts. The same control behaves oppositely on CLIP (Section~\ref{sec:mergebaselines}), sharpening the modality split: the attribution sign is informative where gradients are dense, and vacuous where the replay buffer leaves most coordinates without gradient.
