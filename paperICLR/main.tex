...
\definecolor{aprPurple}{RGB}{95,35,205}
\definecolor{aprPurpleLight}{RGB}{242,229,250}
\definecolor{aprOrange}{RGB}{245,105,0}
\definecolor{aprBlue}{RGB}{20,80,190}
\definecolor{aprGreen}{RGB}{0,150,30}
\definecolor{aprRed}{RGB}{235,25,15}
...
\title{Expert-Anchored Replay Refinement \\ for Task-Vector Merging}
...
We introduce expert-anchored gradient descent (XGD), which can start from any initial merge or directly from the pretrained model.
...
We propose a refinement that exploits the position of the task experts, \emph{expert-anchored gradient descent} (XGD): each coordinate takes a gradient step scaled by its remaining distance to the expert and confined to the interval between its current value and the expert's value---it moves only toward the expert, only where the replay gradient locally agrees, and never past it---from \emph{any} initialization, including the pretrained model itself. The update is provably non-expansive toward the experts' box hull at any learning rate (Proposition~\ref{prop:box-nonexpansive}).

Our contributions are:
(i)~a replay-refinement method---expert-anchored gradient descent, a distance-preconditioned descent step projected onto the interval between the current model and each task expert---with a divergence-safety guarantee;
...
The experts anchor the update. For current model parameters $\vartheta$, let $v_i(\vartheta)=\theta_i-\vartheta$ be the coordinate-wise direction to expert $i$ and let $g_i(\vartheta)$ be task $i$'s replay gradient under its prediction head. Expert $i$ is a parameter setting known to attain low loss on task $i$, so for each coordinate $r$ the interval between the current value $\vartheta_r$ and the expert value $\theta_{i,r}$ is the natural admissible region for a task-$i$ update: within it, the coordinate interpolates between the current model and one that works for the task, whereas a step beyond the expert, or away from it, has no such certificate. Our method---\emph{expert-anchored gradient descent} (XGD)---is ordinary descent made to respect this region. For each task $i$ and coordinate $r$, we (i) \emph{precondition} the negative-gradient step by the current expert distance $|v_{i,r}|$, so that the expert's offset sets each coordinate's unit of movement and coordinates already agreeing with the expert barely move; and (ii) \emph{project} the preconditioned step onto the admissible interval. The projection has two coordinate-wise effects: a step pointing away from the expert ($g_{i,r}v_{i,r}\ge 0$) returns zero, so the coordinate does not move---we call this the \emph{gate}---and a step pointing past the expert stops at the expert value---the \emph{expert-distance cap}. No update ever leaves the interval between the current value and the expert value.
...
\end{equation}
the model is updated immediately, $\theta^{(s,k)}=\theta^{(s,k-1)}+u_i^{(s,k)}$, with $\theta^{(s+1)}=\theta^{(s,T)}$ after all $T$ tasks in the sweep have been processed. Equation~\eqref{eq:clipped-update} is the projected, distance-preconditioned step written in one line: on coordinates that pass the gate, $-g_{i,r}^{(s,k)}$ and $v_{i,r}^{(s,k)}$ agree in sign, so the preconditioned descent step $-\eta\,g_{i,r}^{(s,k)}|v_{i,r}^{(s,k)}|$ equals $\eta\,|g_{i,r}^{(s,k)}|\,v_{i,r}^{(s,k)}$, and the $\min(\cdot,1)$ factor is the projection onto the admissible interval. Its magnitude can equivalently be written as
\[
    |u_{i,r}^{(s,k)}|
    =
    \min\!\bigl(\eta\,\lvert a_{i,r}(\theta^{(s,k-1)})\rvert,\;\lvert v_{i,r}^{(s,k)}\rvert\bigr),
    \qquad
    a_{i,r}(\vartheta)
    :=
    g_{i,r}(\vartheta)\,v_{i,r}(\vartheta).
\]
The product $a_{i,r}$ has a meaning of its own: it is coordinate $r$'s term in the directional derivative $\langle g_i(\vartheta),v_i(\vartheta)\rangle$, i.e.\ the first-order estimate of the loss change from the single-coordinate patch that sets $\vartheta_r$ to the expert value $\theta_{i,r}$---it attributes the predicted effect of adopting the expert wholesale to the individual coordinates. XGD thus moves exactly the coordinates whose patch is predicted to reduce the loss ($a_{i,r}<0$; throughout, we call this sign condition the \emph{attribution gate}), and never beyond the patch whose effect the estimate describes. The estimate certifies only a local first-order effect for task $i$; cross-task interference must be assessed empirically. Figure~\ref{fig:sequential-anchored-updates} illustrates two consecutive task updates.

\begin{figure}[t]
\centering
\resizebox{\linewidth}{!}{\input{illustration.tex}}
\caption{Sequential expert-anchored updates. (a) Starting from $\theta^{(s,k-1)}$, the task-$i$ update $u_i$ moves coordinate-wise toward $\theta_i$ and produces $\theta^{(s,k)}$. (b) The subsequent task-$j$ update $u_j$ moves from $\theta^{(s,k)}$ toward $\theta_j$ and produces $\theta^{(s,k+1)}$. Each violet rectangle is the coordinate-wise admissible region between the current iterate and the selected expert; the dashed blue rectangle is the expert box spanned by $\theta_i$ and $\theta_j$.}
\label{fig:sequential-anchored-updates}
\end{figure}
...
\begin{algorithm}[H]
\caption{Sequential expert-anchored gradient descent (XGD)}
...
\paragraph{Protocol.}
Every method receives the same budget of $B$ labeled examples per task ($B\in\{16,32\}$), drawn from the training data and disjoint from the evaluation split (Appendix~\ref{app:protocol} specifies the sampling rule), and never sees a label outside it. Merge baselines choose their hyperparameters (coefficient, density, scale) on all $B$ examples. XGD and its ablations split the budget in half: they refine on one half for a fixed horizon of $S=50$ sweeps, choose the learning rate $\eta$ on the other half, and are then refit once on all $B$ examples at the chosen $\eta$. Selection everywhere uses the tasks' own held-out metrics, averaged over tasks, with ties going to the smaller learning rate (for a merge, the smaller scaling coefficient); grids are extended until the selected cell is interior, and the evaluation split is read only for selected cells. Every cell is repeated over three independent buffer draws and reported as mean $\pm$ standard deviation; the draws are fresh, the selection rule itself having been settled on three earlier diagnostic draws (Appendix~\ref{app:selection}). Appendix~\ref{app:protocol} gives the grids and the remaining details.

\paragraph{Baselines.}
We compare against checkpoint-only merges (task arithmetic, TIES, DARE-TIES, Breadcrumbs), RegMean fitted on the same $B$ inputs unlabeled, DOGE/APGD \citep{wei2025apgd}, whose construction is data-free and whose budget selects only its global scale, and further ways of spending the same labels: TATR's trust-region mask \citep{sun2025tatr}, GradFix's gradient-sign mask \citep{rinaldi2025gradfix}, aTLAS's learned anisotropic coefficients \citep{zhang2024atlas}, and Fisher merging and Localize-and-Stitch estimated on the replay buffer (Appendix~\ref{app:protocol}). XGD's two ablations---ordinary replay gradient descent (GD; no anchor, no gate) and the ungated distance-scaled update (anchor without gate)---share XGD's optimizer, schedule, and grid, and are run from the pretrained model, where they isolate what each ingredient contributes; from merge initializations only XGD is run.

\paragraph{Metrics.}
The headline aggregate is the plain mean over tasks of each task's own metric---top-1 accuracy on the CLIP suites; Matthews correlation (CoLA), the mean of Pearson and Spearman correlation (STS-B), the mean of accuracy and F1 (MRPC, QQP) and accuracy elsewhere on GLUE-8, the benchmark's own convention---with the zero-shot floor and the expert ceiling quoted for scale. Worst-task values accompany every mean, since a merge can raise the average while harming one task badly; on GLUE-8 the worst task is reported on its own metric's scale and is usually CoLA's Matthews correlation. Normalized retention, which rescales each task between the pretrained model and its expert (Appendix~\ref{app:protocol}), is reported per task in the appendix only: as an aggregate it lets a task whose expert barely improves on the pretrained model dominate the average, so every table here reports the plain mean.
...
Pretrained      & alone      & $0$ & $0$ & $0$ & $0.550$ \\
                ...
                & $+$XGD     & --- & --- & --- & $\mathbf{0.686^{\dagger}{\pm}.004}\ (.34)$ \\
\midrule
Task arithmetic & alone      & $0.320\ ({-}.18)$ & $0.270\ ({-}.53)$ & $0.383\ ({-}.25)$ & $0.604\ (.12)$ \\
                ...
                & $+$XGD     & $0.542{\pm}.062\ ({-}.17)$ & $0.605{\pm}.008\ ({+}.15)$ & $0.537\ ({+}.02)$ & $0.680\ (.22)$ \\
\midrule
Breadcrumbs     & alone      & $0.143\ ({-}.27)$ & $0.431\ ({+}.08)$ & $0.409\ (.00)$ & $0.611\ (.11)$ \\
                & $+$XGD     & $0.523\ (.10)$ & $0.606\ (.21)$ & $0.468\ ({+}.06)$ & $0.678\ (.18)$ \\
\midrule
DARE-TIES       & alone      & $0.301\ ({-}.34)$ & $0.364\ ({-}.32)$ & $0.391\ ({-}.42)$ & $0.617\ (.15)$ \\
                & $+$XGD     & $\mathbf{0.620}{\pm}.021\ ({+}.18)$ & $0.626\ (.19)$ & $\mathbf{0.687}\ ({+}.11)$ & $0.695\ (.25)$ \\
\midrule
TIES            & alone      & $0.302\ (.00)$ & $0.528\ ({+}.26)$ & $0.457\ ({+}.01)$ & $0.626\ (.15)$ \\
                & $+$XGD     & $0.571\ (.17)$ & $0.635{\pm}.061\ ({+}.15)$ & $0.619\ ({+}.17)$ & $0.697\ (.26)$ \\
\midrule
AdaMerging      & alone      & $0.173\ (.00)$ & $0.610\ ({+}.22)$ & --- & $0.665\ (.10)$ \\
                & $+$XGD     & $0.583\ (.05)$ & $\mathbf{0.699}{\pm}.044\ ({+}.31)$ & --- & $0.707\ (.19)$ \\
\midrule
RegMean         & alone      & $0.478\ ({+}.18)$ & $0.733\ ({+}.44)$ & --- & $0.723\ (.23)$ \\
                & $+$XGD     & $0.103\ ({-}.06)$ & $\mathbf{0.762}\ (.44)$ & --- & $\mathbf{0.749}\ (.49)$ \\
\midrule
...
\caption{\textbf{All three suites at $B{=}32$} (32 labels per task for every method; written $\mathrm{mean}_{\pm\mathrm{std}}$ over three fresh buffer draws). Each suite reports the plain mean of its tasks' own metrics and the worst task on the same scale (Section~\ref{sec:protocol}); expert ceilings are $0.903$ (CLIP-8), $0.860$ (GLUE-8) and $0.896$ (CLIP-20). A ``$+$XGD'' row refines that draw's own selected merge and is directly comparable to the merge row above it. Every method, merges included, selects its hyperparameters inside the budget by the same held-out-metric rule, so a merge row carries a draw spread wherever its selection varied. GD and ungated are XGD's ablations, run from the pretrained model where they isolate the anchor and the gate. The last block spends the same $B$ examples per task on constructing or reweighting the merge instead of on refinement (RegMean reads only the inputs; DOGE's construction is data-free), each paired with its ``$+$XGD'' refinement where run. Zero-shot floors are the $\thetab$ row.}
\label{tab:b32}
\begin{tabular}{lcccccc}
\toprule
 & \multicolumn{2}{c}{CLIP-8} & \multicolumn{2}{c}{GLUE-8} & \multicolumn{2}{c}{CLIP-20} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}\cmidrule(lr){6-7}
Model & Mean & Worst & Mean & Worst & Mean & Worst \\
\midrule
Pretrained $\thetab$     & $0.478$ & $0.315$ & $0.354$ & $-0.047$ & $0.550$ & $0.100$ \\
\midrule
Task arithmetic          & $0.685_{\pm0.011}$ & $0.503_{\pm0.026}$ & $0.552_{\pm0.000}$ & $0.130_{\pm0.000}$ & $0.604_{\pm0.000}$ & $0.115_{\pm0.000}$ \\
\quad $+$XGD             & $0.765_{\pm0.011}$ & $0.557_{\pm0.004}$ & $0.681_{\pm0.023}$ & $0.059_{\pm0.045}$ & $0.664_{\pm0.005}$ & $0.208_{\pm0.051}$ \\
Breadcrumbs              & $0.705_{\pm0.000}$ & $0.521_{\pm0.000}$ & $0.473_{\pm0.015}$ & $0.149_{\pm0.012}$ & $0.598_{\pm0.009}$ & $0.133_{\pm0.024}$ \\
\quad $+$XGD             & $0.766_{\pm0.008}$ & $0.556_{\pm0.006}$ & $0.686_{\pm0.009}$ & $0.096_{\pm0.030}$ & $0.674_{\pm0.004}$ & $0.260_{\pm0.021}$ \\
DARE-TIES                & $0.708_{\pm0.000}$ & $0.486_{\pm0.000}$ & $0.563_{\pm0.000}$ & $0.268_{\pm0.000}$ & $0.520_{\pm0.000}$ & $0.202_{\pm0.000}$ \\
\quad $+$XGD             & $0.793_{\pm0.010}$ & $0.556_{\pm0.013}$ & $0.699_{\pm0.006}$ & $0.211_{\pm0.028}$ & $0.692_{\pm0.004}$ & $0.329_{\pm0.018}$ \\
TIES                     & $0.732_{\pm0.002}$ & $0.524_{\pm0.018}$ & $0.549_{\pm0.000}$ & $0.234_{\pm0.000}$ & $0.619_{\pm0.000}$ & $0.194_{\pm0.000}$ \\
\quad $+$XGD             & $0.784_{\pm0.007}$ & $0.564_{\pm0.005}$ & $0.690_{\pm0.010}$ & $\mathbf{0.363}_{\pm0.028}$ & $0.695_{\pm0.003}$ & $0.319_{\pm0.010}$ \\
\midrule
$\thetab+$GD             & $0.546_{\pm0.032}$ & $0.368_{\pm0.041}$ & $0.521_{\pm0.017}$ & $0.111_{\pm0.068}$ & $0.573_{\pm0.021}$ & $0.103_{\pm0.005}$ \\
$\thetab+$ungated        & $0.579_{\pm0.009}$ & $0.443_{\pm0.012}$ & $0.480_{\pm0.060}$ & $0.115_{\pm0.074}$ & $0.602_{\pm0.005}$ & $0.156_{\pm0.011}$ \\
$\thetab+$XGD            & $0.751_{\pm0.004}$ & $0.524_{\pm0.003}$ & $0.612_{\pm0.015}$ & $0.133_{\pm0.047}$ & $0.663_{\pm0.009}$ & $0.267_{\pm0.029}$ \\
\midrule
aTLAS                    & $0.743_{\pm0.004}$ & $0.577_{\pm0.012}$ & $0.612_{\pm0.014}$ & $0.286_{\pm0.022}$ & $0.675_{\pm0.003}$ & $0.187_{\pm0.032}$ \\
\quad $+$XGD             & $0.768_{\pm0.002}$ & $0.593_{\pm0.012}$ & $0.667_{\pm0.022}$ & $0.221_{\pm0.025}$ & $0.698_{\pm0.004}$ & $0.283_{\pm0.067}$ \\
TATR                     & $0.719_{\pm0.002}$ & $0.526_{\pm0.011}$ & $0.563_{\pm0.008}$ & $0.238_{\pm0.036}$ & $0.612_{\pm0.006}$ & $0.125_{\pm0.009}$ \\
\quad $+$XGD             & $0.777_{\pm0.005}$ & $0.563_{\pm0.007}$ & $\mathbf{0.701}_{\pm0.009}$ & $0.143_{\pm0.086}$ & $0.671_{\pm0.006}$ & $0.243_{\pm0.026}$ \\
RegMean                  & $0.810_{\pm0.003}$ & $0.610_{\pm0.008}$ & $0.625_{\pm0.004}$ & $0.120_{\pm0.015}$ & $0.721_{\pm0.001}$ & $0.223_{\pm0.002}$ \\
\quad $+$XGD             & $\mathbf{0.819}_{\pm0.005}$ & $\mathbf{0.627}_{\pm0.013}$ & $0.369_{\pm0.095}$ & $0.008_{\pm0.059}$ & $\mathbf{0.743}_{\pm0.003}$ & $\mathbf{0.396}_{\pm0.023}$ \\
Fisher (replay)          & $0.657_{\pm0.017}$ & $0.361_{\pm0.012}$ & $0.541_{\pm0.032}$ & $0.112_{\pm0.170}$ & $0.610_{\pm0.002}$ & $0.117_{\pm0.007}$ \\
\quad $+$XGD             & $0.749_{\pm0.007}$ & $0.554_{\pm0.020}$ & $0.655_{\pm0.030}$ & $0.169_{\pm0.240}$ & $0.673_{\pm0.005}$ & $0.237_{\pm0.074}$ \\
Localize-and-Stitch      & $0.662_{\pm0.015}$ & $0.487_{\pm0.005}$ & $0.487_{\pm0.077}$ & $-0.060_{\pm0.221}$ & $0.605_{\pm0.003}$ & $0.130_{\pm0.015}$ \\
\quad $+$XGD             & $0.710_{\pm0.023}$ & $0.517_{\pm0.010}$ & $0.652_{\pm0.017}$ & $0.100_{\pm0.026}$ & $0.653_{\pm0.001}$ & $0.206_{\pm0.016}$ \\
GradFix                  & $0.609_{\pm0.005}$ & $0.471_{\pm0.014}$ & $0.413_{\pm0.048}$ & $-0.007_{\pm0.012}$ & $0.594_{\pm0.005}$ & $0.079_{\pm0.007}$ \\
DOGE/APGD                & $0.700_{\pm0.006}$ & $0.394_{\pm0.022}$ & $0.363_{\pm0.002}$ & $-0.009_{\pm0.016}$ & $0.516_{\pm0.000}$ & $0.160_{\pm0.000}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\footnotesize
\setlength{\tabcolsep}{2.4pt}
\caption{\textbf{All three suites at $B{=}16$} (16 labels per task for every method; mean $\pm$ std over three fresh buffer draws). Each suite reports the plain mean of its tasks' own metrics and the worst task on the same scale (Section~\ref{sec:protocol}); expert ceilings are $0.903$ (CLIP-8), $0.860$ (GLUE-8) and $0.896$ (CLIP-20). A ``$+$XGD'' row refines that draw's own selected merge and is directly comparable to the merge row above it. Every method, merges included, selects its hyperparameters inside the budget by the same held-out-metric rule, so a merge row carries a draw spread wherever its selection varied. GD and ungated are XGD's ablations, run from the pretrained model where they isolate the anchor and the gate. The last block spends the same $B$ examples per task on constructing or reweighting the merge instead of on refinement (RegMean reads only the inputs). Same conventions as Table~\ref{tab:b32}; aTLAS$+$XGD was run at $B=32$ only.}
\label{tab:b16}
\begin{tabular}{lcccccc}
\toprule
 & \multicolumn{2}{c}{CLIP-8} & \multicolumn{2}{c}{GLUE-8} & \multicolumn{2}{c}{CLIP-20} \\
...
Pretrained $\thetab$     & $0.478$ & $0.315$ & $0.354$ & $-0.047$ & $0.550$ & $0.100$ \\
\midrule
Task arithmetic          & $0.685_{\pm0.011}$ & $0.503_{\pm0.026}$ & $0.552_{\pm0.000}$ & $0.130_{\pm0.000}$ & $0.602_{\pm0.004}$ & $0.117_{\pm0.004}$ \\
\quad $+$XGD             & $0.740_{\pm0.004}$ & $0.538_{\pm0.015}$ & $0.676_{\pm0.012}$ & $0.052_{\pm0.042}$ & $0.653_{\pm0.007}$ & $0.215_{\pm0.032}$ \\
Breadcrumbs              & $0.705_{\pm0.000}$ & $0.521_{\pm0.000}$ & $0.470_{\pm0.020}$ & $0.132_{\pm0.017}$ & $0.603_{\pm0.000}$ & $0.119_{\pm0.000}$ \\
\quad $+$XGD             & $0.756_{\pm0.006}$ & $0.560_{\pm0.013}$ & $0.667_{\pm0.005}$ & $0.114_{\pm0.037}$ & $0.658_{\pm0.005}$ & $0.225_{\pm0.037}$ \\
DARE-TIES                & $0.708_{\pm0.000}$ & $0.486_{\pm0.000}$ & $0.554_{\pm0.016}$ & $0.256_{\pm0.021}$ & $0.520_{\pm0.000}$ & $0.202_{\pm0.000}$ \\
\quad $+$XGD             & $0.766_{\pm0.001}$ & $0.521_{\pm0.003}$ & $0.684_{\pm0.003}$ & $0.181_{\pm0.013}$ & $0.662_{\pm0.006}$ & $0.286_{\pm0.022}$ \\
TIES                     & $0.734_{\pm0.007}$ & $0.512_{\pm0.014}$ & $0.549_{\pm0.000}$ & $0.234_{\pm0.000}$ & $0.612_{\pm0.013}$ & $0.201_{\pm0.013}$ \\
\quad $+$XGD             & $0.774_{\pm0.004}$ & $0.549_{\pm0.011}$ & $0.673_{\pm0.006}$ & $0.290_{\pm0.042}$ & $0.678_{\pm0.010}$ & $0.275_{\pm0.006}$ \\
\midrule
$\thetab+$GD             & $0.525_{\pm0.036}$ & $0.351_{\pm0.032}$ & $0.395_{\pm0.048}$ & $0.011_{\pm0.064}$ & $0.563_{\pm0.016}$ & $0.107_{\pm0.013}$ \\
$\thetab+$ungated        & $0.548_{\pm0.006}$ & $0.402_{\pm0.010}$ & $0.456_{\pm0.008}$ & $0.066_{\pm0.055}$ & $0.578_{\pm0.003}$ & $0.151_{\pm0.007}$ \\
$\thetab+$XGD            & $0.681_{\pm0.050}$ & $0.502_{\pm0.020}$ & $0.565_{\pm0.032}$ & $0.062_{\pm0.051}$ & $0.613_{\pm0.056}$ & $0.188_{\pm0.083}$ \\
\midrule
aTLAS                    & $0.715_{\pm0.014}$ & $0.562_{\pm0.032}$ & $0.609_{\pm0.024}$ & $\mathbf{0.301}_{\pm0.046}$ & $0.653_{\pm0.005}$ & $0.164_{\pm0.009}$ \\
\quad $+$XGD             & -- & -- & -- & -- & -- & -- \\
TATR                     & $0.719_{\pm0.001}$ & $0.525_{\pm0.012}$ & $0.568_{\pm0.007}$ & $0.217_{\pm0.025}$ & $0.607_{\pm0.005}$ & $0.136_{\pm0.013}$ \\
\quad $+$XGD             & $0.758_{\pm0.002}$ & $0.553_{\pm0.006}$ & $\mathbf{0.685}_{\pm0.005}$ & $0.103_{\pm0.026}$ & $0.654_{\pm0.012}$ & $0.217_{\pm0.066}$ \\
RegMean                  & $0.800_{\pm0.006}$ & $0.590_{\pm0.009}$ & $0.610_{\pm0.009}$ & $0.115_{\pm0.035}$ & $0.716_{\pm0.002}$ & $0.227_{\pm0.008}$ \\
\quad $+$XGD             & $\mathbf{0.801}_{\pm0.005}$ & $\mathbf{0.591}_{\pm0.010}$ & $0.424_{\pm0.136}$ & $0.024_{\pm0.033}$ & $\mathbf{0.727}_{\pm0.004}$ & $\mathbf{0.347}_{\pm0.013}$ \\
Fisher (replay)          & $0.658_{\pm0.016}$ & $0.401_{\pm0.077}$ & $0.509_{\pm0.025}$ & $0.021_{\pm0.032}$ & $0.607_{\pm0.010}$ & $0.113_{\pm0.005}$ \\
\quad $+$XGD             & $0.716_{\pm0.003}$ & $0.516_{\pm0.010}$ & $0.623_{\pm0.029}$ & $0.042_{\pm0.089}$ & $0.652_{\pm0.002}$ & $0.159_{\pm0.032}$ \\
Localize-and-Stitch      & $0.661_{\pm0.014}$ & $0.480_{\pm0.009}$ & $0.505_{\pm0.107}$ & $-0.063_{\pm0.209}$ & $0.584_{\pm0.013}$ & $0.134_{\pm0.011}$ \\
\quad $+$XGD             & $0.695_{\pm0.007}$ & $0.506_{\pm0.010}$ & $0.583_{\pm0.144}$ & $0.050_{\pm0.017}$ & $0.631_{\pm0.003}$ & $0.189_{\pm0.017}$ \\
GradFix                  & $0.571_{\pm0.019}$ & $0.433_{\pm0.006}$ & $0.376_{\pm0.014}$ & $0.000_{\pm0.000}$ & $0.588_{\pm0.005}$ & $0.078_{\pm0.015}$ \\
DOGE/APGD                & $0.700_{\pm0.006}$ & $0.394_{\pm0.022}$ & $0.364_{\pm0.004}$ & $-0.009_{\pm0.016}$ & $0.507_{\pm0.008}$ & $0.173_{\pm0.011}$ \\
\bottomrule
\end{tabular}
\end{table}

% ---------------------------------------------------------------------------
% RETIRED (2026-09-01): the 16+16 and 8+8 split-selection grids (tab:main16,
% tab:main8). Superseded by tab:b32 / tab:b16, which report all three suites
% under protocol v3 on fresh seeds. Their unique content -- DOGE/APGD,
% AdaMerging, RegMean, Fisher and Localize-and-Stitch, and the compositions on
% top of AdaMerging/RegMean -- has NOT been re-run under v3 and returns only
% when it has. DOGE additionally carries an open reproduction failure (see
% results/compare/grid_nn_*_apgd_{published,paperk}_n16.json).
% ---------------------------------------------------------------------------
\iffalse
\begin{table}[t]
\centering
...
Pretrained      & alone      & $0$ & $0$ & $0.550\ (.10)$ \\
                ...
                & $+$XGD     & $0.223^{\dagger}\ ({-}.33)$ & $0.295^{\dagger}\ ({-}.45)$ & $0.645\ (.22)$ \\
\midrule
Task arithmetic & alone      & $0.320\ ({-}.18)$ & $0.270\ ({-}.53)$ & $0.495\ (.15)$ \\
                & $+$XGD     & $0.498\ ({-}.18)$ & $0.462^{\dagger}\ ({-}.12)$ & $0.648\ (.22)$ \\
\midrule
Breadcrumbs     & alone      & $0.030\ ({-}2.14)$ & $0.431\ ({+}.08)$ & $0.602\ (.12)$ \\
                & $+$XGD     & $0.528\ ({-}.06)$ & $0.339^{\dagger}\ ({-}.46)$ & $0.652\ (.22)$ \\
\midrule
DARE-TIES       & alone      & $0.301\ ({-}.34)$ & $0.364\ ({-}.32)$ & $0.520\ (.20)$ \\
                & $+$XGD     & $\mathbf{0.576}\ ({+}.19)$ & $0.498^{\dagger}\ ({-}.31)$ & $0.649^{\dagger}\ (.27)$ \\
\midrule
TIES            & alone      & $0.208\ ({-}.22)$ & $0.525\ ({+}.21)$ & $0.619\ (.19)$ \\
                & $+$XGD     & $0.489^{\dagger}\ ({+}.01)$ & $0.566^{\dagger}\ ({-}.08)$ & $0.665\ (.28)$ \\
\midrule
DOGE/APGD       & alone      & $0.016\ ({+}.00)$ & $0.138\ ({-}1.93)$ & $0.516\ (.16)$ \\
\midrule
AdaMerging      & alone      & $0.028\ ({-}3.01)$ & $0.463\ ({-}.04)$ & $0.592\ (.10)$ \\
                & $+$XGD     & $0.543\ ({+}.02)$ & $0.554^{\dagger}\ ({+}.15)$ & $0.640\ (.19)$ \\
\midrule
RegMean         & alone      & $0.477\ ({+}.06)$ & $0.627\ ({+}.32)$ & $0.697\ (.21)$ \\
                & $+$XGD     & $0.517\ ({+}.10)$ & $\mathbf{0.632}^{\dagger}\ ({+}.31)$ & $0.709\ (.24)$ \\
\bottomrule
\end{tabular}
\end{table}
\fi
...
\paragraph{Refining from the pretrained model.}
Tables~\ref{tab:b32} and~\ref{tab:b16} report all three suites under the protocol of Section~\ref{sec:protocol}: every method given the same $B$ labels per task, every hyperparameter selected inside that budget by one rule, and every number a mean over three fresh buffer draws. With no merge to inherit, XGD is the strongest cold-start treatment on all three suites at $B=32$---$0.751$ on CLIP-8 against $0.732$ for the best checkpoint-only merge, $0.612$ on GLUE-8 against $0.563$, and $0.663$ on CLIP-20 against $0.619$---using $B$ labels per task in place of the eight or twenty expert checkpoints a merge requires. Neither ablation is viable at this budget: ordinary descent and the ungated update reach $0.546$/$0.579$ on CLIP-8, $0.521$/$0.480$ on GLUE-8 and $0.573$/$0.602$ on CLIP-20, so the anchored, capped step is worth $0.06$--$0.21$ over unconstrained descent from the same initialization, and the anchor without the gate does not account for it. The worst-task columns qualify the GLUE-8 result: XGD's mean there is bought partly from its weakest task ($0.133$ against DARE-TIES's $0.268$), and every selected cell sits in the high-displacement part of the grid.

At $B=16$ the cold start is less stable. XGD still leads on GLUE-8 ($0.565$ against $0.554$) and ties TIES on CLIP-20 ($0.613$ against $0.612$), but on CLIP-8 it falls to $0.681\pm0.050$ against TIES's $0.734$---the one cell of either table in which a checkpoint-only merge beats it. The spread is selection rather than the update: two draws select $\eta\in\{4,8\}$ and score $0.63$--$0.67$ while the third selects $\eta=32$ and scores $0.73$, and pinning $\eta=32$ on the two draws that rejected it scores $0.725$ and $0.732$. Cells are constructed on $B/2=8$ examples per task, where the larger rate genuinely collapses on some draws, and refit on $16$, where it is safe; selection is right about the models it can see and conservative about the model it will build (Appendix~\ref{app:selection}). At $B=32$ on the same suite XGD varies by $\pm0.004$ and selects the same interior cell on every draw. An earlier version of that experiment showed a different instability---a $\pm0.081$ spread with the fine-grained Cars task collapsing on two draws of three---which we traced to the replay sampler: with more classes than the budget holds it had kept the lowest class ids deterministically, so Cars was refined and selected on $32$ of its $196$ classes, identically for every seed. Drawing the covered classes at random per seed (Appendix~\ref{app:protocol}) removes both the collapse and the spread. We report this because the failure is instructive: at these budgets a fine-grained task is representable only in part, and a selection buffer that inherits the same blind spot cannot detect damage to the classes it never sees.

\paragraph{Refining a merge.}
The same rule, unchanged, applies from any constructed initialization, and each ``$+$XGD'' row of Tables~\ref{tab:b32} and~\ref{tab:b16} refines the row above it. Across the fifty-one composition cells run (eight initializations at both budgets, aTLAS at $B=32$), every cell improves its initialization on every draw except RegMean's on GLUE-8, discussed below. The gains run from ${+}0.02$ on top of already-optimized starts (aTLAS, RegMean on the CLIP suites) to ${+}0.24$ from the weakest ones---$0.17$ from DARE-TIES on CLIP-20, $0.19$--$0.21$ from Breadcrumbs on GLUE-8 at $B=16$---and the best cell of every suite is a composition: RegMean$+$XGD at $0.819$ on CLIP-8 and $0.743$ on CLIP-20, TATR$+$XGD at $0.701$ on GLUE-8. Refinement compresses the spread between starts, so which construction one begins from matters less once the labels are spent: CLIP-20's four checkpoint merges span $0.520$--$0.619$ before refinement and $0.664$--$0.695$ after, GLUE-8's at $B=16$ span $0.470$--$0.554$ before and $0.667$--$0.684$ after. Composition is also the more stable setting at the smaller budget---on CLIP-8 at $B=16$ the cold start varies by $\pm0.050$ across draws while the checkpoint-merge compositions vary by $\pm0.001$--$0.006$---and the selected rates from a constructed start are an order of magnitude smaller than from the pretrained model, which is what a start that already carries multi-task signal should require. Worst-task behaviour still splits by initialization on GLUE-8: TIES$+$XGD raises the weakest task from $0.234$ to $0.363$, while task arithmetic$+$XGD lowers it from $0.130$ to $0.059$. The ablation arms are reported only from the pretrained model, where they isolate the anchor and the gate; from a constructed start the comparison of interest is against the start itself, which every paired row makes directly.

RegMean is the one exception, and it marks a real boundary. RegMean$+$XGD gains on the CLIP suites ($0.810\to0.819$ and $0.721\to0.743$ at $B=32$) but regresses on GLUE-8 at both budgets---$0.625\to0.369$ and $0.610\to0.424$, losing up to $0.32$ on a single draw---and on one CLIP-8 draw at $B=16$. Every initialization built in task-vector space is improved on every draw; the one built by a closed-form fit to input statistics is the one refinement can destroy, and it does so at unremarkable selected rates ($\eta\in\{1,2,4\}$), so the damage is not a visible selection outlier. We report the boundary as measured and leave its diagnosis open.

\paragraph{Other uses of the same budget.}
Seven baselines spend the same $B$ examples per task on constructing or reweighting the merge rather than on moving coordinates. Four are not competitive in any cell. Fisher merging and Localize-and-Stitch fall below the best checkpoint-only merge everywhere, and the latter is the least stable estimator in the study, with its GLUE-8 worst task below the pretrained model. GradFix \citep{rinaldi2025gradfix}, which zeroes each task vector where its sign disagrees with the budget's gradient at $\thetab$, lands $0.03$--$0.14$ below plain task arithmetic in all six cells even with its coefficient grid centred on the released $\alpha/T$ range; the mask removes what would not transfer in its cross-base setting, but within one base it removes signal. DOGE/APGD \citep{wei2025apgd} reaches $0.700$ on CLIP-8 with an interior scale but stays at the degenerate floor on GLUE-8 and below the zero-shot floor on CLIP-20, and carries a reproduction caveat: at the authors' published setting, with no budget involved, our implementation reaches $69.3$ on CLIP-8 against their reported $80.7$, while the same pipeline reproduces task arithmetic ($68.5$ against $69.1$) and TIES ($73.2$ against $72.4$) to within a point---so these rows are lower bounds on the method, not statements about its achievable performance. TATR \citep{sun2025tatr} improves plain task arithmetic in all six cells and is the best checkpoint-tier merge on GLUE-8 at $B=16$.

The two strong constructions are RegMean and aTLAS \citep{zhang2024atlas}, and they press the paper's claim from opposite sides. RegMean, fitting linear-layer Gram statistics on the budget's inputs alone---it reads no labels---reaches $0.810$/$0.800$ on CLIP-8 and $0.721$/$0.716$ on CLIP-20, above every other cell of its columns except its own refinement, with the tightest spreads we measure; on GLUE-8, where each task keeps its own trained head and no closed form applies, it stays $0.07$ below the best composition. aTLAS, which learns a coefficient per task and parameter block by supervised descent on exactly the labels XGD uses, beats every checkpoint-only merge in all six cells and is the strongest labeled alternative: it ties the cold start on GLUE-8 at $B=32$ and beats it outright in three cells of six (GLUE-8 at $B=16$, CLIP-20 at both budgets), with consistently better worst tasks. Spent on coefficients rather than coordinates, the same labels are at least as good a cold start; the refinement's remaining edge is what it adds on top of a constructed start---including aTLAS's own (${+}0.02$--$0.06$, previous paragraph)---where coefficients alone have converged.

\paragraph{Twenty tasks.}
\label{sec:clip20}
CLIP-20 is the high-interference regime the protocol was meant to probe. With twenty task vectors competing for one encoder, the best checkpoint-only merge recovers about a fifth of the floor-to-ceiling gap ($0.619$, against a $0.550$ zero-shot floor and a $0.896$ expert ceiling), where at eight tasks TIES recovers roughly half; Appendix~\ref{app:clip20} traces this to the geometry of the twenty-expert compromise. The selected task-arithmetic coefficient falls accordingly, to $\lambda\in\{0.05,0.08\}$ against $\{0.2,0.3\}$ at eight tasks. Refinement is correspondingly more useful there: the cold start clears the best merge by $0.044$, and every composition improves its own initialization by $0.04$--$0.18$. Pinning $S\in\{20,100\}$ at the CLIP-20 composition cells confirms that the fixed horizon is not what limits them (Appendix~\ref{app:selection}).
...
\caption{Task performance versus distance from the pretrained model $\thetab$. Circles, triangles, and diamonds show split-selected XGD, GD, and ungated refinement from $\thetab$; squares show the selected merge baselines. The horizontal dotted line marks pretrained performance, and distance is shown on a log scale.}
\label{fig:n16disp}
\end{figure}

\paragraph{Drift on the pretraining objective.}
Figure~\ref{fig:n16disp} shows the refinement reaching merge-level scores at a small fraction of the merge's displacement, but parameter distance is a geometric quantity, not a capability measure. We therefore measure drift on the pretraining objective itself: masked-language-model loss on WikiText-2 validation text. Each refined encoder is evaluated under RoBERTa's original pretrained MLM head; the head is never modified by any experiment, and the WikiText-2 text is never used for refinement or selection, so any increase in this loss over the pretrained model is attributable to encoder drift alone. The probed states are exactly the from-pretrained cells of Table~\ref{tab:b32} at $B=32$---the saved winner of each of the three fresh draws, nothing re-derived---with the pretrained model and the merges at their selected hyperparameters as references (Table~\ref{tab:drift}).

With the selected merges probed alongside (Table~\ref{tab:drift}), the picture parallels the held-out-task study below. XGD is the only point that combines the best task score with sub-merge drift: at $0.612$ it exceeds every merge (DARE-TIES $0.563$, task arithmetic $0.552$) while adding $+0.93$ to the pretraining loss against their $+1.59$ and $+2.51$---a better multitask model for $37\%$ of task arithmetic's damage, at roughly $3\%$ of its parameter displacement---and it dominates Breadcrumbs outright (the same drift at $0.14$ more score). The exception is TIES, which drifts least of every merge ($+0.44$) at $0.063$ less score, so the frontier consists of $\thetab$, TIES and XGD alone. The ablations locate the drift protection: ordinary descent barely moves ($0.40$ from $\thetab$) and still drifts more than XGD, while the ungated update, which moves less far than XGD on average, adds $+12.3$ to the loss on one draw of three---distance scaling without the gate can leave the encoder while barely displacing it. Refinement from the pretrained model is thus not the minimum-drift point, but it is the only one at the high-score end that costs the base model less than half of what a merge at that level does.

\begin{table}[t]
\centering
\small
\caption{Pretraining-objective drift on GLUE-8 at the $B{=}32$ budget of Table~\ref{tab:b32}. MLM loss is measured on WikiText-2 validation with RoBERTa's pretrained MLM head; $\Delta$ is relative to $\thetab$. Every probed state is the model Table~\ref{tab:b32} scores (the saved winner of each of the three fresh draws, and the merges at their selected hyperparameters), so the GLUE-8 column repeats that table's mean of per-task metrics; mean $\pm$ std over draws. $^\ast$ marks the score--drift Pareto frontier.}
\label{tab:drift}
\begin{tabular}{lccc}
\toprule
...
\midrule
XGD from $\thetab$$^\ast$                & $3.54\pm1.28$ & $\mathbf{0.612}\pm0.015$ & $3.44\pm0.31\ (+0.93)$ \\
...
XGD from base$^\ast$           & $1.05\pm0.02$ & $\mathbf{0.751}\pm0.004$ & $0.721\pm0.009\ (-0.059)$ \\
...
\midrule
TIES, $S{=}5$        & XGD      & $0.697\pm0.006$ & $0.261\pm0.024$ \\
                     & ungated  & $0.699\pm0.003$ & $0.276\pm0.019$ \\
AdaMerging, $S{=}5$  & XGD      & $0.703\pm0.003$ & $0.177\pm0.008$ \\
                     & ungated  & $0.702\pm0.004$ & $0.146\pm0.008$ \\
\midrule
TIES, $S{=}30$       & XGD      & $0.708\pm0.005$ & $0.369\pm0.013$ \\
AdaMerging, $S{=}30$ & XGD      & $0.712\pm0.003$ & $0.321\pm0.016$ \\
RegMean, $S{=}30$    & XGD      & $0.764\pm0.002$ & $0.501\pm0.011$ \\
\midrule
Base, $n{=}64$       & XGD      & $0.686\pm0.004$ & $0.340\pm0.017$ \\
                     & GD       & $0.523\pm0.187$ & $0.142\pm0.072$ \\
Base, $n{=}8$        & XGD      & $0.626\pm0.005$ & $0.170\pm0.017$ \\
...
XGD from base, $\eta{=}4$$^\ast$      & $0.44$ & $0.618$ & $0.790\ (-0.012)$ \\
XGD from base, $\eta{=}8$$^\ast$      & $0.60$ & $0.637$ & $0.780\ (-0.022)$ \\
Task arithmetic ($\lambda{=}0.08$)    & $0.87$ & $0.536$ & $0.781\ (-0.021)$ \\
XGD from task arith., $\eta{=}4$      & $1.06$ & $0.654$ & $0.772\ (-0.030)$ \\
Breadcrumbs$^\ast$                    & $1.28$ & $0.536$ & $0.792\ (-0.010)$ \\
XGD from AdaMerging, $\eta{=}4$$^\ast$ & $1.44$ & $\mathbf{0.682}$ & $0.780\ (-0.022)$ \\
...
The multi-head GLUE-8 benchmark (CoLA, SST-2, MRPC, STS-B, QQP, MNLI, QNLI, RTE, excluding WNLI as is common) \citep{wang2018glue} uses RoBERTa-base as the encoder backbone \citep{liu2019roberta}: the shared encoder is merged and evaluated with each task's own trained head. 
...
columns of the retired $8{+}8$ split-selection grid. Each row is the cell selected using the
...
\caption{GLUE-8 at the split-selected $8{+}8$ budget: raw per-task evaluation scores for every nonempty GLUE-8 row of the $8{+}8$ split-selection grid. The single-task-expert row evaluates each task's own expert and is included as a ceiling reference.}
...
\setlength{\tabcolsep}{2.5pt}
\renewcommand{\arraystretch}{0.92}
\caption{CLIP-8 at the split-selected $8{+}8$ budget: per-task top-1 accuracy for every nonempty CLIP-8 row of the $8{+}8$ split-selection grid. The single-task-expert row evaluates each task's own expert and is included as a ceiling reference.}
...
\textbf{XGD (gated, proposed)} & $\mathbf{0.840\pm0.021}$ & $\mathbf{0.849}$ (stable) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{GLUE-8 (all eight tasks, RoBERTa-base, single seed). Best mean normalized retention per family (worst-task retention in parentheses); the task-arithmetic merge baseline is $0.320$ (worst $-0.175$). XGD is reported at its largest stable learning rate, $\eta=16$; the $\eta=32$ configuration reached $0.614$ once but is not reproducible (see text).}
\label{tab:glue8}
\begin{tabular}{lcc}
\toprule
...
\textbf{XGD (gated, proposed)} & $0.595\ (0.11)$ & $\mathbf{0.617}\ (0.05)$ \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{XGD (gated, proposed)} & $\mathbf{0.598}$ & $\mathbf{+0.159}$ \\
\midrule
...
\caption{Per-task top-1 accuracy on the eight-task CLIP suite: zero-shot backbone, task expert, task-arithmetic merge, and the proposed gated refinement (XGD, $\eta=8$). Refinement recovers most of the merge's interference loss, including on Cars and SUN397, which the merge had degraded below the zero-shot backbone.}
\label{tab:clip8pt}
...
\textbf{XGD} & $\mathbf{0.650}$ & $\mathbf{0.648}$ & $\mathbf{0.769}$ & $\mathbf{0.923}$ & $\mathbf{0.874}$ & $\mathbf{0.851}$ & $0.961$ & $\mathbf{0.578}$ & $\mathbf{0.782}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Replay-budget study on the eight-task CLIP suite (mean normalized retention; worst-task in parentheses). Best configuration per family; the gated update (XGD) near-converges by $S=5\text{--}10$ sweeps, while ordinary replay GD is given a longer horizon ($S=25\text{--}35$) as a generous baseline. XGD holds an ${\approx}\,+0.10$ mean advantage at every budget and is the only method with non-negative worst-task retention down to $n=16$.}
\label{tab:clipbudget}
\begin{tabular}{lcc}
\toprule
Replay budget & \textbf{XGD (ours)} & Ordinary replay GD \\
\midrule
...
\midrule
XGD, $S{=}5$ (baseline)        & $0.598\ (0.16)$ & $0.551\ (0.02)$ & $0.542\pm.062$ & $0.554\pm.068$ \\
XGD, $S{=}30$, fixed, constant & $\mathbf{0.616}\ (0.13)$ & $0.570\ (0.12)$ & --- & --- \\
XGD, $S{=}30$, anneal          & $0.604\ (0.13)$ & $\mathbf{0.578}\ (0.12)$ & $\mathbf{0.641\pm.028}$ & $\mathbf{0.596\pm.032}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{lccc}
\toprule
Initialization & Merge alone & $+$\,ungated & $+$\,XGD \\
\midrule
...
Table~\ref{tab:clip20matrix} runs XGD and the ungated anchored update from five initializations (tuned task arithmetic, TIES, DARE-TIES, Breadcrumbs, AdaMerging-layer) plus RegMean, each with its learning rate tuned per initialization. Two findings. (i)~XGD and the ungated anchored update are within $\pm 0.01$ everywhere---confirmed over five replay seeds ($-0.002\pm0.006$ from TIES, $+0.001\pm0.005$ from AdaMerging): at twenty tasks the gate is neutral on this modality too, and the anchored, distance-scaled, clipped step is the load-bearing ingredient. The attribution \emph{sign} still carries signal---the inverted-gate control collapses to $0.100$, far below the zero-shot floor---but the sign gate adds nothing over no gate. (ii)~Refinement improves every initialization, and better initializations retain their lead through refinement: the ordering ada $>$ ties $\approx$ dareties $>$ ta $\approx$ bc is preserved.
\fi
...
Base model (\,$0.550\ (0.100)$\,) & $+$\,GD  & $0.573\ (0.093)$ & $0.580\ (0.113)$ & $0.602\ (0.160)$ \\
                                  & $+$\,XGD & $\mathbf{0.632}\ (0.187)$ & $\mathbf{0.640}\ (0.205)$ & $\mathbf{0.654}\ (0.236)$ \\
\midrule
Task arith.\ (\,$0.604\ (0.115)$\,) & $+$\,XGD & $0.650\ (0.197)$ & $0.659\ (0.242)$ & $0.674\ (0.274)$ \\
TIES (\,$0.626\ (0.150)$\,)         & $+$\,XGD & $0.665\ (0.224)$ & $0.673\ (0.250)$ & $0.691\ (0.284)$ \\
AdaMerging (\,$0.665\ (0.101)$\,)   & $+$\,XGD & $0.683\ (0.207)$ & $0.688\ (0.249)$ & $0.698\ (0.241)$ \\
\bottomrule
\end{tabular}
\end{table}
\end{comment}
