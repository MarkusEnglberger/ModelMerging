...
Families (all sequential, same data, same #gradient evals):
  ...
  apr         : -g|v|, AP-gated, clip to +/-|v|     (proposed)
  nogate      : -g|v|, ungated, clip to +/-|v|      (isolates the gate)
"""
...
def family_configs(steps):
    ...
    for lr in [1, 2, 4, 8, 16, 32, 64]:
        # clip AFTER lr (vdist): the step is bounded by |v|; clipping fires when
        # lr*|g| > 1, i.e. when the step would overshoot the expert.
...
def main():
    ...
    fams = family_configs(args.steps)
        ...
        for seed in args.seeds:
            ...
            for fam, cfgs in fams.items():
                ...
                for lr, rc in cfgs:
                    ...
                    clip_g = sum(h["clipped_frac_gated"] for h in hist) / len(hist)
                    cand = {"mean": ag["mean_normret"], "worst": ag["worst_normret"],
                            "lr": lr, "disp": disp, "clipped_frac_gated": clip_g}
                ...
                _log(f"[n={n} seed={seed}] {fam}: best mean={best['mean']:.3f} "
                     ...
                     f"clip%gated={100*best['clipped_frac_gated']:.3f} @lr={best['lr']:g}")
    ...
    print(f"Best-of-lr per family, mean +/- std over {len(args.seeds)} seeds "
          f"(steps={args.steps}); merge baseline mean={merge_mean:.3f}")
    ...
    for n in args.n_probes:
        ...
        for fam in ["ordinary_gd", "apr", "nogate"]:
            ...
            clips = [b["clipped_frac_gated"] for b in results[n][fam]]
            mu = st.mean(means)
