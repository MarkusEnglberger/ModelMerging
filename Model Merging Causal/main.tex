...
On CLIP-8 APR improves \emph{every} merge initialization it is given, and the best results always come from the strongest initializations, never plain task arithmetic: it lifts AdaMerging from $0.610$ to $0.722$, RegMean from $0.733$ to $0.762$, and the calibrated merge from $0.737$ to $\mathbf{0.825}$ mean / $\mathbf{+0.543}$ worst-task. On GLUE-8 the same pattern holds with one instructive exception analysed below: APR lifts DARE-TIES from $0.301$ to $0.610$ and the calibrated merge from $0.580$ to $0.662$, but \emph{harms} the RegMean initialization at every learning rate. The refinement is complementary to good merging, not an alternative---and the two-stage calibrate-then-refine pipeline consumes only the $64$ labeled training examples per task, never touching the evaluation split.

\paragraph{Error bars, and a single-seed conclusion retracted.}
Replicating the headline configurations over five replay-buffer seeds (Table~\ref{tab:multiseed}) mostly confirms the single-seed picture, with one exception we retract explicitly. In a single-seed sweep, layer-wise AdaMerging ($0.610$) appeared to edge out APR from the tuned task-arithmetic merge ($0.588$) on CLIP-8 despite using no labels; at five seeds the ordering is statistically indistinguishable and nominally \emph{reversed} (APR $0.605\pm0.008$ vs.\ AdaMerging $0.595\pm0.037$), so we treat the two as tied. Two findings survive. First, AdaMerging is remarkably data-efficient: restricting it from its standard transductive protocol to exactly the $64$ unlabeled inputs per task that APR sees labeled does not degrade it at all ($0.604\to0.610$ mean; $+0.165\to+0.219$ worst). Second, that a label-free method \emph{ties} a labeled one from the same initialization demands explanation---provided next, and converted by the calibrate-then-refine experiment into an improvement of our own method.
...
\label{par:calibrate}
The two observations above---the dominant recoverable error is per-tensor mixing, and a labeled objective cannot be confidently wrong---combine into the two-stage instantiation of the method formalized in Section~\ref{sec:calibrate}, using nothing beyond the existing replay buffer. \emph{Calibrate}: fit the per-tensor coefficients $\lambda_i^{(b)}$ of Equation~\eqref{eq:calibrated-merge} by minimizing the labeled replay loss (Equation~\eqref{eq:calibrate-objective}; the supervised twin of AdaMerging), with an $\ell_2$ pull toward the initial coefficient and early stopping on a held-out quarter of the buffer. \emph{Refine}: run Algorithm~\ref{alg:expert-gated-refinement} from the calibrated merge $\theta^{(0)}_{\mathrm{cal}}$. Calibration fixes the tensor-mixing error; refinement fixes the residual coordinate-level interference.

On CLIP-8 the result is the strongest configuration in this study, fully inductively: the calibrated merge alone reaches $0.737$---already ahead of every other merge, labeled or not---and refinement lifts it to $\mathbf{0.825}$ mean / $\mathbf{+0.543}$ worst-task (an interior learning-rate peak). On GLUE-8, where AdaMerging's entropy objective collapses ($0.173$), the \emph{labeled} calibration works robustly ($0.585\pm0.035$ over five seeds), but the fixed-rate refinement from it does not: a favorable seed reaches $0.662$, yet across seeds the composition is unstable ($0.480\pm0.153$; the long-horizon control below gives the diagnosis and predicted fix). Two anticipated failure modes did not materialize: $\sim\!10^3$ coefficients fit on $64$ labeled examples per task did not overfit (the lightest regularization won, early stopping doing the rest), and the calibration objective transferred across modalities because correctness and confidence coincide under labels. Calibrate-then-refine is the recommended form where the gate carries signal (CLIP)---the same $64$ labels spent twice, first on the mixing error in a $10^3$-dimensional space, then on the interference error in the full parameter space; on GLUE the current best practice is the annealed single-stage refinement (Table~\ref{tab:longapr}).

\paragraph{Is the two-stage advantage an optimization-budget artifact? (preliminary control)}
A natural objection is that single-stage APR was under-optimized: $S=5$ sweeps, fixed order, constant rate---perhaps more sweeps, order randomization, and a decaying step would close the gap without any calibration stage. We test this directly (Table~\ref{tab:longapr}): $S=30$ with re-randomized task order and cosine decay to $5\%$ of the peak rate, at two peaks, plus a steps-only control ($S=30$, fixed order, constant rate), at $n=64$ and $n=32$ (single seed). On CLIP-8 the answer is unambiguous: the whole schedule package buys $+0.006$ to $+0.027$ over the $S=5$ baseline, never exceeds $0.616$, and does not reliably beat the steps-only control, while calibrate-then-refine reaches $0.825$ at $n=64$ and $0.782$ at $n=32$ (where calibration fits on $24$ examples per task)---a structural gap no schedule in the full parameter space reproduces. On GLUE-8 the control overturns the single-seed picture, and we state the corrected result plainly: over five replay seeds the annealed long refinement is the \emph{best and most stable} GLUE method at both budgets ($0.641\pm0.028$ at $n=64$; $0.596\pm0.032$ at $n=32$), while the two-stage method at its fixed rate $\eta=16$ is \emph{unstable}---$0.480\pm0.153$ at $n=64$ (per-seed $0.662/0.536/0.340/0.302/0.559$; MRPC collapses on bad seeds) and $0.450\pm0.140$ at $n=32$. The $0.662$ reported for GLUE-8 two-stage elsewhere is thus a favorable seed, flagged as such where it appears. The instability has the same signature as the TIES and RegMean cases: calibration \emph{down-weights} the large-data tasks (mean coefficients $0.19$ QQP, $0.22$ QNLI), redistributing expert distances, so a rate tuned on the task-arithmetic geometry sits at the edge of stability---and on GLUE no informative gate cushions the error; a per-initialization retuned or annealed rate is the predicted fix, left to future work. Fair summary: the calibration stage is not replaceable by optimization schedules where the gate's signal is strong (CLIP); where it is uninformative (GLUE), the annealed single-stage refinement is currently the most robust choice.
...
\centering
\small
\caption{Shared-output T5 track: flan-T5-base text-to-text GLUE-8 (single seed, $n=64$, $S=5$, $\gamma=1$, $\lambda_i=0.3$). The whole sequence-to-sequence model including the LM head is merged; every task decodes through the same head, so no oracle task identity is assumed. Mean normalized retention (worst-task in parentheses); best configuration per family. STS-B retention is measured against a zero base (see text). The calibrated rows are replay-selected end to end (fully inductive); the evaluation-selected calibration reaches $0.539$. Nearly every refinement family's optimum sits at the \emph{top} of its learning-rate grid (the grids were inherited from the RoBERTa study and shifted down one notch, which turned out too conservative for T5), so the refinement values are lower bounds; see text.}
\label{tab:t5}
\begin{tabular}{lc}
\toprule
Method & mean (worst) \\
...
Calibrated merge (per-tensor, labeled)    & $0.519\ (+0.068)$ \\
APR $\leftarrow$ calibrated merge         & $0.664\ (+0.019)$ \\
\midrule
...
\paragraph{Calibrate-then-refine transfers, fully inductively.}
The two-stage form also carries over. Layer-wise calibration on the replay buffer improves the merge from $0.383$ to $0.519$ (replay-selected; $0.539$ evaluation-selected), while the task-wise variant collapses ($\le 0.26$, worst $-0.95$)---the ``which tensors to re-mix, not which tasks to weight'' diagnosis now holds on a third setting. Refining from the calibrated merge reaches $0.664$ with \emph{both} stages selected on the replay objective alone (the replay-selected and evaluation-selected refinement rates coincide), making it the strongest fully inductive configuration on this track; unlike GLUE-8, the composition improved monotonically over the whole refinement-rate grid rather than destabilizing, though the grid-edge caveat below tempers this. On this track AdaMerging is not run: its entropy objective is written for a single classification distribution per example and needs a sequence-level adaptation, so the calibrated merge is the only per-tensor data-dependent initialization here.

\paragraph{Caveats.}
Three limitations are explicit. The track is single-seed. Nearly every refinement family's tuned optimum lies at the top of its learning-rate grid ($\eta=8$ ungated, $\eta=32$ for the compositions, $\eta=64$ for the two-stage rows), so the labeled-tier values are lower bounds and the grids need re-centering upward before these numbers are final. And the far edge already shows the familiar warning sign: extending APR from task arithmetic to $\eta=64$ raises mean retention ($0.537\to0.557$) while flipping worst-task retention negative ($+0.024\to-0.103$)---the same edge-of-stability signature documented for GLUE-8 at $\eta=32$, so chasing the unbracketed optima will require the same interior-of-the-stable-range discipline used there.

\subsection{Scaling to twenty tasks: the high-interference regime}
\label{sec:clip20}

We next scale the vision track to the twenty-task suite of \citet{wang2024tall}, using the published fine-tuned ViT-B/32 encoders for all twenty tasks (every expert was validated to beat its zero-shot floor by a wide margin, including the three tasks whose class-name orderings we had to reconstruct: PCAM $0.556\!\to\!0.887$, FER2013 $0.376\!\to\!0.711$, RenderedSST2 $0.586\!\to\!0.713$). Unless noted, all results in this subsection are single-seed with $n=64$ replay examples per task and hyperparameters selected on the evaluation split, symmetrically for every method; a stricter disjoint-holdout selection protocol, piloted at the end of this section, changes the numbers negligibly.
...
At eight tasks, ordinary replay GD had only ever been compared against the refinement from the task-arithmetic initialization. Table~\ref{tab:clip20matrix} closes that gap: from five initializations (tuned task arithmetic, TIES, DARE-TIES, Breadcrumbs, AdaMerging-layer) plus RegMean, we run APR, the ungated anchored update, and ordinary replay GD with a deliberately wide GD grid (six learning rates spanning three decades; the GD optimum is an interior point of the grid at every initialization except AdaMerging). Three findings. (i)~At matched budget ($S=5$), APR beats ordinary GD from \emph{every} initialization ($+0.021$ to $+0.033$ mean accuracy). (ii)~APR and the ungated anchored update are within $\pm 0.01$ everywhere: at twenty tasks the gate is neutral on this modality too, and the anchored, distance-scaled, clipped step is the load-bearing ingredient. The attribution \emph{sign} still carries signal---the inverted-gate control collapses to $0.100$, far below the zero-shot floor---but thresholding at zero adds nothing over no gate, consistent with the GLUE analysis. (iii)~Refinement improves every initialization, and better initializations retain their lead through refinement: the ordering ada $>$ ties $\approx$ dareties $>$ ta $\approx$ bc is preserved.

\begin{table}[t]
\centering
\small
\setlength{\tabcolsep}{4pt}
\caption{Twenty-task CLIP suite: initialization $\times$ method at matched budget ($n=64$, $S=5$, single seed). Mean top-1 accuracy over the twenty tasks (worst-task in parentheses); zero-shot floor $0.550$, expert ceiling $0.896$. Best per family by mean accuracy; each family's learning rate is tuned per initialization. APR $>$ ordinary GD from every initialization; APR $\approx$ ungated anchored everywhere. $^\dagger$GD from AdaMerging peaked at the low edge of its grid (its true optimum may be lower; the value is a lower bound); the ungated update from RegMean also peaked at its low edge. RegMean row from the matched $n=64$ Gram budget.}
\label{tab:clip20matrix}
\begin{tabular}{lcccc}
\toprule
Initialization & Merge alone & $+$\,GD & $+$\,ungated & $+$\,APR \\
\midrule
Task arithmetic ($\lambda=0.08$) & $0.604\ (0.115)$ & $0.656\ (0.141)$ & $0.670\ (0.203)$ & $\mathbf{0.680}\ (0.221)$ \\
TIES                             & $0.626\ (0.150)$ & $0.677\ (0.184)$ & $\mathbf{0.701}\ (0.277)$ & $0.697\ (0.262)$ \\
DARE-TIES                        & $0.617\ (0.149)$ & $0.672\ (0.198)$ & $\mathbf{0.697}\ (0.277)$ & $0.695\ (0.254)$ \\
Breadcrumbs                      & $0.611\ (0.111)$ & $0.656\ (0.138)$ & $0.669\ (0.168)$ & $\mathbf{0.678}\ (0.177)$ \\
AdaMerging-layer                 & $0.665\ (0.101)$ & $0.674^\dagger\ (0.098)$ & $0.701\ (0.140)$ & $\mathbf{0.707}\ (0.188)$ \\
RegMean                          & $0.723\ (0.234)$ & $0.757\ (0.481)$ & $0.756\ (0.520)$ & $0.749\ (0.486)$ \\
\midrule
Calibrated merge                 & $0.751\ (0.551)$ & --- & --- & $\mathbf{0.786}\ (\mathbf{0.594})$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Longer horizons: APR leads at six of seven initializations under symmetric budgets.}
Ordinary GD given six times the budget ($S=30$, constant and cosine-annealed) closes most of its matched-budget deficit, and at the interim stage---before APR's own long-horizon cells were run---it appeared to reach parity or briefly overtake (from TIES, $0.706$ vs.\ APR's $S=5$ $0.697$). But the comparison must be budget-symmetric, and APR was \emph{not} converged at $S=5$ on this suite (unlike at eight tasks). With APR also run to $S=30$ (constant and cosine/random-order), the best-over-any-budget picture (Table~\ref{tab:clip20budget}) is that APR leads ordinary GD at \emph{six of the seven} starting points---base, task arithmetic, Breadcrumbs, DARE-TIES, TIES, and AdaMerging ($+0.002$ to $+0.058$ mean accuracy)---and trails only from RegMean ($-0.004$, within single-seed noise), the one initialization whose anchor geometry is corrupted (discussed below). The long horizon also transforms APR's \emph{worst-task} accuracy: from every checkpoint-space initialization it reaches $0.31$--$0.39$, against $0.17$--$0.38$ for GD, reviving the fragile-task-recovery property that the $S=5$ snapshot had lost at this scale. A further robustness point favors APR: long-horizon \emph{constant}-rate GD is unreliable---at its optimal learning rate it ended below its own $S=5$ result at two of five checkpoint initializations (from Breadcrumbs, below the merge itself), a failure cosine annealing repaired in every case---whereas APR reaches its peak without schedule tuning.

\begin{table}[t]
\centering
\small
\setlength{\tabcolsep}{5pt}
\caption{Twenty-task CLIP suite: best result over \emph{any} refinement budget (learning rate, sweeps $S\in\{5,30,60\}$, and constant/cosine schedule tuned per family), mean top-1 accuracy with worst-task in parentheses; zero-shot floor $0.550$, expert ceiling $0.896$; single seed. Initializations ordered by their own accuracy. Bold marks the better refinement per row: APR leads at six of seven starting points, trailing GD only from RegMean (whose anchor vectors are corrupted; see text). The gain from refinement shrinks monotonically as the initialization improves ($+0.13$ from the base model to $+0.04$ from the calibrated merge), yet remains positive throughout and at least doubles worst-task accuracy at every initialization. The calibrated merge has no GD arm (it is the two-stage method's own first stage).}
\label{tab:clip20budget}
\begin{tabular}{lccc}
\toprule
Initialization & Init.\ alone & $+$\,GD (best) & $+$\,APR (best) \\
\midrule
Base model $\theta_0$              & $0.550\ (0.100)$ & $0.625\ (0.207)$ & $\mathbf{0.683}\ (0.350)$ \\
Task arithmetic ($\lambda{=}0.08$) & $0.604\ (0.115)$ & $0.674\ (0.205)$ & $\mathbf{0.693}\ (0.351)$ \\
Breadcrumbs                        & $0.611\ (0.110)$ & $0.673\ (0.197)$ & $\mathbf{0.695}\ (0.357)$ \\
DARE-TIES                          & $0.617\ (0.149)$ & $0.696\ (0.349)$ & $\mathbf{0.706}\ (0.374)$ \\
TIES                               & $0.626\ (0.150)$ & $0.706\ (0.384)$ & $\mathbf{0.708}\ (0.386)$ \\
AdaMerging-layer                   & $0.665\ (0.101)$ & $0.700\ (0.167)$ & $\mathbf{0.712}\ (0.312)$ \\
RegMean                            & $0.723\ (0.234)$ & $\mathbf{0.768}\ (0.511)$ & $0.764\ (0.502)$ \\
\midrule
Calibrated merge                   & $0.751\ (0.551)$ & --- & $\mathbf{0.786}\ (0.594)$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Hyperparameter robustness is where the methods separate.}
Aggregating \emph{every} refinement cell run on this suite (all learning rates, sweep counts, and schedules): of $23$ APR cells, none fell below its own initialization and none fell below the zero-shot floor---the worst APR cell in the entire campaign scores $0.614$, at double its optimal learning rate. Of $86$ ordinary-GD cells, $43$ fell below their initialization and $30$ below the zero-shot floor (to near-chance accuracy ${\approx}0.13$), and annealing does not rescue the aggressive rates. APR's tested safe window spans its full $8\times$ grid and its optimum ($\eta\approx4$--$8$) transfers across all checkpoint-space initializations, whereas GD's one-decade safe window ends in a cliff and its optimum does not transfer (the rate that is optimal from four initializations collapses to $0.482$ from AdaMerging at matched budget). This asymmetry is the practical content of the trust region: with $\gamma\le1$ and clipping applied after the learning rate, no step can overshoot its expert coordinate-wise (Appendix, box-distance monotonicity), so the catastrophic regime that half of GD's grid occupies is provably absent.

\paragraph{Composition, again.}
The two-stage calibrate-then-refine remains the best method at twenty tasks: per-tensor calibration alone reaches $0.751/0.551$, and APR on top reaches $\mathbf{0.786}$ mean / $\mathbf{0.594}$ worst-task---closing $39\%$ of the merge-to-expert-ceiling gap from the best data-free start. RegMean is the strongest label-free initialization ($0.723$ at the matched $64$-input Gram budget; $256$ inputs add only $+0.006$), and the far-initialization step-size law replicates exactly: APR's optimal rate from RegMean is $\eta=0.5$ (interior peak), an order of magnitude below its rate from checkpoint-space merges. AdaMerging must itself be initialized with care at this scale: started at its conventional $\lambda=0.3$ (inside the collapse region) it climbs to only $0.654$ in $300$ steps, while started at $\lambda=0.05$ it reaches $0.665$; both learned endpoints agree on $\bar\lambda\approx0.1$, approached from opposite sides.

RegMean is also the single initialization at which ordinary GD edges out APR (Table~\ref{tab:clip20budget}), and the reason is specific to how RegMean sits in parameter space. RegMean solves each linear layer by matching \emph{outputs} on that layer's inputs, $W=(\sum_i G_i)^{-1}\sum_i G_i W_i$; in input directions with negligible activation variance the solution is essentially unconstrained, so $W$ acquires large components that are functionally inert on in-distribution inputs. The resulting merge point is over $450$ from the base model in parameter norm---larger than the encoder norm itself ($336$)---while scoring a healthy $0.723$, which is only possible because most of that displacement lies in inference-invisible directions. Every ingredient of APR reads the anchor $v_i=\theta_i-\theta$, so from this initialization $v_i$ inherits that inflated, non-functional component: the step $-g\odot|v|$ scales noise on directions where $|v|$ is huge but the gradient is near zero, and the trust region $\gamma|v|$ is widest exactly there. Ordinary GD, whose step depends only on $g$, never moves along those directions and is therefore unaffected. This both explains the lone exception and yields a concrete prediction: anchoring on the functional part of $v$ (equivalently, refining RegMean from a task-vector-defined anchor) should restore APR's parity---an experiment we have not yet run. It also implies that parameter-space distance from base is a valid drift proxy for the task-vector-based methods but not for RegMean, whose functional drift must be measured in output space.

\paragraph{Why sixty-four examples do not overfit---and when they do.}
The strongest labeled-replay result at this scale is also the most surprising: from the RegMean initialization, ordinary GD at its safe rate can be run to \emph{near-interpolation of the replay buffer}---over $S=60$ sweeps ($1{,}200$ full-batch passes over the same $64$ examples per task) the replay loss falls to under $3\%$ of its starting value ($0.8\%$ of the reference merge's)---while test accuracy \emph{rises} to $0.768$ and worst-task holds at $0.51$, with no decline at any horizon. Three structural facts explain the absence of memorization: the zero-shot head is frozen, so the cheap channel for fitting arbitrary labels is unavailable (unfreezing it on the same $64$ examples is catastrophic, Table~\ref{tab:matched}); the total parameter displacement is minute (${\approx}0.05\%$ of the encoder norm, ${\approx}8\%$ of one expert distance), bounding the capacity of the perturbation; and twenty interleaved tasks overwrite each other's sample-specific noise directions while preserving the shared task-restoration signal. Consistent with this account, the class-balanced buffers of the $397$-class SUN397 task cover at most $64$ of its classes, yet all-class test accuracy improves---transfer through shared features by construction. The failures that do occur at aggressive rates are optimization instabilities (immediate, below-floor, schedule-sensitive), not gradual memorization.
...
All twenty-task numbers above select each method's configuration on the evaluation split (symmetrically across methods), which is optimistic in absolute terms. A pilot of the strict protocol splits the historical $64$ labeled examples per task disjointly into $32$ for the refinement sweeps and $32$ held out for selection, and picks each configuration by held-out accuracy. From the RegMean initialization the selection gap is negligible---validation-selected configurations lose $+0.001$ (APR) and $0.000$ (GD) of test accuracy against the evaluation-oracle choice---so the evaluation-selected tables are not materially inflated here. Halving the sweep budget to $32$ costs both methods ${\approx}0.02$ mean accuracy symmetrically, and the RegMean near-tie persists. The held-out signal also catches a GD failure the evaluation oracle would too: at $n=32$ one annealed GD configuration that scored well at $n=64$ collapses to $0.21$, and its held-out accuracy ($0.19$) flags it correctly---honest selection steers around a landmine that APR's grid does not contain.

\paragraph{What is still missing.}
The baseline tiers are now populated (checkpoint-only, label-free with RegMean and AdaMerging, and the matched labeled tier including learned Localize-and-Stitch), and the headline configurations carry error bars. The remaining gaps are protocol-level: the hyperparameter \emph{sweeps} behind Tables~\ref{tab:mergebaselines}, \ref{tab:clip8}, and the twenty-task tables of Section~\ref{sec:clip20} are still single-seed, and are evaluation-selected except for the disjoint-holdout pilot reported there (which found the selection gap negligible); the calibrate-then-refine cells and the six-of-seven scoreboard of Table~\ref{tab:clip20budget} await multi-seed replication; the predicted functional-anchor fix for the RegMean initialization, and direct measurement of held-out-task retention for the minimal-drift from-base operating point, are not yet run; the shared-output track of Section~\ref{sec:t5} is single-seed with refinement grids whose optima are not yet bracketed; and the decoder-LLM setting promised in the experimental protocol remains the subject of ongoing work. The matched-budget comparison already answers the central alternative explanation---that a small labeled sample would help \emph{any} reasonable method equally: at $64$ labeled examples per task, most alternative uses of the same data are markedly worse than the refinement, the strongest single-stage competitors are themselves parameter-space edits of the merge, and the best use of the labels we found is the proposed two-stage one.

\section{Expected Contribution}
The expected contribution is an attribution-patching perspective on replay-guided model merging. Rather than using attribution scores to build a pre-merge sparsification mask, the method starts from a standard task-arithmetic merge and asks a local intervention question: for task $i$, which coordinates would reduce the replay loss if the current model moved toward expert $i$? The answer is the sign of the coordinate-wise product $g_{i,r}v_{i,r}$, which coincides with a trust-region acceptance rule that keeps each step moving toward its expert, so the attribution-patching and replay-refinement views motivate the same procedure. This leads to a simple implementable algorithm: mask out expert directions that are locally loss-increasing, scale the remaining negative-gradient update by distance to the expert, clip each per-task update by a trust-region fraction, and immediately apply that task's update before computing the next task's gradient and gate. Empirically, its strongest form on the CLIP suite is two-stage: first fit per-tensor merge coefficients on the same labeled replay buffer (a supervised counterpart of AdaMerging that cannot be confidently wrong), then run the gated refinement from that calibrated merge---the two stages address the tensor-level mixing error and the coordinate-level interference error respectively. Where the thresholded gate adds nothing over the ungated anchored step (GLUE encoders, and the twenty-task CLIP suite), a longer refinement with randomized task order and an annealed step size is currently the more robust single-stage choice; the anchored trust-region step itself, however, retains its matched-budget advantage and its hyperparameter safety at every scale tested. On the shared-output T5 track (Section~\ref{sec:t5}), which drops the oracle-task-identity assumption entirely, the same picture recurs: the refinement improves every merge it is started from, the best result again comes from composing with a stronger merge rather than from plain task arithmetic, and the two-stage form is the strongest fully inductive configuration. The proposal also contributes a stricter empirical protocol: fair labeled-replay baselines, an aggregated-update ablation, mask-stability tests, systematic GLUE pair/subset evaluations, and secondary shared-head or CLIP/ViT benchmarks. If successful, the method would provide a lightweight bridge between checkpoint-only task arithmetic and full multitask fine-tuning with replay buffers, while making clear which gains come from the attribution gate rather than from supervised replay alone.
