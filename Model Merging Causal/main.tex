...
\paragraph{Longer horizons change the comparison---symmetrically.}
Ordinary GD given six times the budget ($S=30$, with constant and cosine-annealed schedules) closes its matched-budget deficit: its best long-horizon cells reach within $\pm0.008$ of APR's $S=5$ results at every checkpoint-space initialization, and from TIES it briefly overtakes ($0.706$ vs.\ $0.697$). But the comparison must be symmetric, and APR was \emph{not} converged at $S=5$ on this suite (unlike at eight tasks): at $S=30$ with a cosine schedule and random task order, APR regains the lead from both initializations tested (TIES $0.708/0.386$; AdaMerging $0.712/0.312$, with the constant-schedule variant reaching worst-task $0.346$ against GD's $0.167$). The long horizon particularly repairs \emph{worst-task} accuracy for APR ($0.262\!\to\!0.386$ from TIES). Two qualifications favor neither method being declared a clear asymptotic winner: the surviving margins ($\pm0.01$) are within plausible single-seed noise, and long-horizon \emph{constant}-rate GD is unreliable---at its optimal learning rate it ended below its own $S=5$ result at two of five initializations (from Breadcrumbs it ended below the merge itself), a failure the cosine schedule repaired in every case. The honest summary: at matched budget APR wins everywhere; at long budget the two families converge to similar peaks, with APR reaching its peak without schedule tuning and GD requiring annealing to reach its peak reliably.
