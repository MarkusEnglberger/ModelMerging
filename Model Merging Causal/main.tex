...
Every experiment above starts from a uniform task-arithmetic merge, which leaves the central objection unanswered: perhaps the refinement is only repairing a weak initialization, and a better \emph{merge} would make it unnecessary. We therefore evaluate the refinement against, and on top of, the merge methods it is supposed to complement (Table~\ref{tab:mergebaselines}). Four checkpoint-only baselines use no data at all: task arithmetic with a tuned coefficient, TIES \citep{yadav2023ties}, DARE \citep{yu2023dare}, their composition DARE-TIES, and Model Breadcrumbs \citep{davari2023breadcrumbs}. One label-free but data-dependent baseline, AdaMerging \citep{yang2024adamerging}, learns task-wise or layer-wise merge coefficients by minimizing prediction entropy on unlabeled inputs. All families share the same experts, the same replay buffers, the same frozen heads, and the same per-family tuning rule.

\paragraph{The refinement composes with better merges rather than replacing them.}
On both suites APR improves \emph{every} merge initialization, and the best result is obtained on top of the strongest merge, never on top of plain task arithmetic. On CLIP-8 it lifts AdaMerging from $0.610$ to $0.722$ mean and from $+0.219$ to $+0.429$ worst-task; on GLUE-8 it lifts DARE-TIES from $0.301$ to $0.610$. This is the clearest evidence that the gated replay update is doing something the interference-aware merges are not: it is complementary to them, not an alternative. Notably, the CLIP-8 composition uses only $64$ training examples per task in total---the same $64$ inputs unlabeled to fit the merge coefficients, then labeled for the refinement---so it never touches the evaluation split.

\paragraph{But the refinement is not a standalone winner.}
Layer-wise AdaMerging ($0.610$) slightly exceeds APR started from the tuned task-arithmetic merge ($0.588$) on CLIP-8, and it does so \emph{without labels}. We tested the obvious explanation---that AdaMerging benefits from its standard transductive protocol, which draws unlabeled inputs from the evaluation split in effectively unlimited quantity---by restricting it to exactly the $64$ unlabeled inputs per task that APR sees labeled. It does not degrade ($0.604\to0.610$ mean; $+0.165\to+0.219$ worst). The data advantage therefore does not account for the gap, and the honest claim is the narrower one: APR is a refinement that stacks on any initialization, including the strongest, and is not the best way to spend a small budget on its own.

A plausible mechanism is a mismatch of parameterization rather than of information. Inspecting the learned layer-wise coefficients shows that AdaMerging does not merely rescale the merge---the spread of $\lambda_i^l$ across tensors within a task ($0.151$) is comparable to its mean ($0.258$), attention projections are amplified to nearly twice the initial coefficient while embedding and bias contributions are driven toward zero, and $38\%$ of coefficients \emph{increase} relative to initialization. The dominant recoverable error in the CLIP merge is thus a per-tensor mixing error, living in a well-conditioned space of $\sim\!1600$ coefficients. AdaMerging searches exactly that space; APR searches $\sim\!10^8$ coordinates with a step family---coordinate-wise moves \emph{toward} the experts---that cannot express ``double this tensor's contribution beyond the merge''. Consistent with this reading, the \emph{task-wise} AdaMerging variant, which has only eight degrees of freedom, scores $0.344$, worse than a single tuned global coefficient ($0.407$); the useful knowledge is which tensors to re-mix, not which tasks to weight.
...
\paragraph{The optimal step size depends on the initialization.}
APR's update and its trust region both scale with the expert distance $|v_{i,r}| = |\theta_i - \theta^{(0)}|$, which differs across merge points. A learning-rate grid centered on the task-arithmetic optimum therefore diverges when the refinement is started from the sparser TIES or DARE-TIES merges---in our first attempt, every such cell failed, and we initially mis-read this as evidence that the refinement does not compose. Re-centering the grid recovers interior optima at every initialization. Measuring the geometry confirms the mechanism: relative to task arithmetic, the TIES merge is \emph{closer} to six of the eight GLUE experts but substantially farther from the two large-data ones (QQP, MNLI), so at a fixed $\eta$ those two tasks take steps $1.6\text{--}2.2\times$ larger and trample the fragile MRPC. In every diverged cell, exactly one fragile task collapses---MRPC on GLUE, Cars on CLIP.

\begin{table}[t]
\centering
\small
\caption{Merge baselines and composition (single seed, $n=64$, $S=5$). Mean normalized retention, worst-task in parentheses. Every family shares the same experts, replay buffers, and heads; hyperparameters ($\lambda$, density, sparsity, $\eta$) are tuned per family on the evaluation split (see Section~\ref{sec:results}). $^\ddagger$: standard \emph{transductive} AdaMerging draws unlabeled inputs from the evaluation split; the CLIP rows instead use the matched budget (the same $64$ unlabeled inputs per task that APR sees labeled), which \emph{improves} on transductive ($0.333$ / $0.604$ for task/layer wise), and the matched layer-wise merge is the initialization for the last row (making that composition fully inductive). On GLUE both budgets fail alike (transductive $0.023$ / $0.173$; matched $0.029$ / $0.198$ for task/layer wise).}
...
\midrule
\multicolumn{3}{l}{\emph{Labeled replay ($n=64$): APR started from each merge}}\\
APR $\leftarrow$ task arithmetic          & $0.588\ (0.185)$           & $0.595\ (0.108)$ \\
APR $\leftarrow$ Breadcrumbs              & $0.606\ (0.212)$           & $0.523\ (0.095)$ \\
APR $\leftarrow$ DARE-TIES                & $0.626\ (0.190)$           & $\mathbf{0.610}\ (0.178)$ \\
...
Localize-and-Stitch (magnitude, data-free)& $0.520\ (+0.22)$ & $0.284\ (+0.04)$ \\
Ordinary replay GD                        & $0.458\ (-0.20)$ & $0.504\ (-0.02)$ \\
Ungated distance-scaled                   & $0.521^\S\ (-0.32)$ & $\mathbf{0.637}\ (+0.18)$ \\
\textbf{APR (proposed)}                   & $\mathbf{0.598}\ (+0.16)$ & $0.570^\S\ (+0.09)$ \\
\bottomrule
\end{tabular}
\end{table}
